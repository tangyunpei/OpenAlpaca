...
## Current Work — read first (as of 2026-09-11, branch `feat/ui-rework`)

**The plan and the design are implemented.** `tasks/api-fix-plan.md` Phases 0–8 all landed on `feat/ui-rework` (`2656a55..ab31299`), and the extension enable/disable design's C1–C8 landed with them between Phases 1 and 2 (`5c105b8..97740a5`; C1 is `e92d76e`, C8's purge `eff30cb`). Migration head is **041**. The GUI still ships no mock data, and the honest-"unavailable" registry is down to **2** entries (`apps/openalpaca-gui/src/lib/unavailable.ts`, `GapId = "GAP-17" | "GAP-20"`): adding a connector, and the per-template enable toggle — both narrowed to the half nothing serves, not whole surfaces. One planned item was deliberately never built (Phase 0 item 6 / GAP-22, under owner decision T14; C7 deleted the six `plugin_*` variants it would have amended). **What remains: the owner decisions — plan §0's T1–T19 and design §13 Q5–Q14, each a recommendation, none applied — and merging this branch.** Documents of record:

| Document | Role |
|---|---|
| `tasks/api-fix-plan.md` (rev 3.4 — **implemented; the record of what shipped**) | Phases 0–8, migrations 035–041 (§11 ledger). Each `### Phase N` heading carries the commit range it landed in and the deviations recorded while building it; §0 holds every settled decision (D1–D5, N1–N5), the four implementer pre-checks and the pending owner decisions T1–T19; §12 holds the residue the whole-branch review refuted — read it before re-raising any of those four. |
...
- **Everything lives under `~/.openalpaca/`** (D1, landed — `crates/openalpaca_storage/src/store/mod.rs`; `OPENALPACA_HOME_STORE` overrides the root, absolute paths only, empty/relative rejected): `state/` holds the DB (+ `-wal`/`-shm`), discovery, lock, master key, interim assets, backups, and logs; `config/` and `plugins/` sit beside it; content dirs (`artifacts/`, `uploads/`, `sessions/`, …) complete the root, and a project store (`<project>/.openalpaca/`) has the same content shape — `content_dir(scope, kind)` is `root/<kind>` in both. **`state/` is the machine's; everything else at the root is the human's**, and a new content kind exists only when it is added to `ContentKind`. First boot of the rebuilt binaries moves the legacy `~/Library/Application Support/OpenAlpaca` contents into place (idempotent, resumable, **not reversible** — `store/migrate.rs`); a still-running old daemon blocks the move, and a database found at both roots refuses to start until one is moved aside.
- **Owner decisions pending** — the plan's §0 table, rows **T1–T19** (added during execution: T16 scoping the unscoped task read/action routes, T17 `.trash/` retention, T18 a CLI turn from another project switching the shared `{owner}:gui` lane's session under R48, T19 `[web_search] api_key` stored in the clear); design twins at §13 Q5–Q14. Each carries a recommendation, none is applied, and the shipped code is the "no change" answer. A "default" that adds an enforcement point, widens a component's job, or reverses a row of an accepted design is a decision — present it, never adopt it silently.
