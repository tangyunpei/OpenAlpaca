...
**The plan and the design are implemented.** `tasks/api-fix-plan.md` Phases 0–8 all landed on `feat/ui-rework` (`2656a55..ab31299`), and the extension enable/disable design's C1–C8 landed with them between Phases 1 and 2 (`5c105b8..97740a5`; C1 is `e92d76e`, C8's purge `eff30cb`). Migration head is **040**. The GUI still ships no mock data, and the honest-"unavailable" registry is down to **2** entries (`apps/openalpaca-gui/src/lib/unavailable.ts`, `GapId = "GAP-17" | "GAP-20"`): adding a connector, and the per-template enable toggle — both narrowed to the half nothing serves, not whole surfaces. One planned item was deliberately never built (Phase 0 item 6 / GAP-22, under owner decision T14; C7 deleted the six `plugin_*` variants it would have amended). **What remains: the owner decisions — plan §0's T1–T17 and design §13 Q5–Q14, each a recommendation, none applied — and merging this branch.** Documents of record:

| Document | Role |
|---|---|
| `tasks/api-fix-plan.md` (rev 3.3 — **implemented; the record of what shipped**) | Phases 0–8, migrations 035–040 (§11 ledger). Each `### Phase N` heading carries the commit range it landed in and the deviations recorded while building it; §0 holds every settled decision (D1–D5, N1–N5), the four implementer pre-checks and the pending owner decisions T1–T17. |
