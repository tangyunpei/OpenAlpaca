...
Single SQLite connection wrapped in `Arc<Mutex<Connection>>`. WAL mode, `busy_timeout=5000ms`. Schema managed via numbered migrations (**head 040** — `crates/openalpaca_storage/src/migrations/mod.rs`, asserted at `database/tests.rs:11`; the next one is 041). Memory search is hybrid: FTS5 full-text + sqlite-vec 768-dim KNN with cascading scope (workspace → global).
...
- `.gitignore:92` blanket-ignores `*.md`. A new doc is invisible to git until whitelisted with a `!path` line (`.gitignore:93-114`, or the UI-rework block at `:133-139`); confirm with `git check-ignore -q <file>` (exit 0 = still ignored) before calling it committed.
