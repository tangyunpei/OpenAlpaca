...
**Revision:** rev 3, 2026-09-22 — the daemon-lifecycle and first-run surfaces the design never drew (§3.32 `StopDaemonDialog` and the stopped state, §3.35's `Danger ghost` label, §5.1 the first-run card, §5.4 / §5.4a the Connection stop control and the key form, §5.5's third overlay, §5.6's shipped notes), built to the existing tokens. The design's own strings, tokens and layout are unchanged. See the [revision log](#revision-log) at the end.
...
Preserve the existing connection plumbing conceptually. _(Shipped: the SvelteKit build's `src/lib/daemon.ts` this pointed at is gone; its React port is `apps/openalpaca-gui/src/lib/connection.ts` — the two Tauri commands and the `instanceId` guard — and `src/lib/events.ts` — the socket, the backoff and the resync signal.)_
