...
**Revision:** rev 4, 2026-09-22 — the daemon-lifecycle and first-run surfaces the design never drew (§3.32 `StopDaemonDialog` and the stopped state, §3.35's `Danger ghost` label, §5.1 the first-run card, §5.4 / §5.4a the Connection stop control and the key form, §5.5's third overlay, §5.6's shipped notes), built to the existing tokens. The design's own strings, tokens and layout are unchanged. See the [revision log](#revision-log) at the end.
...
| `JetBrainsMono-Variable.woff2` / `SpaceGrotesk-Variable.woff2` (in the SvelteKit app's `static/fonts/`) | **NOT used.** Zero occurrences of "JetBrains" or "Space Grotesk" anywhere in the design. They were leftovers from the SvelteKit app being replaced and were deleted with it — `static/fonts/` no longer exists. Do not reintroduce them.                   |
...
_Shipped addition — not in the design:_ a **conversation column** sits to the **left** of the transcript (`components/chat/SessionSidebar.tsx` — the lane's conversations with `New chat`, rename, archive, delete and `Show more`). `width = chatSessionsW` (default 236, min 200, max 360), `flex-shrink:0`, followed by its own 7px resizer (direction `+1`). Collapsed, it leaves a mono `Conversations` button in the chat header as the way back.
...
### 2.7 Resizers (3 identical in the design; 4 shipped)
...
| Conversations ↔ chat _(shipped; not in the design)_ | `chatSessionsW` | 236     | 200 | 360 | `+1`                                 |
...
- _Shipped:_ the body is not a single `<p>`. `components/chat/prose.ts` parses the markdown a model actually writes — paragraphs, bullet and ordered lists (an ordered list keeps its own start number), `**bold**` / `*italic*` (either may wrap a code span), inline code, `#`–`####` headings, thematic breaks, one level of `>` blockquote, fenced code with a language label and its own scroll, and pipe tables — into elements in this section's type. It is a fixed vocabulary with **no HTML and no dependency**, it parses incrementally so a half-received fence or table never flashes raw markup, and `_underscores_` are deliberately not emphasis. `marked` + DOMPurify stay the artifact renderers' business (§3.25).
- _Shipped:_ a user turn's attached files are drawn as cards under the message, read from the stored row's `content_json` file parts; the row's text is `content`, never the daemon's `display_text` with its `[Attachments: …]` suffix.
...
- _Shipped (S2):_ when the turn carries SSE `reasoning {text}`, the `thinking…` label becomes a disclosure button — `thinking… (hide)` / `thinking… (show)`, open by default — over a panel in the same faint mono meta type on the raised surface: `max-height:88px` with its own scroll, 1px border, `border-radius` md, `padding:6px 9px`, `margin-bottom:8px`. It holds the last 4 000 characters, follows the newest text while it is pinned to the bottom, and is live only — reasoning is never stored and never part of `done.content`.
...
_Shipped:_ the note's subject is whoever raised the prompt — the agent template's display name (from `GET /v1/agent-templates`, so it reads the same live and after a reload) for a workflow's prompt, and `Alpaca` for the main loop, whose `agent_id` is `orchestrator`. The banner is held against the **conversation**, not the turn's stream: a workflow's prompt arrives on the WS after its turn's `done`, and `GET /v1/chat/confirmations` seeds the cards on load and on reconnect (API_MAP §4.1).
...
_Shipped:_ a third label, `Timed out` (T1) — the daemon's wait ran out and the tool did **not** run: "`<tool>` timed out — not run. Nobody answered in time, so the agent continued without it." The approved row starts as "`<tool>` approved · waiting for the tool to run…" and is upgraded in place by the WS `tool_executed` frame that matches it on tool name, time **and** owner (`agent_id` + `task_id`, F5). A prompt answered elsewhere or withdrawn clears the card and leaves no row. The rows are kept in `views/chat/resolution-store.ts`, so they outlive a trip to another view; a conversation switch or a reload drops them. The same box shape carries the muted `Not sent to the model` note (§5.6).
...
2. **Input shell** — `position:relative; display:flex; gap:9px; align-items:flex-end; border:1px solid #DCD6CB; border-radius:10px; background:#FFFDF9; padding:7px 7px 7px 11px`
   ...
   - _Shipped:_ a ghost xs **`Attach`** button sits left of the model button, with the draft chips in a strip above the textarea (§5.6). Below a 180px textarea the three controls wrap to a row of their own, and the model label elides at 150px (§8.7).
...
- **StopDaemonDialog** (Connection, §5.5's third overlay — _not in the design; built to the existing tokens_): the palette's veil and dialog shell (`rgba(30,29,27,.28)`, z-50, dismissed only on the veil itself; `rounded-4xl`, popover border, dialog shadow), 520px wide. Title `13.5px/600` "Stop the OpenAlpaca daemon?"; body paragraphs `12.5px; line-height:1.6; secondary`, saying what a stop costs and nothing more (nothing drained; running workflows come back `interrupted` with Rerun; typed interjections kept as follow-ups; a streaming reply lost; a pending approval dropped; nothing deleted; connector users meet silence — shown only when a connector is running). One mono `11px` count line from `GET /v1/status.busy` ("Right now: 2 workflows running · …"), or a generic "cannot tell" line when the daemon served none. Actions right-aligned: `Cancel` (secondary sm, **default focus**, Escape) and `Stop daemon` (the existing **danger ghost** — no filled danger variant is added, T25), disabled until the dialog's fresh status read has settled and while the stop is in flight. No typed word (T33).
- **Stopped state** (Connection card): the `StatusCard` dot takes the muted token (`#8A8578`), never the error red; title `Daemon stopped` / `Daemon stopped from elsewhere`; meta `stopped <relative time>`; `Start daemon` (secondary sm) replaces `Reconnect`, and `Stop daemon…` (ghost sm) is hidden. **A stop from this window is titled by what the shell's wait concluded, not by the request:** from the shutdown POST until `await_daemon_stopped` answers the title is `Stopping the daemon…`, the meta `waiting for the process to exit`, and the button reads `Stopping…` and is disabled — the old process can hold its single-instance lock for up to 15 s, and a daemon started in that tail loses the lock race and exits. Then `Daemon stopped`, or `Daemon did not stop` / `Daemon exited, lock still held` / `Daemon stop not confirmed` with meta `stop asked <relative time>` (and `Start daemon` enabled again). The rail's `ConnectionRow` says the same in its own words: `stopping`, `stopped`, `still running`, `lock still held`, `stop unconfirmed`.
...
| **Danger ghost** (Cancel · Stop daemon, T25)  | transparent | `1px #E3C9B8` | `#8C2F1E` | 6      | `5–6px 10–11px`        | 11.5–12/500       | `#F7E7DE`              |
...
**Persistence:** `localStorage["oa-pane-widths"] = JSON.stringify({workW, workListW, libListW})`, read in `componentDidMount` inside a `try/catch` with per-key fallbacks to the defaults. Reuse this exact key and shape so an existing user's layout survives the rework. _(Shipped: the key is unchanged and the payload gained a fourth member, `chatSessionsW` — §2.7. `parsePaneWidths` in `stores/pane-widths.ts` falls back **per key**, so an older three-key payload keeps the widths it carries.)_
...
| ----------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| **Connection**    | —     | Two `StatusCard`s (daemon status grid; today's spend/runs/tokens + progress bar) — plus `Stop daemon…` / `Start daemon` (§3.32) | —                                            |
| **Models & keys** | 3     | `ListCard` with model chips per provider                                                                                        | `Add key` _(design: `Add provider` — §5.4a)_ |
...
- Connection: "Daemon status, endpoint and today's spend against the cap." _(one correction: there is no daily budget and none is coming — **N4** — so today's figure has no cap to be "against"; the two caps the daemon enforces on a turn or a workflow are per workflow and per agent turn, which is what the panel's own note prints (three background jobs carry small daily ceilings of their own — `[orchestrator.costs] *_max_daily_cost_usd` — which the panel does not show). Shipped copy: "Daemon status, endpoint and today's spend; caps are per workflow and per agent turn.")_
...
### 5.4a First-run setup (D-F, D-G)

No design exists for this; it is built from the daemon's own copy and the shapes already on screen, and it is **not** a wizard, a modal or a persisted flow.

**"Setup completed" is derived (D-G).** It is `GET /v1/status` → `llm.effective_default_model !== null`, read by `views/chat/first-run.ts` (`firstRunState`) and nowhere else. No flag, no `localStorage` key, no config key, no status field, no migration. Three states, told apart with `=== undefined` / `=== null` — never with a falsy test:

| `effective_default_model` | Meaning                                                              | Empty transcript renders           |
| ------------------------- | -------------------------------------------------------------------- | ---------------------------------- |
| `undefined`               | the status has not answered yet, or the daemon serves no `llm` block | **nothing** — no card, no skeleton |
| `null`                    | the daemon's own answer: it can route no model                       | the first-run card                 |
| a string                  | a model will answer                                                  | nothing                            |

`GET /v1/status` has no `initialData` and a first boot answers slowly (the embedding model downloads first), so reading `undefined` as "no model" would flash the card at every cold start of a configured install. The card (`views/chat/FirstRunCard.tsx`) is drawn only while the transcript is empty, above the error lines, in the Models banner's amber treatment, `role="status"`: a fixed title, the daemon's reading of its `llm` block (`effectiveModelNote(llm, "elsewhere")`, `lib/model-availability.ts` — the same four branches the Models banner reads, with the call to action naming the screen), a line saying a local model needs no key, and one `Open Models & keys` button. It never blocks the composer.

**Inline, never modal.** `effective_default_model` is a liveness fact, re-read on every 30 s poll: quit Ollama or let a cloud key lapse and it goes `null` again. A modal derived from it would re-fire in the middle of someone's work over a condition that often heals by itself; a card in an empty transcript that vanishes on the next poll costs nothing when it is wrong. Three writes can make a model routable and each invalidates `qk.statusAll()` so the card goes without waiting for the poll: the provider enable, `Refresh models`, and — new with this section — a key save (and, as its mirror, a key removal).

**The key form (D-F).** `Add key` in the Models & keys card header (the design's `Add provider`: there is no provider to add — the three are compiled in and `GET /v1/settings/llm` enumerates all of them) and on each provider row opens `views/settings/AddKeyForm.tsx`, a port of `openalpaca llm keys add`:

| CLI (`keys_add`)                                     | GUI                                                                                                                                                                                                                         |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| provider `Select`                                    | `role="radiogroup"` of buttons, one per key of `GET /v1/settings/llm`'s `providers` — never a hardcoded three                                                                                                               |
| hidden `API key secret`                              | `<input type="password" aria-label="API key">` — never logged, toasted or put in a query key                                                                                                                                |
| automatic validate                                   | an explicit `Check key` button — advisory, cleared when the key changes, never gates a save (`POST …/validate` answers `504 KEY_VALIDATION_TIMEOUT` for every failure; the reason is `format_error`, there is no `message`) |
| priority `primary` / `fallback`                      | `<select aria-label="Priority">`, default `primary`                                                                                                                                                                         |
| source (display labels)                              | `<select aria-label="Source">` sending the snake_case `KeySourceValue`s                                                                                                                                                     |
| notes, optional                                      | `<input aria-label="Notes (optional)">`, sent only when non-empty                                                                                                                                                           |
| body `{provider, key: {id: "<provider>_<unix>", …}}` | the same, via `useUpsertKey` — **`id`, never `key_id`**, which serde drops                                                                                                                                                  |

**A switched-off provider is refused before any typing.** A key saved there is accepted (`200`), written to `llm.toml` and ignored — the router drops a disabled provider's models from its catalogue and only the enable restores them. The form therefore shows the refusal the moment such a provider is selected — the consequence, the mechanism, the instruction — with a `Turn <provider> on` button that is the section's own provider-toggle mutation. It is not an auto-enable and not save-then-offer: nothing flips the bit as a side effect of opening, typing or saving. `Save key` is guarded twice — disabled in the DOM, and re-planned in the handler (`keySavePlan`, `views/settings/key-copy.ts`) — because the invariant is "no `PUT /v1/settings/llm` for a disabled provider". "Enabled" there is the daemon's word, not the settings cache's: the provider toggle writes the cache optimistically, so while any switch is in flight the plan answers `wait-switch` — one muted line, `Save key` off — and a switch that then fails cannot leave a key behind on a provider that stayed off.

**A provider that needs no key** (`requires_key: false` — Ollama) gets no key field, no check, no priority/source/notes and no save: the page says turning it on is the whole setup, with the switch if it is off. While such a provider is off the window **cannot** tell whether it is reachable — the daemon never asks a disabled provider, `POST /v1/models/refresh` asks loaded ones only — so nothing claims either; the enable's own `200` (`discovered_models` / `discovery_error`) is the first answer, and its toast names an unreachable one as `… could not be reached (<reason>) — start it and press Refresh models`.

**Stored keys.** Each provider row lists its keys under its description — `masked_secret` as the daemon masked it (never unmasked, copied or logged), priority, source and the `status` `GET /v1/settings/llm` carries (`unknown` reads `health unknown`, not a guessed health; no second health request). `KeyInfo.source` is read as a plain string: the CLI stores display labels there, so an unknown value renders verbatim (`views/settings/key-rows.ts`). A non-primary key offers `Make primary` (`PUT …/keys/priority`, body `key_id` — unlike the upsert's `id`); `Remove` arms on the first click and deletes on the second (`DELETE …/keys/{provider}/{key_id}`), and any blur disarms it.
...
- **Stop daemon confirmation** — Settings → Connection only; the palette's veil (§3.32 `StopDaemonDialog`).

### 5.6 Things shown only as a hint (no full design exists)

| Hint                                                  | Where                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `⌘N`, `⌘⇧S`, `⌘2`, `⌘3`, `⌘⇧D`, `⌘,` shortcuts        | Palette rows only — **not bound** _(Shipped: all bound for real by `components/overlays/useCommandShortcuts.ts`, plus `⌘1`; `↵` stays `useGlobalKeys`'s — §4.5.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `Export` / `Reveal`                                   | Library header buttons with no handler _(Shipped: both wired. `Export` saves the content blob through an `<a download>`; `Reveal` calls `POST /v1/files/{id}/open`, which opens the file in its default app rather than revealing it, and the toast says so.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `Reconnect` / `Copy log path`                         | Settings connection buttons with no handler _(Shipped: both wired, and joined by `Stop daemon…` — the §3.32 confirmation, then `POST /v1/command` and the shell's `await_daemon_stopped` — and `Start daemon`, which replaces `Reconnect` while stopped — a re-bootstrap plus a socket reopen, and `GET /v1/status.log_path`, inert with the reason when it is `null`, which since T30 means only a daemon started by hand. Beside them, `Show daemon log` (ghost xs) opens the last 200 lines of `daemon.log` read by the shell, and an unreachable daemon's reason — for a failed start, the end of what it wrote to its log — is drawn under the cells as a mono block with its own scroll, never a toast.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `Add provider` / `Connect service` / `Install plugin` | Only toast a placeholder _(Shipped: `Install plugin` became `Add extension` and is real — plugin or MCP server, with a dry run. `Add provider` became **`Add key`** and is real — the key form (§5.4a), wired to `PUT /v1/settings/llm` and `POST /v1/settings/llm/validate`, refusing a disabled provider per D-F; there is no "add a provider" verb because the three providers are compiled in and `GET /v1/settings/llm` enumerates all of them. `Connect service` still toasts: no route adds a connector, GAP-17.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `Manage providers & keys ↗`                           | Navigates, but the target section has no key-entry UI _(Shipped: the target section edits keys now — add, remove and re-prioritise, §5.4a.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Settings toggles                                      | Rendered from data; **no toggle handler exists** — they are display-only in the mock _(Shipped: real for providers, connectors and extensions; the per-template toggle is not served — GAP-20.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Search                                                | **There is no search input anywhere** except the palette's; the palette is the search                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Attachments / file upload in the composer             | **Built (U5) to this row's own prescription:** a ghost xs `Attach` button left of the model button, opening a hidden `<input type="file" multiple>` — no paperclip, and still no drop zone (a drop on the composer is accepted silently; the chips are the feedback). Picking, pasting an image, or dropping uploads at once through `POST /v1/files/upload`; each file becomes a chip above the textarea carrying name · size · `uploading…` / `✓` / the daemon's own refusal, with `✗` to remove. Ten per turn — the daemon's `TOO_MANY_ATTACHMENTS` cap, refused in the UI with its sentence — `Send` is off while any upload is in flight, and the chips clear when the daemon accepts the turn — **or when the conversation changes under them** (F6: they belonged to the conversation they were attached in, and a file picked in one used to ride the next one's first message; an upload still in flight then lands as a no-op). The composer's draft _text_ is window-level and is not cleared on a switch; per-conversation drafts would be a decision, not a fix. `done.attachments_skipped` (U3) prints a muted `Not sent to the model` note under the answer, in §3.15's shape. |
| Notifications / errors                                | No error banner or connection-lost state is designed. Extrapolate from `#F7E7DE` / `#E9D3C6` / `#8C2F1E`. _(Shipped: a **stopped** daemon is a state, not an error — the rail chip and the Connection dot take the muted token and read `stopped` / `stopped elsewhere`, the composer's Send closes with "The daemon is stopped — start it to send a message.", nothing polls and no toast repeats; a daemon that went away unasked keeps `disconnected`, red, and its reconnect ladder.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| ------------------------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `static/fonts/JetBrainsMono-Variable.woff2` | **No**                            | Not used. **Deleted** with the SvelteKit app — `static/fonts/` no longer exists (`static/` holds `favicon.png`, `tauri.svg`, `vite.svg`).                                                                                                                                                                                                                                                                                                                                                                                                |
...
Preserve the existing connection plumbing conceptually. _(Shipped: the SvelteKit build's `src/lib/daemon.ts` this pointed at is gone; its React port is `apps/openalpaca-gui/src/lib/connection.ts` — the Tauri commands (four since T30: `ensure_daemon_running`, `get_connection_info`, `read_daemon_log_tail`, `await_daemon_stopped`) and the `instanceId` guard — and `src/lib/events.ts` — the socket, the backoff and the resync signal.)_
...
| Assistant message body                           | `POST /v1/chat` → `{stream_id, lane_key}` → `GET /v1/chat/stream/{stream_id}?token=` SSE `delta {content}` as a live preview; `done.content` **replaces** it                                                        |
| `StreamingIndicator`                             | SSE `thinking`; SSE `reasoning {text}` fills its disclosure panel (§3.11)                                                                                                                                           |
...
| `ResolutionRow`                                  | local echo of the confirmation response + WS `tool_executed`; WS `tool_confirmation_resolved {outcome: "timed_out"}` writes the `Timed out` row                                                                     |
...
| Event log rows (`tag`)                           | `tool` ← `tool_executed`/`tool_confirmation_requested`/`tool_confirmation_resolved`; `spawn` ← `subagent_span`; `artifact` ← `artifact_written`; `steer` ← `workflow_steered`; `run` ← `task_status` and the others |
...
| `spend` in the composer hint + Settings "Today"  | composer: `GET /v1/orchestrator/config.daily_cost_usd`; Settings: `GET /v1/usage/summary?window=today`. `llm_call_completed` only invalidates the usage queries                                                     |
...
7. **No responsive design.** The artboard is a fixed 1440×900 desktop window. All panes flex, but no breakpoints exist. Minimum sensible window: rail 196 + min transcript ~500 + min aside 300 ≈ **1000px** wide. _(Shipped, T2: the window floor is real — `minWidth: 1000, minHeight: 640` in `src-tauri/tauri.conf.json` — and the chat view alone gives way under it, because refusing to be narrower than the sum of its columns drew the aside outside a 1080-point window. `components/shell/pane-fit.ts` keeps a 440px transcript and collapses the conversation column first, then the aside, when `196 + 440 +` the open columns (each with its 7px resizer) no longer fits. It is collapse-only and runs on mount and on a window resize, so a column re-opened by hand stays open until the next resize. The composer follows: its control cluster wraps to its own row below a 180px textarea, and the model label elides at 150px with the full id on its `title`. Work, Library and Settings keep a 1000px floor — none of their columns collapses.)_
...
- **rev 3 (2026-09-22, same branch).** Two review fixes to surfaces rev 2 added, no new token or variant. §3.32's stopped state gains the _stopping_ phase and titles a stop from this window by what `await_daemon_stopped` concluded (`Start daemon` is `Stopping…` and disabled until then), with the `ConnectionRow` words to match. §5.4a's key form waits for an in-flight provider switch (`wait-switch`) before it will save, because the toggle's optimistic cache write is not the daemon's answer.
- **rev 4 (2026-09-22, same branch).** Documentation only, no surface changed. §7's shipped note said `connection.ts` wraps "the two Tauri commands"; since T30 the shell registers four (`ensure_daemon_running`, `get_connection_info`, `read_daemon_log_tail`, `await_daemon_stopped`) and `connection.ts` wraps all of them.
