...
- **Stopped state** (Connection card): the `StatusCard` dot takes the muted token (`#8A8578`), never the error red; title `Daemon stopped` / `Daemon stopped from elsewhere`; meta `stopped <relative time>`; `Start daemon` (secondary sm) replaces `Reconnect`, and `Stop daemon…` (ghost sm) is hidden.
