...
| Event log rows (`tag`)                           | `tool` ← `tool_executed`/`tool_confirmation_requested`/`tool_confirmation_resolved`; `spawn` ← `agent_status`; `artifact` ← artifact writes; `steer` ← `workflow_steered`; `run` ← `task_status`/`workflow_started` |
