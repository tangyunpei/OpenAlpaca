...
| -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Run list, `{{ activeCount }} active · {{ doneCount }} done`                            | ✅ `GET /v1/tasks?status=active\|<status>&created_by=&limit=`                                                                                                             | —                                             | **array** of serialized `Task` + injected `outcome` + injected `cost_usd: f64` (GAP-08b, resolved `a827dcf`); the `assigned_agents[]` array was deleted with **P8** — a run's agents are lanes on `GET /v1/tasks/{id}/timeline`                                                                                                                                                                                                                 |
...
| `12 runs` per template                                                  | ✅ **GAP-20 counts half, closed** — `run_count` / `last_run_at` on each `TemplateResponse`, from one grouped `SELECT template_id, COUNT(*), MAX(started_at) FROM subagent_span GROUP BY template_id`. Lifetime, not a 7d window: spans start at spawn, so an in-flight run counts. (`AgentMetrics` remains instance-keyed and unrelated.) |
...
`Find <filename>` → `GET /v1/artifacts?q=`, `Toggle compact density` → client state).
...
### GAP-20 — Agent templates have no enabled state (counts half closed)

**UI needs:** `12 runs` per template row (**served**) and the per-template on/off toggle
in Settings → Agents (**still missing**).

**Counts — closed with P8.** `TemplateResponse` gained `run_count` and `last_run_at`,
filled from one grouped query over the table the run timeline already writes:
...
result and reports `0`. The count is **lifetime and includes runs still in flight** —
a span opens when the subagent is spawned — so the design's `7d` window is not implied
anywhere: the row reads `12 runs · last 4 Sep`. The pre-P8 proposal counted
`agent_task_history`, which only ever knew about runs that had already returned.
...
- **Two list styles exist too.** `GET /v1/tasks`, `GET /v1/connectors`,
  ...
  return **bare arrays**; `GET /v1/conversations`, `/v1/chat/history`, `/v1/orchestrator/latency`,
...
- **`GET /v1/tasks` and `GET /v1/tasks/{id}` still disagree.** The list row is a
  flattened `Task` carrying `outcome` and `cost_usd`; the detail route nests it as
  `{ task, outcome }` and has no `cost_usd`. The agent-run half of the disagreement is
