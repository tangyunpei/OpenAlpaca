...
| The React client                         | `apps/openalpaca-gui/src/lib/connection.ts`, `events.ts`, `chat-stream.ts`, `query-client.ts`, `src/lib/api/*.ts`                                 |
...
```jsonc
{
  ...
  "build": { "version": "0.1.0", "protocol": 1 },
}
```
...
| `ensure_daemon_running` | Reads discovery; probes liveness by TCP-connecting to `listen.host:listen.port` with a 300 ms timeout (`daemon_is_alive`); if dead, spawns the `openalpacad` sidecar detached (`setsid` on unix, `DETACHED_PROCESS` on windows) with `OPENALPACA_CONFIG_DIR=<app_dir>/config`, its stdout/stderr appended to `state/logs/daemon.log` (rotated first, `OPENALPACA_MANAGED_LOG=1` set — T30), then polls `read_discovery()+daemon_is_alive` 25 × 200 ms (≈5 s cap). On timeout the rejection ends with the last 40 lines _that spawn_ wrote to the log, or says it wrote none. | `ConnectionInfo`   |
| `read_daemon_log_tail`  | `{ lines }` → the last `lines` lines of `state/logs/daemon.log` via `store::read_log_tail` (at most 64 KB from the end, no line cut mid-way, ANSI colour removed); `""` when there is no log. Reads the file, not the daemon, so it answers with nothing serving.                                                                                                                                                                                                                                                                                                            | `string`           |
...
- The client's reconnect loop does this (`src/lib/events.ts` over
  `src/lib/connection.ts`): on WS close it re-invokes `get_connection_info`
  (`refreshConnection`) and, if `instanceId` changed, re-bootstraps through
  `ensure_daemon_running` — the event ring is dropped and an `instance_changed`
  resync signal goes out — instead of just reopening the socket.
...
8. **The stop intent** (`lib/connection.ts`, `getStopIntent`/`setStopIntent`):
   `null` (the window should have a daemon), `"stopped_here"` (this window
   stopped it — set, and the socket closed, _before_ the POST) or
   `"stopped_elsewhere"` (a `daemon_shutting_down` frame arrived unasked). While
   it is non-null no reconnect ladder climbs, `GET /v1/health` and
   `GET /v1/status` stop polling, and **nothing respawns the daemon** — the way
   back is an explicit `Start daemon`, which clears the intent and re-bootstraps.
   A refused POST puts the window back as it was; a POST that never reached the
   daemon climbs the read-only ladder (`resume()`), never `ensure_daemon_running`,
   which would spawn one (`lib/daemon-control.ts`).
...
| Streaming reply                                            | ✅ `GET /v1/chat/stream/{stream_id}?token=` (SSE)                                                                                                                                                                                                                                                              | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | events `thinking` / `reasoning` / `delta` / `done` / `error` / `confirmation_requested` / `confirmation_resolved` — see §4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Assistant meta line `sonnet-4-6 · 3.8s · 1284/612 tok`     | ✅ SSE `done`                                                                                                                                                                                                                                                                                                  | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `{ content, model, tokens_in, tokens_out, duration_ms, attachments_used?: string[], attachments_skipped?: { id, reason }[], delegation?: { task_id, title } }` — since U3 the two attachment lists are disjoint: an attachment the adaptation withheld leaves `attachments_used` and appears in `attachments_skipped` with the daemon's reason, and since A1 so does one on a turn a tier answered without carrying it (a task command, `/steer`, the social fast path, a plugin-contributed skill)                                                                                                          |
...
| Transcript on load / scrollback                            | ✅ `GET /v1/chat/history?limit&offset&lane_key&session_id` — retargeted to a **session** by migration 039; omitting `session_id` reads the lane's active one                                                                                                                                                   | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `{ messages: ConversationMessage[], total: i64, lane_key: string, session_id: string \| null }` — `session_id` is what the sidebar highlights, and `null` on a lane that never held a turn. A row is the stored message **flattened** plus `artifacts` (GAP-23); there is **no `attachments` key** and never has been — the files a user turn carried in are the file parts of its `content_json` (`{"v":1,"parts":[…]}`, each `document` or `file_ref` part carrying `file_id`/`filename`/`mime_type`), and a part's `extracted_text` is the daemon's copy of the bytes for the model, not for display (P1) |
...
| Confirmation card `Confirmation required · shell_execute`  | ✅ SSE `confirmation_requested`/`confirmation_resolved` **and** WS `tool_confirmation_requested`/`tool_confirmation_resolved`                                                                                                                                                                                  | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | SSE `{ request_id, tool_name, tool_arguments }`; WS adds `{ agent_id, stream_id?, lane_key? }`. The resolved twin carries `{ request_id, outcome }` (SSE) / the same plus `{ agent_id, tool_name, stream_id?, lane_key?, task_id? }` (WS), `outcome` one of `approved`, `denied`, `timed_out`, `cancelled`                                                                                                                                                                                                                                                                                                   |
...
| first-run card (empty transcript)                          | ✅ `GET /v1/status` → `llm.effective_default_model`                                                                                                                                                                                                                                                            | — (the window's own status poll)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `null` = the daemon can route nothing → the card; `undefined` = not answered yet (or no `llm` block) → **nothing**, told apart with `=== null`; a string → nothing. Derived, never stored — D-G                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Attachments (pick / paste / drop) — **used since U5**      | ✅ `POST /v1/files/upload` (multipart, 100 MB `DefaultBodyLimit`); one file per request, a single part named `file`, `x-workspace-path` chooses the store (D2). `lib/api/files.ts#uploadFile`                                                                                                                  | multipart                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `{ id, filename, mime_type, size_bytes, status }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Attachment count cap                                       | ✅ enforced server-side; the composer mirrors the default (10) and refuses the 11th pick with the daemon's own sentence, so the cap is never a surprise on send                                                                                                                                                | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `400 TOO_MANY_ATTACHMENTS` when `> upload.max_files_per_message`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Inline artifact card (`MD connector-audit-findings.md v2`) | ✅ `artifact_written` → `WrittenArtifactCard`, linking into the Library                                                                                                                                                                                                                                        |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

**Two more optional fields on `POST /v1/chat`.** `model?: string` is the Model picker
row above. `unattended?: bool` (M6/S10) declares "this client cannot answer a tool
confirmation", so a tool needing one is refused at once instead of waited on; the CLI
sets it when stdin or stdout is not a terminal, and **this client never sends it** — it
can always draw the card.
...
| `Export`                                                              | ✅ client-side, no server route: `GET /v1/files/{id}/content` as a `Blob`, saved through an `<a download>` (`views/library/LibraryDetail.tsx`). No Tauri fs plugin — it is not a dependency; `dialog` is the only plugin                                |
...
| `Stop daemon…` / `Start daemon`                                         | ✅ `POST /v1/command {"command":"shutdown"}` — the route's first GUI caller — then the `await_daemon_stopped` Tauri command; the confirmation reads `GET /v1/status.busy` (T23) and renders the generic line when it is absent. `Start daemon` replaces `Reconnect` while stopped: clear the stop intent → `ensure_daemon_running` → reopen the socket → refetch everything (a restart changes the port, token and instance id)                                                                                                                                                                                                   |
| `Copy log path`                                                         | ✅ **GAP-14, closed** — `GET /v1/status.log_path` = `state/logs/daemon.log` when this daemon's launcher wrote it — the CLI or, since T30, the app's own sidecar — else `null` (a daemon started by hand; the button is inert with the reason). Both launchers rotate it at 16 MB, keeping three generations. `Show daemon log` reads it through the `read_daemon_log_tail` Tauri command, not a route                                                                                                                                                                                                                             |
...
| daemon workload (for a stop warning)                                    | ✅ `GET /v1/status.busy` (T23, adopted) — `{ running_tasks, pending_confirmations, connected_clients }`, counts only: one `COUNT(*)` over the non-terminal task rows and two in-memory reads. `null` on a daemon with no confirmation broker, absent from an older daemon — read it defensively and never as "idle"                                                                                                                                                                                                                                                                                                               |
...
| **The project** — its path, and the re-base offer                       | ✅ `GET /v1/workspaces?path=` → `{ path, store_present, recorded_root, moved, rows: { artifacts, sessions, tasks, memories }, active_tasks, queued_tasks }`; `PATCH /v1/workspaces` `{ old_path, new_path }` → `{ old_path, new_path, moved: {…the four counts}, store_moved }`; `404 WORKSPACE_NOT_FOUND`, `409 WORKSPACE_BUSY` (a run under the old root is in flight) \| `WORKSPACE_EXISTS` \| `WORKSPACE_MOVE_BLOCKED`, `500 WORKSPACE_MOVE_FAILED` (rows re-based, store not moved). Not in the design. The path is client state (`oa-project`, §2.6) sent as `x-workspace-path`                                             | `lib/api/workspaces.ts` (its type omits `queued_tasks`); `POST /v1/workspaces/purge` exists and the GUI never calls it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Models & keys** — provider rows                                       | ✅ `GET /v1/settings/llm` (full `LlmConfig`, each provider carrying `requires_key` — a `false` row reads `no key needed`, never `0 keys`, L1), `GET /v1/settings/llm/status` (`key_health()`), `GET /v1/settings/llm/providers/usage` → `ProviderUsageSummary[] { provider, total_cost_usd, total_tokens, total_requests, health, external_usage? }`                                                                                                                                                                                                                                                                              | ⚠️ `health` is hardcoded `"healthy"` in `get_provider_usage`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| model chips per provider                                                | ✅ `GET /v1/models` → rows carrying `supports_tools` (L2), and `POST /v1/models/refresh` behind the section's `Refresh models` button — the verb that makes a model pulled after boot reachable without a restart                                                                                                                                                                                                                                                                                                                                                                                                                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `Add key` / key CRUD                                                    | ✅ `PUT /v1/settings/llm` (upsert), `DELETE /v1/settings/llm/keys/{provider}/{key_id}`, `PUT .../keys/reorder`, `PUT .../keys/priority`, `POST .../validate`, `GET .../credentials`, `POST .../credentials/rescan`, `GET .../cli-backends`                                                                                                                                                                                                                                                                                                                                                                                        | ✅ wired by `views/settings/AddKeyForm.tsx` (upsert, validate — §5.4a) and the per-provider key rows in `ModelsSection.tsx` (`DELETE …/keys/{provider}/{key_id}` on a second click; `…/keys/priority` for `Make primary`, whose body is **`key_id`** — the one route here that really does take `key_id`). `PUT /v1/settings/llm` takes `key.id` — **`id`, not `key_id`**, which serde drops silently. `POST …/validate` answers `504 KEY_VALIDATION_TIMEOUT` for _every_ failure of the check, so the form runs it on its own button and never gates a save on it; the failure reason is `format_error`, and there is no `message` field. A key saved for a **disabled** provider is answered `200` and ignored, so the form refuses it before the request (D-F). Rows read `KeyInfo.source` as a plain string — the CLI stores its display labels there, not `KeySourceValue`s — and render an unknown one verbatim (`views/settings/key-rows.ts`). ⚠️ `…/keys/reorder`, `…/credentials`, `…/credentials/rescan` and `…/cli-backends` are still uncalled (the credentials pair is owner decision T24). |
| provider on/off toggle                                                  | ✅ `PUT /v1/settings/llm/providers/{provider}/enabled` `{ enabled }` → `200 { id, enabled, loaded, discovered_models, discovery_error, warning }` — `discovered_models` is the enable's own count of what the provider can serve (L2), and the toast says it; `404 PROVIDER_NOT_FOUND`, `409 PROVIDER_IS_DEFAULT` when it serves the default model, `409 DEFAULT_MODEL_UNRESOLVED` when the default model places nowhere. Writes `llm.toml` through the one atomic writer, then unloads or reloads the provider live                                                                                                              |
...
- `oa-pane-widths` → `{ workW: 396, workListW: 340, libListW: 326, chatSessionsW: 236 }`
  (drag ranges `workW` 300–600 reversed, `workListW` 260–480, `libListW` 260–480,
  `chatSessionsW` 200–360 — the chat view's conversation column, which the design
  does not have; an older three-key payload still loads, each key falling back on
  its own). `stores/pane-widths.ts`.
- `oa-project` → the one absolute project path sent as `x-workspace-path`
  (`stores/project.ts`); absent means no header.
- `oa-pins` → a **cache** of the server-side pin state (`PUT /v1/artifacts/{id}/pin`
  is the authority, and every server answer overwrites it).
- `view`, `secId`, `libKind`, `artTab`/`panelTab`, `dense` (compact density),
  `workOpen`, `panelArt` — in-memory only in the shipped client (`stores/ui.ts`
  persists the widths and the pin cache, nothing else), so they reset on a reload.
...
> **Closed in Phase 8 (item 7, T50).** `GET /v1/usage/summary?window=today` serves today's total, its per-provider breakdown and the two caps that actually bound spend. Per **N4** there is no daily budget and none is coming, so the total ships with no denominator and the design's progress bar stays undrawn _by decision_ — which is what the panel now says. N4 is about turns and workflows: three background jobs (memory extraction, conversation summaries, task-output extraction) do carry small daily ceilings of their own — `[orchestrator.costs]` `extract_max_daily_cost_usd` / `summary_max_daily_cost_usd` / `task_extract_max_daily_cost_usd` — which this route does not serve and the panel does not show.
...
>   retention: { log_max_session_bytes, log_max_total_bytes, log_retention_days },
>   sessions: { last_sweep: {…} | null, dropped_records },
>   routing: { resume_enabled },
>   llm: { default_model, default_model_routable, effective_default_model } | null
> }
> ```
>
> `retention` is the pair of byte caps the sweep is measured against (the panel
> prints `log_max_total_bytes` as the denominator of its `over_cap_after` note) plus
> `log_retention_days`, which is served but inert (R85).
>
> `llm` arrived with L3: the fallback ladder may answer on a model other than
> `[orchestrator] model` — every shipped agent template pins a Claude id — and
> Settings → Models reads this pair to name the substitution rather than show a
> picker that disagrees with every reply. `null` means this daemon has no
> router at all.
>
...
5. `confirmation_requested` — `{ request_id, tool_name, tool_arguments }`. May arrive
   ...
   **Every way a prompt stops being pending is announced (T1)**:
   `confirmation_resolved` on the SSE stream — `{ request_id, outcome }` — and
   `tool_confirmation_resolved` on the WS, with `{ request_id, agent_id, tool_name,
outcome, stream_id, lane_key, task_id }`. `outcome` is `approved` | `denied` |
   `timed_out` | `cancelled`, and only `approved` ran the tool: a timeout is a denial
   with a different reason. Before it existed a prompt that ran out its
   `confirmation_timeout_secs` produced nothing at all, so a client that settles a
   card only on an answer it posted itself kept the card — and its paused composer —
   until it was reloaded. Settle on **any** outcome, and do not settle on this frame
   alone: it is one of three signals, beside the pending list below and a terminal
   `task_status` for the run named on the frame.
...
6. **Terminal:** exactly one of
   - `done` — `{ content, model, tokens_in, tokens_out, duration_ms, attachments_used?, attachments_skipped?, delegation? }`.
     `content` is the **full** text (not the tail) — prefer it over the accumulated
     deltas as the source of truth. `attachments_used`, `attachments_skipped` and
     `delegation` are omitted entirely when absent
     (`skip_serializing_if = "Option::is_none"`), and the SSE payload strips the serde
     `"event"` tag (`done_event_data`). `attachments_skipped: [{ id, reason }]` (U3) is
     every attachment the model that answered never received — no vision, no native
     document part, or bytes that could not be read — and `attachments_used` is truthful
     against it: the two lists are disjoint, so "used" may be read as "reached the
     model". Since A1 that holds on **every** tier, not only the two that adapt parts: a
     turn answered by a task command, by `/steer`, by the withdrawn-skill tombstone, by
     the social fast path or by a plugin-contributed skill carries no attachments to its
     model (or to any model at all), and each one is listed here with a sentence saying
     which arm answered. Nothing persists a skip, so `GET /v1/chat/history` never
     replays it.
   - `error` — `{ message }`.
...
- The client's own union of the frames it handles — all 34 `ServerEvent` variants —
  is `apps/openalpaca-gui/src/lib/events.ts`, and the one place a frame becomes a
...
**Client lifecycle (`DaemonEventsClient`, `src/lib/events.ts`):**
...
6. Null all four handlers **before** `close()` when tearing down, or `onclose` will
   re-trigger `scheduleReconnect` (`teardownSocket` does this deliberately).
7. **A `daemon_shutting_down` frame ends the ladder** (§1.2 item 8). The daemon sends
   it on each socket just before closing it with code 1001. The client turns
   reconnecting off and, when this window did not ask for the stop, sets the stop
   intent to `"stopped_elsewhere"` — so the close that follows schedules nothing — and
   still delivers the frame to its listeners like any other. `resume()` restarts the
   ladder without bootstrapping, for a stop whose request never reached the daemon.
8. **One socket at a time, whatever the call order** (F4). `connect()` and
   `disconnect()` bump a generation counter; every `await` (the boot path's
   `bootstrap()`, the backoff ladder's `refresh()`) captures it before and compares it
   after, and a stale continuation opens nothing. `openSocket` tears the previous
   socket down first. Without it two `connect()` calls landing back to back —
   StrictMode's mount → cleanup → mount, or two clicks on Reconnect during one sidecar
   spawn — left the first socket open and unreachable, and every frame arrived twice.
...
| Run detail → Event log  | every `RUN_LOG_EVENT_TYPES` frame (`lib/api/run-events.ts`) naming the run invalidates its `GET /v1/events/history?task_id=` query. `tool_confirmation_resolved` joined the list with F3 — the card kept `awaiting approval` after a deny or a timeout — and its row reads the outcome from the row's **`result`** blob, not `detail`    |
| Chat confirmation card  | `tool_confirmation_requested` and `tool_confirmation_resolved` (dedupe with the SSE frames by `request_id`; the resolved frame carries the `outcome`, T1)                                                                                                                                                                                |
...
| Connection chip         | `heartbeat`, plus the socket's own open/close; `daemon_shutting_down` → `stopped elsewhere`                                                                                                                                                                                                                                              |
