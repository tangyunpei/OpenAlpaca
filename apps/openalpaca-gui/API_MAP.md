...
| `Start now` (queued run)                                                               | ✅ **GAP-06, closed** — `POST /v1/tasks/{id}/action`, answered before `apply_task_action` (it is a dispatch, not a transition)                                                                                                                                                               | `{ action: "start" }`                         | `200 { task_id, status }` — `task_id` is the id you sent (**D5**); `409 TASK_ALREADY_RUNNING`, `422 TASK_NOT_STARTABLE`, `503 DISPATCH_FAILED`, `404 NOT_FOUND` (owner-scoped, R40)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
>   Refusals: `409 TASK_ALREADY_RUNNING` (a compare-and-set on the run slot, not
>   a look — two simultaneous starts must not put two lead agents on one id),
>   `422 TASK_NOT_STARTABLE`, `503 DISPATCH_FAILED`, `404 NOT_FOUND`.
>
