...
| `Always allow` (allowlist)                                 | ⚠️ same route, **scope not accepted** — see **GAP-01**                                                                                                                |                                                                                                                                   |                                                                                                                                                  |
...
| `{{ spend }} today` footer                                 | ⚠️ `GET /v1/llm/usage/daily?date=YYYY-MM-DD` — client must sum `total_cost_usd`. `GET /v1/orchestrator/config.daily_cost_usd` is **hardcoded `0.0`** — see **GAP-08** |                                                                                                                                   | `LlmUsageDaily[] { date, agent_id, model, total_requests, total_input_tokens, total_output_tokens, total_cost_usd }`                             |
...
the `POST /v1/chat` response — there is **no endpoint that returns the default lane key
up front** (see GAP-16).
...
| -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Run list, `{{ activeCount }} active · {{ doneCount }} done`                            | ✅ `GET /v1/tasks?status=active\|<status>&created_by=&limit=`                                                                                                      | —                                             | **array** of serialized `Task` + injected `assigned_agents[]` + injected `outcome`                                                                                                                                                                                                |
| `Task` fields                                                                          | —                                                                                                                                                                  | —                                             | `{ id, title, description?, status, priority, progress_current?, progress_total?, result_summary?, created_by, source_lane, created_at, updated_at, completed_at?, state_version, outcome_kind?, artifact_count }` (`state_json`/`outcome_json` are `#[serde(skip_serializing)]`) |
...
| Live status ticks                                                                      | ✅ WS `task_status`                                                                                                                                                | —                                             | `{ type:"task_status", task_id, title, status, progress_current, progress_total, result_summary, outcome_kind?, artifact_count?, outcome_summary?, ts, instance_id }` — ⚠️ `title` is `""` on updates, see **GAP-07**                                                             |
| `{{ r.meta }}` = `11m 04s · 5/8 steps · $0.41`                                         | ⚠️ duration derivable (`created_at`→`updated_at`/`completed_at`), steps = `progress_current`/`progress_total`, **cost is not served** — see **GAP-08**             |                                               |                                                                                                                                                                                                                                                                                   |
...
| `Today: {{ spend }} of $5.00 cap · 15 runs · 41k tokens`                      | ⚠️/❌ spend+tokens from `GET /v1/llm/usage/daily?date=` (sum `total_cost_usd`, `total_input_tokens+total_output_tokens`); run count from `GET /v1/tasks?limit=…` filtered client-side; **the cap is not served** — **GAP-08**                                                                                    |
...
| `41k tok today` per provider                                                  | ⚠️ `ProviderUsageSummary.total_tokens` is **lifetime**, not today; per-day-per-provider requires client math over `/v1/llm/usage?limit=` (`LlmCallLog { timestamp, agent_id, task_id, provider, model, key_id, input_tokens, output_tokens, cost_usd, status, latency_ms, error_message }`) — **GAP-08**         |
...
### GAP-01 — "Always allow" cannot be expressed _(trivial fix, high value)_
...
### GAP-07 — `task_status` drops `title` and `agent_status` drops `name`
...
**Why nothing fits — three separate holes:**

1. **Per-task cost.** `LlmUsageRepository::get_task_usage(task_id, limit)` **exists**
   (`repository/llm_usage/mod.rs:95`) and `llm_call_log` has a `task_id` column — but
   `LlmUsageQuery` in `routes/settings_types.rs` accepts only
   `{ agent_id, key_id, limit }`, and `get_llm_usage` never calls `get_task_usage`.
   _The data is one query param away from being reachable._
2. **Today's spend.** `GET /v1/orchestrator/config` returns `daily_cost_usd` — but
   `routes/settings.rs` hardcodes it:
   `let daily_cost_usd = 0.0; // Cost tracker requires async access via LlmRouter`.
   The handler is already `async` and `service.router().cost_tracker` is right there
   (`get_provider_usage` awaits it three functions later).
3. **The cap.** `execution.max_cost` (default `1.00` per agent, `5.0` per task —
   `daemon_config/execution.rs`) and `orchestrator.costs.*_max_daily_cost_usd` exist in
   config, but `router.rs` exposes **no** `GET /v1/daemon/config`; the only daemon-config
   route is `/v1/daemon/config/providers`, which returns web-search settings only.

**Proposal:**

```
# 1. one new query param on an existing route
GET /v1/llm/usage?task_id=b41c8e02&limit=200   → LlmCallLog[]

# 2. fix daily_cost_usd (no API change) — await cost_tracker in get_orchestrator_config

# 3. a purpose-built rollup, so the UI needs one call instead of three
GET /v1/usage/summary?window=today
→ 200 {
  "window": "today", "date": "2026-08-31",
  "cost_usd": 0.0184, "cost_cap_usd": 5.0,
  "tokens_in": 34120, "tokens_out": 7005,
  "requests": 88, "runs_started": 15,
  "by_provider": [ { "provider": "anthropic", "cost_usd": 0.0184, "tokens": 41125, "requests": 71 } ]
}
```

Adding `cost_usd` (and `tokens_in`/`tokens_out`) directly onto the serialized `Task`
would be even better for the Work list, which needs it per row.
...
agent_instance_id, template_id, ts }` — a point-in-time ping with no span and (per
  GAP-07) an empty `name`.
...
### GAP-16 — The default lane key is not discoverable
...
| #    | Gap                                                    | Blocks                      | Fix size                         |
| ---- | ------------------------------------------------------ | --------------------------- | -------------------------------- |
| 01   | `approval_scope` dropped by the confirm route          | "Always allow" button       | **XS** (field passthrough)       |
| 07   | `task_status.title` / `agent_status.name` sent as `""` | Work list titles            | **XS** (bridge fix)              |
| 08.2 | `daily_cost_usd` hardcoded `0.0`                       | spend indicators            | **XS** (await cost_tracker)      |
| 08.1 | `get_task_usage` not reachable                         | per-run cost                | **XS** (query param)             |
| 11   | content route is header-auth only                      | image/html preview          | **S** (query token)              |
| 16   | default lane key / user id undiscoverable              | history on mount            | **S**                            |
...
**Recommended order:** the XS/S column first (01, 07, 08, 11, 14, 16, 06, 02, 15, 21, 22) unblocks roughly two-thirds of the design for a handful of one-file changes. Then
...
- **Lag handling:** on `RecvError::Lagged(n)` the handler logs and `continue`s — the
  ...
  live hints_, never as the source of truth; always have a REST refetch path
  (this is why GAP-07 matters — a missed/emptied field forces an N+1 refetch).
