<script lang="ts">
  ...
  /** Refresh data — can be called externally via bind:this */
...
<div class="oa-panel">
  <div class="oa-panel-header">
    <h2>Agent Templates ({agents.length})</h2>
  </div>
  <div class="p-2.5 max-h-[40vh] overflow-y-auto">
    {#each agents as agent (agent.id)}
      <AgentCard {agent} onclick={() => (selectedAgentId = agent.id)} />
    {:else}
      <div class="text-muted-foreground text-center py-10 px-6">
