...
**Status:** design of record, **rev 16 — implemented: C1–C8 landed** (`e92d76e..eff30cb` on `feat/ui-rework`; C8 is the `global_tool_deny` purge + the documentation sweep). Rev 16 changes no mechanism: it records the corrections and residue observed while building C1–C8 (see the revision log) and leaves §13 Q5–Q14 pending and unapplied. Rev 15 was the verified design implemented from: **rev 15 — verified.** Directive fidelity (S1–S4), source claims and plan/KB consistency passed on earlier revisions; the consistency lens confirmed every remaining check on rev 13 and its two follow-ups found only step-order wording, the last a single word in E-PRE's closing sentence (rev 14 → 15), fixed and grep-verified — no live sentence pairs E-PRE with "then E0". Implement from this revision. (rev 14: one residual — five sites still read "E-PRE, then E0"; all now say E0 CAS → E-PRE → E1–E5. Every other check confirmed: §3.3.1 derived cell by cell, the four crash races under the table, E-FAIL, C1, §3.7, one predicate, §12, §13, ~60 anchors; round-11 — the §3.3.1 table is regenerated row by row from §4.1 and the prose, E-PRE runs after E0's CAS so a redundant `enable` can never tear down a live handle; round-10 — handle ownership is now a derived state × verb table, §3.3.1, and every exit from a pre-reaper `Failed{Crashed}` tears the residue down; round-9 — a Retry that beats the crash reaper now tears down load N before building load N+1 (E-PRE); round-8 — a bring-up failure after a successful connect/spawn now tears down the just-built handle before `Failed` is committed; round-6: three internal-consistency blockers on rev 7 repaired — §6.2 #8 deny is write-first like every other statement of W-deny; §3.7 step 4 diffs against the live subset so a server-re-added tool is reachable; one skill-refusal predicate, `CapabilityOracle::is_satisfiable`, at every consumer), accepted mechanism. **Residue for the next pass:** none blocking in this file; the owner-gated items are enumerated in §13 (Q5–Q14) and are **not applied anywhere** in this document — §13's arrows are recommendations, and silence enacts none of them. **Cross-file follow-ups still owed (not this file):** the cost-lockout fix now has its slot (api-fix-plan Phase 0 **A5** with its *Verify* clause) and the roadmap's Known-broken row reads confirmed + scheduled (both 2026-09-02); plan Phase 0 **A1** now orders deny write-first, matching §6.2 #8; the GUI `GapId` union (`unavailable.ts:37`/`:233` still read `GAP-19`, C7 work); and the small plan/KB wording items listed in the rev-7 log entry. The plan-side N5/Phase 8/ADR-030 items rev 6 listed as owed **landed** with plan rev 3 and ADR-030 (`94a8f42`). Supersedes ADR-029 (flat per-tool disable list) entirely.
**Rev 8 in one line:** three rev-7 self-contradictions closed, nothing else moved — §6.2 #8 now reads as
§3.2 W-deny / §4.1 / C3 / ADR-030 (a) already did (write the denial, then T0–T4, `Unapproved{Denied}` at T5);
§3.7 step 4 diffs the fetched list against `retained \ server_withdrawn` so a name the server drops and later
re-adds lands in `added` and is re-registered, its tombstone restored and its flag cleared (seventh C2 test);
and §6.2 #10/#11, §10 case 3 and §7.2 all state the one predicate the router, catalog, cron and `/slash`
already used — refuse when **any** required capability classifies `withheld`; "partial" is
`partially_withheld` only (a surviving provider of the same capability) and runs with the prefix (C5 test).
Owner-gated items stay in §13, unenacted.
**Rev 7 in one line:** the `tools/list_changed` refresh (§3.7) now **fetches outside the per-extension mutex** with a
one-shot, non-reconnecting `list_tools_once` and re-checks state + generation before applying the diff, so a
dying server can no longer hold a toggle behind a reconnect ladder; `record_tools` never touches
`server_withdrawn` (E5's `restore` clears it); a *live incumbent* (§10 case 13) is `Enabled` **and** not
server-withdrawn; the legacy `tools.allow` branch now reaches the availability machinery — router,
`<available_skills>`, cron skip, dependent scan — not only the invocation sites; `reload`'s T0–E5 window has
its own transient wording (`pending_cause` on the record); an unparseable `mcp.toml` under the watcher keeps
the last-good desired set (fail-closed is a boot rule); the `Allowlist` type is homed A0-else-C5 and out of C1;
a plugin parked at scan as `Failed{ConfigInvalid}` carries the bit as read; `Moment::ScheduledSkip` is exempt
from dedup; the `config_fingerprint` preimage is defined; §13's arrows read *Recommendation*; every plan/KB
anchor re-verified against api-fix-plan rev 3 and ADR-030. Everything the lessons marked surface-to-owner
stays in §13, unenacted.
**Date:** 2026-09-02 · **Branch:** `feat/ui-rework`
**Answers:** `tasks/api-fix-plan.md` §0 **N5** (line 47 in plan rev 3) — the settled model S1–S4, mechanism now fixed.
...
- **The writer** is `toml_edit`-based (surgical `doc["servers"][name]["enabled"] = …`), so every comment in the
  ...
  aborts the write with the file untouched. `McpConfig` still needs no `Serialize` impl for the **writer**; the
  §3.3 E2 fingerprint derives `Serialize` on `McpServerConfig`/`HttpAuthConfig` for its own preimage and nothing
  else reads it. The lock + temp +
...
- **Backups are the price of fail-closed parsing.** §5.1 refuses to load an unparseable store and never
  ...
  `state/backups/` (api-fix-plan's machine-state area — today `openalpaca_storage::paths::app_dir()/state/backups/`, so C1 does not wait on the re-home; never beside the human's file); a boot-time parse
...
- **Identity: the directory name is the extension id (rev 6 decision — X-3).** Today `.permissions.toml`
  ...
  parked as `Failed{ConfigInvalid, "manifest name does not match directory"}` with **no spawn** — and the
  **scan order is pinned**: name check → consent (`is_approved`) → bit, so the name check runs before the
  §6.2 #7 gate reads anything; the parked record carries the bit **as read** from the directory-keyed entry
  (`false` if the owner had pre-set it), which is the one `Failed` cell exempt from §4's `Failed ⇒ bit == true`
  rule because W and E0 were never reached;
  `ExtensionId::plugin(<dir name>)` and the `.permissions.toml` key are the directory name; and the map insert
  at `manager.rs:263` (the `plugins.insert(` line inside the `:261-280` block) **refuses to replace** an entry
  whose `process` or `capability_provider_handle` is `Some` — a *held handle*, which after E-PRE/T4 is never the case
  on a legitimate load (the state-list condition `Disabled`/`Failed`/`Unapproved` is therefore redundant and dropped);
  and E-PRE (§3.3) runs **before** this insert, as the first statement after E0's CAS on the load path for an id that already has an
  entry — otherwise the insert would overwrite a pre-reaper `Failed{Crashed}` entry's live `process` and provider handle
  before E-PRE could read them. Outcome of a refused insert: `error!` + `Err(PluginError::HandleHeld)` — asserted unreachable after E-PRE/T4, which the C3 tests cover
...
reconcile, with the crash reaper (§3.6) or with the **apply** half of a tool-list refresh (§3.7 — its fetch runs
*outside* the mutex precisely so a slow or dying server cannot hold a toggle behind a reconnect ladder; the
holders of this mutex and their bounds are listed once, in §3.2 T4). **Step W executes after the mutex
...
1. `ledger.begin(ext, Enabling, None)` increments the record's `generation` and returns it; the supervisor threads
...
The four entrants that do not write the bit at W — watcher, declaration-gone, deny (a route, but W-deny writes consent, never the bit) and reload (no W) — never write it here; the reaper never takes T0 at all (`mark_failed` already set the state) and enters at T1: on the **watcher** path the write *is* the trigger
...
**T0 — GATE (authoritative).** `ledger.begin(ext, Disabling, cause)`, a CAS on the entry's state that also
records the verb's `WithdrawalCause` (§7.3) as the record's `pending_cause`, cleared at T5/E5 — `describe`
(§7.1) reads it so a `reload`'s T0–E5 window is worded *reloading*, never *being turned off*. From this instant
...
1. record its `provides_capabilities` (read from the live registry entry; **an absent entry contributes nothing**, which is what makes the second, idempotent pass — reaper after a route disable, disable after E-FAIL — a no-op) into the tombstone index (`capability → Set<ExtensionId>` — a **set**,
...
2. `skill_catalog.remove(skill_id)` per contributed skill (`manager.rs:546`; the `for` is `:545`);
3. `agent_registry.remove_template(agent_id)` per contributed template (`manager.rs:554`; the `for` is `:553`);
...
**T4 — TEARDOWN.**
*T4 asks the map, not the state:* it tears down whatever handle the supervisor's map holds for this extension — the live one from `Enabled`, or the residue from a pre-reaper `Failed{Crashed}` — and **removes it from the map** (MCP: drop the `Arc<McpClient>`; plugin: `process = None`, and `capability_provider_handle = None` after T2); from any state that holds none (`Disabled`, or a `Failed` reached through E2-fail, E-FAIL or the reaper) it is a no-op. It never assumes the state implies the handle.
...
- **MCP:** `(*arc_client).clone().disconnect().await`.
  ...
  `Arc<ClientInner>` (`crates/openalpaca_mcp/src/client.rs:47-51` — doc comment `:47`, `#[derive(Clone)]` `:48`, struct `:49-51`: *"Cheaply cloneable
  ...
  **`mutex_wait + drain_timeout_secs + min(max_tool_runtime_secs, request_timeout) + 1s`**, where `mutex_wait`
  is the time to acquire the per-extension mutex — zero unless another holder is in flight, and the holders
  are exactly four: an `Enabling` or `reload` bring-up (§4.1 queues the `disable` behind it), bounded by that
  server's `connect_timeout_secs` (default 30 s, `tools/mcp/config.rs:40`); the reaper's T4 (§3.6), bounded by
  `request_timeout + 1 s` by the detach rule above; and a §3.7 refresh's **apply** step, which is in-memory and
  microseconds because the fetch runs outside the mutex — so `mutex_wait ≤ max(connect_timeout_secs,
  request_timeout + 1 s)` — and a `200` means *no new call can reach the extension and its
  ...
  *How "the child is gone" is observed for MCP.* The daemon holds **no** handle on an MCP child. The
  ...
  `transport/stdio.rs:79`; `TransportInner` is `pub(crate)` and its doc comment at `transport/mod.rs:39-40` says
  *"Not exposed in public API"* — rev 6 cited `stdio.rs:120`, which is a test-module comment about `Debug`) and `McpClient`
...
before either supervisor lands. Its `event_bridge.rs:30` arm also lands in **C1**, as a `tracing::debug!` no-op:
the daemon's `SystemEvent` match there is exhaustive by design (no catch-all arm), so a variant without its arm
fails `cargo build --workspace` and C1 would not be the green, additive commit §12 promises. The `ServerEvent`
peer, the `eb.extension_state_changed(..)` forwarding and the persistence arm land in **C2**, the first commit
with a transition to announce (§12). The two *capability* events are C4's. The
...
**W-deny / T5-deny (plugins; `deny` from `Enabled`, and from a pre-reaper `Failed{Crashed}`).** Same write-first shape: at **W** the route writes
...
starts from a state with nothing loaded and is W plus the memory commit alone — **except a pre-reaper
`Failed{Crashed}`, which still holds load N's residue: there `deny` is W-deny, then T1 → T2 → T4 with cause `Crash`, then
T5-deny** (§3.3.1), so the crash's T1-step-3 scan fires and nothing is carried into `Unapproved{Denied}`. **T5-deny and T5-gone from a pre-reaper `Failed{Crashed}` store the target state unconditionally under the mutex; they are not CASes from `Disabling`** (the residue exits never enter `Disabling` — they have no T0).
...
**E0 — CAS.** `ledger.begin(ext, Enabling, None)` — the CAS, **and it increments the record's `generation` and
...
**`config_fingerprint`** (§3.4, X-11): `blake3` over a **canonical rendering of the parsed block, not of its
bytes** — `McpServerConfig` and `HttpAuthConfig` (`tools/mcp/config.rs:46-77`, `:110-117`) derive `Serialize`
(every field type already serialises: `HashMap`, `PathBuf`, `Option<u64>`, and `url::Url` under the `serde`
feature the workspace enables, `Cargo.toml:92`), the masked value is converted to a `toml::Value`, every
table's keys are **sorted recursively by the fingerprint function itself** (do not rely on `toml::Table`'s
ordering — it flips with the `preserve_order` feature), and the result is rendered with `toml::to_string`.
A comment, blank line or key-order edit in the file therefore changes nothing; a value edit does. The mask:
the rendering of the block with every
...
secret; no salt or keyed hash is needed because no secret enters the hash. **`enabled` is not part of the
preimage (rev 16, R15):** the disposition bit is the diff key's own half — presence + bit + fingerprint drive
different verbs — so the fingerprint function drops it before hashing
(`tools/mcp/fingerprint.rs::without_disposition`); hashing it would make an untouched declaration read as
changed for as long as the bit and the ledger disagree. The fingerprint is what edge case
...
**E3 — DISCOVER.** `list_tools_once()` (§3.7) — a client that has just handshaken has no reconnect to make, so any error here is E-FAIL and bring-up stays bounded by `connect_timeout_secs` + one `request_timeout`, not up to four reconnect cycles under the mutex (the plain `list_tools` loops through `reconnect()`, `client.rs:226-257`); plugins additionally `skill/info`, `agent/info`.
...
**E-FAIL — TEARDOWN ON ANY FAILURE AFTER CONNECT/SPAWN.** E2's "nothing to unwind" holds only when `connect`/`spawn`
itself failed. Every exit from E2–E4 that occurs *after* `connect`/`spawn` returned — E3's `list_tools` failing after a
good handshake is ordinary, and so is a `skill/info` error — runs **T4 + T4b on the just-built handle before
`Failed{..}` is committed**: for MCP, `(*arc).clone().disconnect()` (which `take()`s `changes_tx`, closing the §3.7
reader's receiver so that task exits) then the `closed` seal; for plugins, `shutdown`/`kill` + the 2 s `wait`. Without
this, a `Failed` row would sit over a live, unsealed client and a live child that nothing ever drops (`McpClient` has
no `Drop`; the reader task holds the `Arc`), and a later `disable` from `Failed` — legal in §4.1 — would run T4 on
nothing while the child outlived the switch: an S2 hole. **At most one handle per extension.** A `Failed{Crashed}` record *owns its handle* until whichever takes the
mutex first: the reaper's T4, or the next handle-building verb's T1–T4 (E-PRE, below). `Disabled` records, and
`Failed` records that got there through E-FAIL or the reaper, own none. **Every transition out of a pre-reaper
`Failed{Crashed}` other than the reaper's own pass tears the residue down first** — `enable`/`reload`
via E-PRE (`approve` never loads from `Failed` — it re-records consent; it reaches E-PRE only from `Unapproved`, where nothing is held), `disable` via T4 (which asks the map, not the state), `deny` via W-deny → T1 → T2 → T4 → T5-deny, declaration-gone
via T1 → T2 → T4 (no write) then drop; all idempotent, a no-op once the reaper has been. §3.3.1 is the table. Ledger signature, stated
once: `begin(ext, target, cause: Option<WithdrawalCause>)` — `None` for `Enabling`. **C2 test:** handshake succeeds,
`tools/list` returns a non-retriable error → row `failed`, no live pid, `changes_rx` closed and the reader task exited;
`disable` from that row returns `200` with nothing to tear down and the bit written.

**E-PRE — TEAR DOWN BEFORE YOU BUILD.** E-PRE is not a property of named verbs but **the first step after E0's CAS on the shared
load path that every E0 entrant calls** — boot `reconcile_all`, the watcher's §3.4 trigger 2 (bit flip or fingerprint
change on a `Failed{Crashed}` record), `enable`, `reload`, `approve` with the bit set, and the config-route enable (§8) —
so no future entrant can skip it. It runs **T1 → T2 → T4 (the reaper's shape — no T3 drain, because the gate has refused
since `mark_failed`) with `WithdrawalCause::Crash` and the record's `detail`, on any handle the supervisor's map still
holds for the extension, under the same mutex hold, **after E0's CAS and before E1** — a CAS failure — `enable` on `Enabled`, including one queued behind an `Enabling`/`Disabling` hold that lands there — returns `200` with nothing touched and never reaches E-PRE (a queued `reload` landing on `Enabled` runs T0(Reload)–T4 first and then a load path whose E-PRE finds nothing; a queued `disable`/`deny` tears down) — and before any write to the supervisor's map** (plugins:
before the step-1 insert at `manager.rs:263`, §2.2; MCP: it takes and tears down the existing `Arc<McpClient>` before
`connect` builds the new one). `reload` from `Enabled` composes as T0(cause `Reload`)–T4 (§3.4.1), which empties the map, then this load path, whose E-PRE finds nothing; E-PRE therefore never runs against a live load: for MCP `disconnect` + seal (which
takes `changes_tx` and ends the old reader task); for plugins T2's provider/skill/template removal then
`shutdown`/`kill` + the 2 s `wait`. Only then E1–E5 as written (E0's CAS has already run; E-PRE sits between it and E1). Without it, a Retry that wins the mutex before the
reaper builds load N+1 on top of load N's live residue — old tools still registered under generation N and
advertised by `extension_tool_defs` (state-filtered only), a leaked reader task, a second `PluginCapabilityProvider`
— and the reaper then finds "superseded" and does nothing. With it, the reaper's superseded branch is a pure no-op:
it *finds* nothing to do rather than being trusted to do nothing. **C2/C3 test extension** (the *Reaper superseded*
tests): after `mark_failed` → `enable` (load N+1) → reaper released, assert no registry entry carries the old
generation, `ledger.audit()` is empty, the old reader task exited (C2), and exactly one capability provider is
registered (C3).

### 3.3.1 Handle ownership matrix (derived row by row from §4.1 and from T0–T5, E0–E5, E-FAIL, E-PRE, §3.6, §5.1)

**"Holds"** means the supervisor holds a *live handle*: for MCP the map value `Arc<McpClient>` (with its §3.7 reader task); for plugins `PluginState.process` or `capability_provider_handle` is `Some` — the `PluginState` entry itself persists across `Disabled`/`Failed`/`Unapproved` (§2.2), so "holds none" never means "no entry". From E2 onward a handle exists and is owned by the load path until it is written to the map (plugins: step 8, `manager.rs:492`; E-FAIL tears down that local handle). Every cell names the rule it comes from; **§4.1 is the authority on end states, this table on handles** — if they disagree, one of them is wrong and the revision log must say which.

| State (bit) | Holds? | `enable` | `reload` | `disable` | `approve` (plugin) | `deny` (plugin) | crash → `mark_failed` (§3.6) | reaper (gen matches) | declaration gone — **mcp** | declaration gone — **plugin** | re-declared (plugin) |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `Disabled` (false) | **none** | E0 CAS → `Enabling` → E-PRE finds none → E1–E5 → `Enabled` (one) | `409 not_loaded` (§3.4.1) | CAS fail → `200` current (§4.1) | consent recorded; bit stays false, nothing starts (§4.1) | W-deny → commit `Unapproved{Denied}`; bit stays false (§3.2 "W plus the memory commit") | n/a | n/a | record dropped, nothing held (§4.1) | → `Orphaned`, entry kept (§5.1) | — |
| `Unapproved{*}` (plugin-only; bit either) | **none** (never loaded) | stays `Unapproved`; bit := true; `200` w/ reason — **never a load** (§4.1) | `409 not_loaded` | stays `Unapproved`; bit := false; no teardown (§4, §4.1, §8) | → `Enabling` if bit=true (E0 → E-PRE finds none → E1–E5), else → `Disabled` (§4.1) | → `Unapproved{Denied}` (W-deny + commit) | n/a | n/a | — (plugin-only state) | → `Orphaned`, entry kept | — |
| `Enabling` (transient, mutex held) | **one from E2 onward** (owned by the load path until the map write) | queued behind the mutex; sees the outcome state (§4.1) | queued | queued | queued | queued | n/a | n/a | n/a | n/a | — |
| ↳ E2 `connect`/`spawn` fails | none (never built) | — | — | — | — | — | — | — | → `Failed{..}` (E2 clause: nothing to unwind) | idem | — |
| ↳ E3/E4 fail after connect | one → **E-FAIL tears it down** → none | — | — | — | — | — | — | — | → `Failed{..}` | idem | — |
| ↳ E4b partial registration | one → unwind published + **E-FAIL** → none | — | — | — | — | — | — | — | → `Failed{..}` | idem | — |
| `Enabled` (true) | **one** | E0 CAS **fails → `200`, E-PRE never reached, handle untouched** (§3.3 E0) | **T0(cause `Reload`)–T4** (§3.4.1: gate flip, drain, *reloading* row; cron notice suppressed if it ends `Enabled`) → none → E0 → E-PRE finds none → E1–E5 → one | T0–T4 → T5 `Disabled` → none | consent re-recorded; no load (§4.1) | W-deny → T0–T4 → T5-deny `Unapproved{Denied}` → none (§3.2) | bare CAS → `Failed{Crashed}`, **still one (residue)** | n/a (state ≠ `Failed`) | T0–T4 (no write) → record dropped (§4.1) | T0–T4 → `Orphaned`, entry kept | — |
| `Failed{Crashed}` **pre-reaper** (true) | **one (residue)** | E0 CAS → `Enabling` → **E-PRE: T1→T2→T4 (cause `Crash`) on the residue** → E1–E5 → `Enabled` (one); reaper then finds gen/state changed → superseded no-op | same as `enable` (§3.4.1: E0 CAS → E-PRE on the residue → E1–E5) | T0–T4 — **T4 asks the map**, tears the residue down → `Disabled` → none | consent re-recorded; no load | W-deny → **T1→T2→T4 (cause `Crash`)** → T5-deny `Unapproved{Denied}` (plain store, §3.2) → none | n/a (already Failed) | **T1→T2→T4** → none; stays `Failed{Crashed}` (never T5) | **T1→T2→T4 (no write)** → record dropped (plain store; reaper then finds no record → superseded) | **T1→T2→T4** → `Orphaned`, entry kept | — |
| `Failed{*}` **post-reaper / via E-FAIL / via E2-fail** (true) | **none** | E0 → E-PRE finds none → E1–E5 | same | T0–T4 find none → `Disabled` (bit written) | consent re-recorded; no load | W-deny → commit `Unapproved{Denied}` | n/a | n/a (nothing queued, or superseded) | record dropped | → `Orphaned`, entry kept | — |
| `Disabling` (transient) | one until T4, then none | queued (§4.1) | queued | queued | queued | queued | n/a | n/a | n/a | n/a | — |
| `Orphaned` (plugin; entry kept, disposition + consent preserved, §5.1) | **none** | — | `409 not_loaded` | — | — | — | n/a | n/a | — | (already gone) | **boot gate on `(approved, enabled)`** (§6.2 #7): `Unapproved{…}` / `Disabled` / `Enabling` → E0 → E-PRE finds none → E1–E5 (then one) |

**Invariants the table proves, and the rule each rests on:**
- *At most one handle per extension* — every "builds" cell is preceded by an E0 CAS that only succeeds from a state that is a load (§4.1), then E-PRE on anything held; `reload` from `Enabled` empties the map at T0–T4 first (§3.4.1); E-FAIL tears down a bring-up that failed after connect.
- *A pre-reaper `Failed{Crashed}` owns its handle until whichever takes the mutex first: the reaper's T4, E-PRE (via `enable`/`reload`), `disable`'s T4, `deny`'s T1→T2→T4, or declaration-gone's T1→T2→T4* — the only state with a residue; every exit tears it down under the mutex (§3.3 rule beside the invariant).
- *`disable` asks the map, not the state* — T4 tears down whatever is held, so it is correct from `Enabled` and from pre-reaper `Failed{Crashed}`, and a no-op from every "none" cell (§3.2 T4).
- *The reaper never writes state and never tears down a newer load* — it acts only on `Failed{Crashed}` at its own generation; an absent record (T5-gone) is superseded too (§3.6).
- *E-PRE never sees a live load* — it runs only after E0's CAS has established that the (state, verb) cell is a load, so from `Enabled`, `Enabling` or `Disabling` it is never reached (§3.3 E0/E-PRE).
- *No path leaves a live child under a "none" cell* — E-FAIL covers bring-up failures after connect; the reaper or any residue exit covers crashes; T4 covers every disable/deny/gone (S2).

**Where the S2 holes fixed in rev 10–12 sit in this table:** rev 10 = the "E3/E4 fail after connect" row (was "one, never torn down"); rev 11 = the `Failed{Crashed}` pre-reaper × `enable` cell (was "builds a second handle over the residue"); rev 12 = that row's `deny` and declaration-gone cells (were "W + commit" / "record dropped" — residue carried out), plus the plugin insert guard (§2.2) that let step 1 overwrite a held handle before E-PRE.


**E4b — PARTIAL-LOAD UNWIND.** `register` can still return `Err` on name validation (`registry/mod.rs:231-242`)
mid-loop, and discovery RPCs can fail after tools registered. If any step after a successful `connect`/`spawn`
fails (E-FAIL above governs the handle; this step governs what was *published*): remove every name registered in this attempt, remove any provider/skill/template already
...
`restore(ext)` also clears the extension's `server_withdrawn` set — a fresh load starts with no server-withdrawn
names — and **`record_tools` never touches that set**: §3.7 step 7 writes the union `live ∪ server_withdrawn`
and relies on the flags surviving the write (rev 6 had `record_tools` clearing them, which would have erased
every flag the refresh had just set and dropped both gate arms back to *"not found"*). For MCP, the
`tools/list_changed` receiver is taken at **E2**, with the client (once, through `McpClient::changes()`), and
the per-server reader task is spawned there too; its first message is **applied** only after E5 (the fetch itself runs outside the mutex and may overlap E3/E4 or E-FAIL, holding the service lock for up to `request_timeout`; T4's timeout + detach rule covers E-FAIL's `disconnect`), because
§3.7 step 3 takes the per-extension mutex the enable still holds and then re-checks for `Enabled` — a
notification during a failed bring-up is dropped by that re-check. The receiver is dropped at T4 with the
client, so a notification can never outlive the load that produced it — and because the sender lives on
`ClientInner`, not on the per-handshake handler, it survives the client's own in-session `reconnect` (§3.6
item 1), which is the same incarnation.
...
from `Enabled` (the ordinary case) and from `Failed{*}` (where, like `enable`, it is E0's CAS → `Enabling` → E-PRE T1→T2→T4 on any held handle → E1–E5; one verb for the
...
as `Stale` (§3.0 Fact 3). T0 records `WithdrawalCause::Reload` as the record's `pending_cause` (§3.2 T0), so a
call landing in the T0–E5 window is refused with the §7.1 *"is being reloaded right now; retry on your next
round"* row. **`Reload` survives the `Disabling → Enabling` CAS (rev 16, R16, as built in C1):** `begin(ext,
Enabling, None)` carries the cause forward **only** when the record it takes is `Disabling` **and** its
`pending_cause` is `Reload` — nothing stale can leak in, because `commit` clears `pending_cause` at the end of
every transition — so the E-half of a reload keeps saying *"reloaded"* rather than reverting to the generic
enabling wording — not *"being turned off"*, which would be false for a verb that ends `Enabled`. T1 step 3's
dependent scan runs with the same cause and the wording *"reloading"*; the **cron notice is suppressed** when the reload ends `Enabled` and fires normally if it ends
`Failed{*}` (the dependency really did become unsatisfiable). **Mechanism (rev 16 — option (a) of the residue, as built in C4):** for cause `Reload` the
supervisor **defers the publish** to E5/E-FAIL and emits the event with the reload's **outcome** state, emptying
`affected_cron_skills` when it ended `Enabled` (`managers/mcp.rs::publish_reload_scan`,
`plugins/manager.rs`); the dispatcher keeps its one rule — post whenever `affected_cron_skills` is non-empty —
with no cause special case. E2 re-resolves `bearer_env`/`api_key_env`, so a
...
3. **The read-side sweep.** `PluginManager::reconcile` and `list()` (i.e. every `GET /v1/extensions`) — and,
   ...
   that has exited (with the record's own current generation — this is the live process, by construction). `PluginState.process` stays `Some` after `mark_failed` until the reaper's T4 or the next verb's E-PRE — the same held-handle window as MCP.
...
clicks Retry; `enable` takes B's mutex first, **E0 CASes `Failed{*} → Enabling` and bumps the generation (one `begin` call, §4.1), runs E-PRE
(T1→T2→T4, cause `Crash`) on load N's still-held handle**, publishes load N+1 and commits `Enabled`. If the reaper then dequeued B and ran T1→T2→T4
...
the one in the message**; otherwise — including when the record is **absent**, dropped by T5-gone, whose per-extension mutex entry goes with it — it logs `debug!(ext, generation, "crash reap superseded")` and drops the
...
(`rmcp-0.16.0/src/handler/client.rs:59`), whose default body is `std::future::ready(())` (`:222`; its `std::future::ready(())` body is `:226`).
...
**Client (C2, `openalpaca_mcp`).** `serve_with_conn(conn, handler)` serves a `NotifyingHandler { server_name,
tx: mpsc::UnboundedSender<ServerChange> }` instead of `()`; `ClientInner.service` becomes
...
ignored by the supervisor until resources/prompts are un-stubbed (§2.3, X-36). **Who owns the channel, stated
so a reconnect does not sever it:** `do_handshake` runs on `connect` *and* on every in-session `reconnect`
(`client.rs:107`, called from `:91-104` and `:221`), and each handshake builds a fresh `RunningService` and
therefore a fresh handler — so the sender must not be created per handshake, or the first respawn (§3.6 item
1, the same incarnation) would drop the only sender and end the supervisor's receiver while the server is
still enabled. `connect` creates the pair once and keeps the sender on `ClientInner` as
`changes_tx: Mutex<Option<UnboundedSender<ServerChange>>>`; every `do_handshake` clones it into the handler it
serves; `disconnect` `take()`s it beside the service (`client.rs:167`), so the receiver ends when — and only
when — T4 closes the client. The receiver waits between `connect` and E2 on `ClientInner` as well —
`changes_rx: Mutex<Option<UnboundedReceiver<ServerChange>>>` beside `changes_tx` — and
`McpClient::changes(&self) -> Option<UnboundedReceiver<ServerChange>>` hands it out **once** (a `take()`; the
supervisor calls it at E2 and spawns the per-server reader task described under the supervisor below); a
second call returns `None`. **Two constructors, not one:** `ClientInner` is also built by the literal in
`McpClient::disconnected_for_tests` (`client.rs:73-88`, behind the `test-utils` feature that
`openalpaca_core`'s dev-dependency enables, `crates/openalpaca_core/Cargo.toml:49`), which must initialise
`closed`, `changes_tx` and `changes_rx` exactly as `connect` (`:91-104`) does or the workspace does not compile
under test. A second client method lands beside them for §3.7 step 2: **`list_tools_once()`** — the `op` future
of `list_tools` (`client.rs:232-237`: lock the service, `list_all_tools`) under `request_timeout`, returning the
typed error on failure and **never entering `reconnect()`**. A notification therefore cannot outlive the incarnation that produced
it, and a reconnect inside that incarnation keeps delivering. Whether a given streamable-HTTP server ever
pushes the notification depends on it holding the GET stream; the design promises only "when it arrives".
Plugins have no equivalent: their `tools/list` is read once at load (`manager.rs:789`), and although the
plugin stdio channel does carry server-originated JSON-RPC notifications (`PluginProcess.notification_rx`,
`process_pool.rs:19-24` — best-effort, and nothing drains it today), the plugin protocol defines **no**
tools-changed method, so a plugin's tool set is fixed for the incarnation and changing it is `reload`. If such
a method is ever added it must feed the same `on_tool_list_changed` path below, never a second refresh route.
...
1. **Coalesce.** One refresh runs at a time per server; a notification arriving mid-fetch or mid-apply sets a
   `dirty` flag and the refresh re-runs once (with a fresh fetch) when it finishes — never more than one
   queued. A chatty server cannot stampede `tools/list`.
2. **Fetch — outside the mutex, one shot.** The reader task calls `client.list_tools_once()` (above) with
   **no** mutex held. Not `list_tools()`: that method (`client.rs:233-256`) loops through `reconnect()` on every
   retriable error — backoff sleep plus a full re-handshake, up to `max_reconnect_attempts` — so a refresh
   triggered by a dying server would hold whatever it held for minutes; rev 6 ran it *under* the per-extension
   mutex, during which a queued `disable` could perform neither W nor T0: the owner's switch would hang while
   calls kept succeeding. **On failure: keep the recorded set** — `warn!(ext, error, "tool list refresh failed;
   keeping previously discovered tools")`; nothing is withdrawn (Claude Code's rule; a transient error must not
   unpublish a working server). Then classify, because the refresh must not leave a dead client under a live
   row: if the typed error is one `classify_call_failure` (§3.6 item 1) maps — `McpError::Unauthorized` once
   it exists — or if `client.connection_state()` (X-5 c) now reads `Failed{..}` because a concurrent tool call's
   ladder ended in `ReconnectExhausted`, call `mark_failed(ext, g, reason, detail)`; the reaper then runs T1
   over the recorded set exactly as for any crash. Otherwise return and let the next tool call's ladder decide.
3. **Mutex + re-check, then apply.** Take the per-extension mutex; proceed only if the record reads `Enabled`
   **and** its generation equals the message's `g`. Anything else — `Disabling`, `Disabled`, `Failed{*}`, or a
   newer generation — logs `debug!(ext, generation, "tool list change superseded")` and drops the fetched
   list. **A Disabled or Failed server's notification therefore does nothing**, by construction: such a
   server's client was sealed at T4, at its failed bring-up (E-FAIL), or never built, its receiver is gone with it — or, for a pre-reaper `Failed{Crashed}`, the client is still live until the first residue exit — the reaper's T4, E-PRE, `disable`'s T4, `deny`'s or declaration-gone's T1→T2→T4 — its step-2 fetch may still run (it holds only the service lock), and the notification reaches this step only to fail the state check under the mutex; a straggler queued
   before T4 fails the same check; a `mark_failed` that landed between step 2 and this step
   fails it the same way, so a diff is never applied onto a `Failed{Crashed}` record. There is no path by
   which a non-`Enabled` server can change the registry. The mutex is held only for steps 4–7 — in-memory
   work, microseconds — which is the term §3.2 T4 and §8 count as the refresh's contribution to `mutex_wait`.
4. **Diff** the new names against the **live subset** of this extension's retained names —
   `retained \ server_withdrawn` — treating every server-withdrawn name as *absent* for the diff: `removed`,
   `added`, `kept`. The base matters: diffed against the whole retained set, a name the server dropped in one
   notification and re-added in a later one is still in `tool_names` (flagged), so it would fall into `kept`,
   never be re-registered, keep its tombstone, and step 6's unflag would be unreachable — the name would stay
   refused on both gate arms with the *"withdrawn by the server"* wording until a `reload`. Against the live
   subset the re-added name lands in `added` and goes through step 6 (replace, `restore_caps`, unflag).
...
6. **Added names — E4 verbatim.** Each goes through `replace()` with the **case-13 collision rule**: a live
   incumbent (`owner_of(name)` is a different extension that is `Enabled` **and** does not hold the name in its
   own `server_withdrawn` set) → skip with `warn!` and append to this row's `skipped_tools`, never silently
   shadow; a dead incumbent — not `Enabled`, or `Enabled` but server-withdrawn — → take the name, removing it
   from the incumbent's retained set and its `server_withdrawn` flag. The new `ToolBackend::Mcp` literal
   ...
   just wrote for a tool removed in the same change. **A re-added name** — one this extension still holds in
   `tool_names` flagged `server_withdrawn`, which step 4's diff base puts in `added` — is not a collision
   (`owner_of(name)` returns this extension, not a different one): it is registered under the current
   generation like any other addition, `restore_caps` clears its capabilities' tombstones under this extension,
   and it is removed from `server_withdrawn`.
7. **Record.** `record_tools(ext, live ∪ server_withdrawn)` — the retained set is the union (a name re-added in
   this change is in `live` and no longer flagged, so the union carries it once), so attribution of
   the removed names survives exactly as it survives a disable (`record_tools` never touches the
   `server_withdrawn` set itself; only E5's `restore` clears it, §3.3); the API row's `tools` is the live subset and
...
call from a pre-change snapshot refuses with the *"withdrawn by the server"* wording and one `warn!`, the
name's capability is tombstoned under the server's id, and the row lists the name under `withdrawn_by_server`
(the dependent-scan **event** for this path — a template named in one `ExtensionCapabilityWithdrawn { cause:
ServerListChange, state: Enabled }` — is asserted in **C4**, where the scan, the variant and the agent-registry
handle land; C2 cannot compile that assertion); a removal and an addition in one change leave the removed
tool's capability tombstoned and the added tool's capability live; a notification emitted just before
`disable` → after T5 the registry holds nothing from the server and the log shows *"superseded"*; a refresh
whose `list_tools_once` fails leaves the set and the row byte-identical; **a server that hangs on
`tools/list`** → the refresh returns within `request_timeout`, and a `disable` issued during it completes W and
T0 without waiting for the fetch (the mutex is free), then T1–T5 normally; **remove, then re-add across two
notifications** → the server removes X (a call is refused with the server-withdrawn row, `withdrawn_by_server:
["X"]`), then re-adds X → X is registered under the **same** generation, its capability's tombstone under the
server's id is restored, `withdrawn_by_server` is empty, and a call to X succeeds (the step-4 diff base).
...
- `Enabled`, `Enabling` and `Failed{*}` ⇒ `disposition == true` — written at W before E0, so a bring-up that
  ...
  and tries again (§3.4). **One scan-time exception, stated:** a plugin directory parked at scan as
  `Failed{ConfigInvalid, "manifest name does not match directory"}` (§2.2, X-3) reached neither W nor E0 — the
  name check runs before the consent gate and before the bit is read — so its row reports the bit **as read**
  from the directory-keyed entry (`false` if the owner had pre-set it there), and §9.2's toggle, which binds to
  `record.enabled`, renders accordingly. Every other `Failed` record passed through W.
...
| `Failed{*}` (bit=true) | → `Enabling` (retry: E0 CAS → generation bumps → E-PRE T1→T2→T4 on any held handle → E1–E5) | → `Disabling` (T4 tears down a held handle, else no-op) | → `Enabling` (as `enable`: E0 CAS → E-PRE on any held handle → E1–E5) | stays `Failed`; consent re-recorded, **never a load** — Retry is `enable` | → T1→T2→T4 if a handle is held (pre-reaper `Failed{Crashed}`), then `Unapproved{Denied}` | — | — | — | — | → T1→T2→T4 if held, then `Orphaned` | → T1→T2→T4 (no write) if held, then record dropped |
| `Enabling` (bit=true) | queued after commit; then the outcome row (a queued `enable` that lands on `Failed{*}` retries — a second, harmless bring-up) | queued after commit | queued after commit | queued after commit | queued after commit | → `Enabled` | → `Unapproved{NeverSeen\|Denied\|CapabilitiesGrew}` (bit stays true) | → `Failed{reason}` | `mark_failed` is a no-op here; E2/E3 error → `Failed{reason}` | — | — |
...
The `Enabled` + *declaration gone — plugin* cell has **no trigger before GAP-24**: nothing watches the plugins
root (`main.rs:259-292` lists `agents/`, `skills/` and the config files only) and `PluginManager::start()`
scans once (`manager.rs:210-235`), so a directory removed mid-session is noticed at the next boot — by which
time nothing is `Enabled` and the entry parks as `Orphaned` from the boot gate. The cell exists for GAP-24's
uninstall path, which runs it in-process (§12.1); an implementer should not look for a watcher.
...
refined only about the *affordance*. (S3's literal *Crashed* — the plan's N5 row keeps the owner's wording,
ADR-030 says *Failed with a reason* — is realised as `Failed{Crashed}`, one `FailureReason` among five; nobody
should look for a state named `Crashed`.)
...
- **Plugins:** the `initialize` / JSON-RPC error may carry `error.data.reason == "needs_authorization"` and an
  optional `hint`. Absent that, it degrades to `Failed{Unreachable}` or `Failed{Crashed}`. **Correction
  (rev 16, R17):** as built, that arm is reachable only through `PluginError::RpcError { data: Some(..) }` —
  the only error variant carrying `data` — so a plugin whose bring-up fails any other way can never be
  `NeedsAuthorization`, and today no shipped plugin sends the signal. The arm is honest, not dead: it fires the
  moment a plugin sends it (`plugins/manager.rs::classify_bringup`).
...
- **HTTP MCP, the exact limit (rev 16, R14).** `McpError::Unauthorized` is raised **only** for a `401` carrying
  a `www-authenticate` header or a `403` carrying one — rmcp types exactly those two as
  `StreamableHttpError::AuthRequired` / `InsufficientScope` and folds every other status into an untyped
  response error before it reaches this crate (`openalpaca_mcp/src/error.rs::unauthorized_status`). A bare 401
  without the header therefore classifies `Unreachable`, and stdio can never classify here at all. No string
  matching was added to widen this — the design prefers a missed CTA to a guessed one.
...
list shows exactly what is wrong, and boot normally. That is the **boot** rule, where there is no last-good
declaration set to keep; under the watcher an unparseable rewrite registers the same pseudo-record and
**skips the diff**, so nothing running is torn down (§10 case 15).
...
- **Assembly-time has two roles, and only one of them is a courtesy.** As *prompt hygiene* — keeping dead tools
  ...
  | plugin skill | `invocation.rs:951-961` — names of the resolved defs | **allow-everything** (`security/capabilities/mod.rs:106-113`) — §6.2 #11; **deny-all once the `Allowlist` type below lands (A0, else C5 — never C1)** |
...
- **The no-match semantics belong in the evaluator, once — not in five callers' discipline (X-22 / bug A).**
  ...
  **moot for security** — every row is deny-all — and the caller-side total-loss refusals of #10/#11 remain for
  ...
  if A0 has not landed when C5 is cut, **C5 carries it — never C1**: constructing `Only(..)` at the seven policy
  sites flips the plugin-skill empty allowlist (`invocation.rs:951-961` → `check_agent_capability`
  `:106-113`) from allow-everything to deny-all, a security-relevant behaviour change that has no place in
  C1's no-functional-change commit and every place in C5, the security commit (§12). Tests, wherever it lands:
...
| **8** | `manager.rs:601` (`deny_plugin`) | Write the denial first (W-deny, §3.2 — `approved = Some(false)`, `approved_at`, `capabilities = []`, `enabled` untouched; `500` + no CAS on write failure), then run T0–T4 and commit `Unapproved{Denied}` at T5; report a consent word (`denied`), never `disabled`. A crash between T4 and a write that came *after* the teardown would leave `.permissions.toml` reading `approved = true`, the plugin would restart `Enabled` at the next boot, and a consent decision would be lost — the ordering §3.2 W-deny, the §4.1 `Enabled`+`deny` cell, C3 and ADR-030 (a) all fix. | **Live security hole today** — deny leaves the child running with tools registered while reporting `"disabled"`. |
...
| **10** | `skill/invocation.rs:152` **and its legacy branch `:153-179`**; `skill/invoke_executor.rs:157` **and its legacy branch `:160-172`**; the plugin-skill fallback `skill/invocation.rs:952-960` | **Both resolution branches, one predicate.** *`requires_capabilities` branch:* consume `resolve_capabilities`; **refuse** when **any** required capability classifies `withheld` (§7.2 — empty lookup with a recorded provider), naming the extension — this is exactly `!CapabilityOracle::is_satisfiable(&fm)` (#12), so invocation, the router, `<available_skills>`, the cron skip and `/slash` agree on one rule, and a non-empty list resolving to **zero** defs is merely the special case in which every capability is withheld (the case #11's escalation closure names, not the rule). *Partial* means **`partially_withheld` only** — a surviving provider of the same capability, so every required capability still resolves to at least one tool — and runs with the chat-visible prefix; a skill with `requires_capabilities: [A, B]` where A is wholly withheld and B live is **refused**, not degraded, even though its defs are non-zero. *Legacy `tools.allow` branch* (`else if !tools.allow.is_empty()`, three lines below each site — documented and supported, `docs/Skill_Template_Reference.md:207`, `:522`; four shipped skills use it, builtins only today): for every name `tool_registry.get(name)` misses, consult `owner_of(name)`; if it returns an extension that is **not `Enabled`** (or `Enabled` with the name server-withdrawn, §3.7), treat the name exactly like a `withheld` capability — the same attributed `warn!` + `ExtensionCapabilityWithheld { Moment::SurfaceAssembly }`, the same total-loss refusal (every allowed name withheld → refuse, naming the extension; this name-level reading is the legacy arm of the same `is_satisfiable(&fm)`, #12) and partial-loss chat prefix — at **both** file-skill sites. Names with no ledger owner keep today's unattributed *"references unknown tools"* warn at the top-level site (`invocation.rs:170`) **and gain it on the nested path**, which today has no warn at all (`invoke_executor.rs:160-172` → `:175` deny retain → `:372-374` `sandbox_policy = None` — that path's binding; `policy_opt` is the top-level `invocation.rs:344` and main-loop `simple_query_handler.rs:260` name — a toolless run in silence). *Plugin-skill fallback* (`invocation.rs:952-960`, `fm.tools.allow.clone()` — never resolved against the registry): apply the same `owner_of` scan to the allow list so a plugin skill whose every allowed name is withheld is refused up front (#11's total-loss rule) rather than running against a policy whose names the gate will refuse one by one; the gate's miss arm already attributes each such call, so this is attribution-at-entry, not a new safety boundary. **Why S4 forces this and not "out of scope":** S4 is categorical; on the nested legacy path a withdrawn extension tool disappeared with no log line, no event and no chat notice — literally silent — and even at top level the outcome was the one this row names as the failure mode, with an unattributed log line as the only trace. | Silent degradation on **both** branches: the SKILL.md body still tells the model to use the missing tool, so the reliable outcome is a confidently fabricated result — and on the nested legacy path, with nothing in the log either. |
| **11** | `skill/invocation.rs:951-961` (`invoke_plugin_skill`) + `security/capabilities/mod.rs:106-113` (`check_agent_capability`) | **Fail closed** on total loss — at the **caller** (refuse up front with the S4 wording when `!is_satisfiable(&fm)` — any required capability `withheld`, the one predicate of #10/#12. **Correction (rev 16, as built in C5, `adf55df`):** the predicate does **not** cover "`requires_capabilities` non-empty but resolves to nothing" in general — a capability nothing ever recorded classifies `unknown` (§7.2), which is *satisfiable* by design so typos keep degrading rather than refusing, and §10 case 8's restart shape lands there. So A0's `resolved.is_empty()` refusal is **kept beside** the predicate on the plugin-skill capability arm, not folded into it; pass `Only(resolved)` otherwise, and `Only(allow)` on the `tools.allow` fallback so an empty legacy list also denies) **and at the callee** (the `Allowlist` type of §6.1: `Only(empty)` denies every non-ambient capability, so no future resolver can reopen this by handing an empty vec to a policy). | **PRIVILEGE ESCALATION.** `allowed_capabilities` is set to the resolved tool *names* (`:951-961`), and `CapabilityManager::check_agent_capability` treats an **empty allow list as ALLOW EVERYTHING** (`security/capabilities/mod.rs:104-113`). A plugin-backed skill whose every declared capability came from a now-disabled extension gets an empty allowlist and can call **any** tool in the registry through `SandboxToolCallback` (`invocation.rs:1067-1092`). Disabling an extension currently **widens** that skill's reach. The file-based path dodges this only by accident, via `if !tool_defs.is_empty()` (`:281`) sending `policy_opt = None`. |
| **12** | `skill/router/mod.rs:101` | Beside the existing `invoke.mode == "disabled"` skip, drop candidates for which `is_satisfiable` is false — **any** one of their `requires_capabilities` wholly withheld (the same predicate #10/#11 refuse on at invocation). `SkillRouter::route` takes only `(&str, &SkillCatalog)` and has no registry handle, so the catalog gains `set_availability_oracle(Arc<dyn CapabilityOracle>)` — a one-method trait **implemented by `ToolRegistry`**, not by the ledger: `withheld` (§7.2) is "the `capability_index` lookup is empty *and* the tombstone set is non-empty", and the ledger holds only the second half — a third, never-disabled provider could still serve the capability, which only the index knows. `resolve_capabilities` is already a registry method, so the oracle is `is_satisfiable(caps) = resolve_capabilities(caps, &[]).withheld.is_empty()` (the capability arm; `is_satisfiable(&fm)` is the frontmatter-dispatching function that selects the `requires_capabilities` arm or the legacy `tools.allow` name-level arm and calls it) on the `Arc<ToolRegistry>` the services bundle already owns. **The legacy `tools.allow` branch is covered here, not only at invocation (#10):** the oracle takes the skill's frontmatter, `is_satisfiable(&fm)`, and for a skill whose `requires_capabilities` is empty runs the predicate over its `tools.allow` **names** through `owner_of` — unsatisfiable iff **every** allowed name is owned by a non-`Enabled` extension or is server-withdrawn under an `Enabled` one (§3.7); names with no ledger owner — builtins, typos — count as satisfiable, preserving today's silent-degrade for unattributed misses. `resolve_capabilities` cannot cover this branch: plugin tool names are not capabilities (`manager.rs:839` — `provides_capabilities` is the manifest's `capabilities.provides`), so only `owner_of` works for both kinds. Without it a legacy-allow skill whose only tool went with a disabled server stayed a router candidate and in `<available_skills>` and was refused only at invocation — attributed and chat-visible, so S4 held, but §7.5's *"dropped before it can fire"* row was false for that branch. Same filter in `catalog_summary` (feeding `<available_skills>` via `build_skills_catalog_block`, `query_handler/mod.rs:212-238`) and the `invoke_skill` listing (`invoke_skill.rs:103-115`). | A `mode: auto` skill whose only capability came from a disabled server still auto-selects at score ≥ 0.65 and runs toolless, while `<available_skills>` keeps coaching the model toward it. |
| **13** | `apps/openalpacad/src/scheduled_skills.rs:147` | After the catalog lookup in `spawn_timer_turn`, check the same predicate (the `CapabilityOracle` of #12, legacy `tools.allow` reading included); skip with a `warn!` + `SystemEvent::ExtensionCapabilityWithheld { moment: Moment::ScheduledSkip, .. }` — the per-call variant with a third `Moment` (so C5 does not invent one; §7.3's `ExtensionCapabilityWithdrawn` is per transition and does not fit a per-fire event). **`ScheduledSkip` is exempt from §7.4's dedup:** each fire is a distinct unattended event and the §1 table promises a warn per fire; its `ScopeKey` is the **skill id**, carried on the event, so two cron skills on one extension never collapse into one line — with the generic `(ScopeKey, ExtensionId, Moment)` key and no `ToolContext` the scope would fall back to `"global"` and both skills would share one warn per ten minutes. **Do not deregister the cron job** — re-enable would then need a re-registration trigger `resync_skill` (`:86-97`) cannot provide, since it keys only on the catalog entry. Skip-and-log is idempotent and self-heals. | Unattended fabrication on a schedule: the turn goes through the gateway as a real user message on `{user}:scheduled` and the fabricated result is pushed cross-channel by the NotificationDispatcher. |
...
| `Disabling` (`pending_cause` ≠ `Reload`) | `<kind>` `<id>` is being turned off right now | — | do not retry it | — |
| `Disabling` entered by `reload` (`pending_cause == Reload`, §3.2 T0 / §3.4.1) | `<kind>` `<id>` is being reloaded right now | retry on your next round | do not report it as failed or as turned off | — |
...
One classification needs its own wording. A capability tombstoned under a still-`Enabled` owner (server-withdrawn,
§3.7 step 5) is `withheld` by the rule above, but no recorded provider is *blocked* — so the attribution the
`Moment::SurfaceAssembly` warn, the `/slash` refusal and the cron skip carry names the owner as **"still
enabled"** and renders the §7.1 *"Enabled, server-withdrawn"* row (for MCP the capability **is** the tool
name, so that row's `<tool>` is the capability), never the `Disabled` row. Precisely: `resolve_capabilities`
attributes each `withheld`/`partially_withheld` capability to every recorded extension that is non-`Enabled`
**or** holds the name in `server_withdrawn`, and the attribution says which of the two it is.

**Partial withdrawal does not affect router candidacy, `/slash` refusal or the cron skip** — only total loss
does. That matches §10 case 3's total/partial split, stated per capability, not per tool: a skill every one of
whose required capabilities still has a provider runs and carries the chat-visible warning; a skill **any** of
whose required capabilities is `withheld` is refused — even if its other capabilities still resolve to tools.
`CapabilityOracle::is_satisfiable` is therefore "no requested capability is `withheld`", `partially_withheld`
is reported, never gating, and the **same** predicate is evaluated at invocation (§6.2 #10, #11), by the
router and the catalog (#12), by the cron skip (#13) and by the `/slash` tier — one rule, not two.
...
(`requires_capabilities`; for a skill whose `requires_capabilities` is empty, its legacy `tools.allow` names,
matched against the withdrawn tool **names** through the same `owner_of` reading as §6.2 #12 — total loss
there means every allowed name is withdrawn) and emits **one** un-deduplicated `tracing::warn!` naming the extension and
...
SystemEvent::ExtensionCapabilityWithdrawn {
    ...
    state: ExtensionState,               // Disabling on the route/watcher/deny/reload paths, Failed{Crashed,..} from the reaper and from deny/declaration-gone residue (which never enter Disabling), Enabling from E-PRE's scan (C4), Enabled from §3.7, and the reload's outcome state on the deferred publish below
    ...
    tools: Vec<String>,                  // the withdrawn tool NAMES (T1's list) — what the legacy tools.allow scan matches on
    affected_templates: Vec<String>,
    affected_skills: Vec<String>,        // skills now unsatisfiable: !is_satisfiable (≥1 required capability wholly withheld)
...
`Reload` suppresses the cron notice when the reload ends `Enabled` (§3.4.1) — by the deferred publish stated there.
**The notice's `<detail>` is untrusted text** (a server or plugin wrote it), so C4 renders the cause
detail-free and appends the detail below through `describe::wrap_detail` — §7.1's wrapping rule, widened from
the tool-result row to this chat-visible notice, because the model reads this row back.
...
`apps/openalpaca-gui/src/lib/events.ts:62-67`; `:61` is the GAP-22 comment), making plugin rows unorderable. This family does not repeat
that.
...
re-announces rather than being swallowed. The transition warning of §7.3 is never deduped, and neither is
`Moment::ScheduledSkip` (§6.2 #13) — a cron fire is a distinct unattended event, and its scope key is the skill
id, not the `ToolContext` fallback. The warned-set governs **observability only** — `warn!` lines and bus events. It never suppresses the
...
| auto-routed skill | no — it is dropped from candidacy before it can fire, on both resolution branches (§6.2 #12) | nothing was attempted |
| cron skill (either branch, §6.2 #13) | no per fire; **one notice on the owner's default lane per transition** (§7.3), written to the conversation store and fanned out cross-channel — not pushed through the connector-only progress path | nobody is watching |
...
(api-fix-plan §7 rule — *"List shapes follow the existing legible rule"*, line 838 in plan rev 3).
...
Runs W, then T0–T5 synchronously. Bounded by the **per-extension mutex wait** (zero unless another holder is
in flight — an `Enabling`/`reload` bring-up, at most that server's `connect_timeout_secs`, default 30 s,
`tools/mcp/config.rs:40`; the reaper's T4, at most `request_timeout + 1 s`; or a §3.7 refresh's in-memory
apply step, microseconds, because its fetch runs outside the mutex — §3.2 T4 lists the four) **plus**
...
§3.4.1: T0–T4 then E0–E5 under one hold of the per-extension mutex, **no W** — the bit is untouched (from `Failed{*}`: no T0 and no drain — E0 CAS → E-PRE on any held handle → E1–E5, §3.4.1; a reload or enable from a pre-reaper `Failed{Crashed}` runs E-PRE's T1 step 3 with cause `Crash`, whose notice is the crash's one announcement and fires regardless of how the reload ends). From
...
`datetime('now')` — **UTC** text (migration 030 line 37) — so "since local midnight" uses today's local
midnight **converted to UTC** by the daemon (`chrono::Local::now().date_naive().and_hms(0,0,0)` →
`.with_timezone(&Utc)` → the same `%Y-%m-%d %H:%M:%S` text), not a bare `date('now')`, which would be off by
the UTC offset. **As built (rev 16): one grouped query, not a per-name seek** —
`SELECT tool_name, COUNT(*) … WHERE timestamp >= ?1 GROUP BY tool_name`
(`repository/skill_execution/mod.rs::tool_invocations_since`), read once per request into a map the row
builder looks names up in. It scans `idx_tel_tool_ts (tool_name, timestamp DESC)` (migration 030 line 39) as a
covering index rather than seeking it per name, which is the right shape at this table size and costs one
statement instead of one per registered tool. A per-name `WHERE tool_name = ?1 AND timestamp >= ?2` seek
remains the shape to switch to if the log ever grows past what a scan can serve.
...
`plugins/.config/<name>.toml` with a bare `fs::write` (`permission_gate.rs:119-135`, the write at `:133`) and
...
to MCP-server add/remove — and stays scheduled as `api-fix-plan.md` Phase 8 item 9, **already relabelled in plan
rev 3** (item 9 at line 822, summary row at line 870; §12.1); C7 replaces the id in the `GapId` union, where
`unavailable.ts:37`/`:233` still read `GAP-19`. GAP-18's tool-catalog
...
| `failed` / `config_invalid` | Toggle bound to `record.enabled` — ON, except a plugin parked at scan with a pre-set `enabled = false` entry (the X-3 exception, §4) — live | `config-invalid` | **`asks`** | Open config | the parse error |
| `failed` / `unreachable` \| `crashed` | Toggle **ON**, live | `crashed` | **`warn`** | Retry (= `reload`, §3.4.1 — as `enable` from `Failed`: E0 CAS → E-PRE on any held handle → E1–E5) | the transport error (rendered from `connection_state()`, X-5) + `since`; `detail` shown as a quoted diagnostic, never interpreted |
...
**fully backed** with no decorative disabled toggle — precisely the failure the api-fix-plan defers GAP-20
part 2 to avoid (a template `enabled` bit without spawn-path enforcement would be decorative — Phase 8 item 5,
line 818 in plan rev 3; rev 2's literal *"the toggles are decorative"* sentence is gone) and which `deregister_provider`
...
The six `plugin_*` mappings (`:67-73`) and their `ServerEvent` union members (`lib/events.ts:62-67`) are
...
| 3 | **Skill `requires_capabilities` includes a withheld one** | **One predicate everywhere — `CapabilityOracle::is_satisfiable`** (§6.2 #10, #11, #12, #13; §7.2): a skill is unsatisfiable iff **at least one** required capability is **wholly withheld** (empty resolution with a recorded provider, §7.2). **Partial loss** — `partially_withheld` only: one of two providers disabled, a surviving provider still serves the same capability, so every required capability still resolves — run with what survives; because it was explicitly invoked, prefix the result with the chat-visible warning naming the extension. **Total loss** — any required capability `withheld`, whether or not the *other* capabilities still resolve to tools: **refuse**. The refusal is mandatory, not stylistic (§6.2 #11 — it closes a privilege escalation). The same rule decides router candidacy, `<available_skills>`, the `/slash` refusal and the cron skip, so a skill the catalog hides is never one `/slash` or `invoke_skill` would run. `requires_capabilities` has no optionality in the schema (`middleware/skill/types.rs:329`) and inventing one is scope creep. **Unattributed** misses (typos) keep today's silent-degrade behaviour, so no existing skill changes behaviour on upgrade. |
...
| 6 | **Re-enable cannot connect** | When `connect`/`spawn` itself fails, E2 precedes E4, so nothing was published and there is nothing to unwind; a failure *after* connect runs E-FAIL (§3.3) instead. `enabled` stays **true**; `state = Failed{reason, detail, since}`. Route returns **200** with that row. GUI: toggle ON + `asks`/`warn` + CTA. Retry is `enable` again, idempotent. No exhausted-forever state — a genuine improvement over `ConnectionState::Failed{ReconnectExhausted}` (`client.rs:186-195`), which has no path back short of a restart. |
...
| 13 | **Two extensions register the same tool name** | Pre-existing and not solved here, but not made worse. `register` overwrites on duplicate and returns `Ok` (`registry/mod.rs:262`). The rule, stated once so §8's `skipped_tools` and this cell agree: **a name is blocked only by a live incumbent.** At E4 the newcomer asks `owner_of(name)`; if it returns a *different* extension whose state is `Enabled` **and** whose `server_withdrawn` set does not hold the name — only then is the tool live in the registry — the newcomer skips the name with `warn!(ext, tool, incumbent, "tool name collision — skipping")` and records it in its row's `skipped_tools`. If the incumbent is **not live** — not `Enabled` (a retained attribution from a disabled, failed or unapproved extension), **or** `Enabled` but holding the name in `server_withdrawn` (the server itself dropped it, §3.7; near-unreachable under `<server>__`/`<plugin>::` namespacing and X-3, but the rule must not be wrong) — the newcomer **takes the name**: `record_tools` writes `tool_names[name] = newcomer` and removes the name from the incumbent's retained set and from its `server_withdrawn` flag, so a later miss attributes to the extension that actually served it last, and the incumbent's own T1 (on its eventual disable) will not touch a name it no longer owns. `tool_names` stays a single `name → ExtensionId` map — two claimants are never recorded at once; the loser is either skipped (live incumbent) or displaced (dead incumbent). At T1 an extension removes only names the ledger currently attributes to it. Without this, disabling A could delete a tool B had overwritten it with, or a re-enabled A could be refused its own tool because a long-disabled B once had the name. **The same rule governs a name a server adds mid-session** (§3.7 step 6): the newcomer is the added tool, the check is identical, and the loser is skipped into `skipped_tools` — never silently shadowed. A real namespace fix is separate work. |
...
| 18 | **A connected MCP server changes its tool set mid-session** (`notifications/tools/list_changed`) | §3.7 in full. rmcp delivers the notification (`handler/client.rs:59`); OpenAlpaca's unit handler dropped it (`client.rs:436-437`). Under the per-extension mutex and only while `Enabled` at the notifying incarnation's generation: refetch `list_tools` (keep the old set on failure); diff against the **live subset** (`retained \ server_withdrawn`, §3.7 step 4), so a name the server dropped and later re-adds lands in `added`; removed names → T1 per name (tombstone + `remove()`), kept in `tool_names` and flagged `server_withdrawn`, refused on **both** gate arms with the *"withdrawn by the server"* wording, dependent scan + cron notice with `WithdrawalCause::ServerListChange`; added names → E4 with the case-13 collision rule and the current generation, per-capability `restore_caps`, and a re-added name unflagged; `record_tools` with the union; **no generation bump**; `ExtensionStateChanged { tools_changed: true }`. A notification from a `Disabled`/`Failed`/`Disabling` server does nothing — its client is sealed, its receiver is gone, and a straggler fails the state re-check. Surfaces pick the change up at the next request (§6.2 #2). Plugins have no such notification; their set is fixed per incarnation and `reload` is the way to change it. |
| 19 | **Two plugin directories carry the same manifest `plugin.name`** | The directory name is the id (§2.2, X-3): a manifest whose `plugin.name` differs from its directory is parked as `Failed{ConfigInvalid, "manifest name does not match directory"}` with no spawn, so two directories can never share a `PluginState` entry or a `.permissions.toml` entry, and the `manager.rs:263` insert refuses to replace a live entry regardless. Test: `two_dirs_same_manifest_name_second_is_config_invalid` (C3). |
...
Phase 8 (line 810 in plan rev 3) holds three relevant items:

- **GAP-18 (`GET /v1/tools`, `/v1/skills`)** — item 2, line 815, already reads *"owned by the extension
  design's C6; read-only until then"* and scopes the `AppState` plumbing this design needs (*"clone the
  tool-registry `Arc` before its move into `Orchestrator::new` at `main.rs:373`; two `AppState` fields"*); the
  rev-2 sentences rev 6 quoted (*"Per-tool enable writes stay out of scope pending the N5 design"*, *"it
  supersedes this bullet"*) no longer exist. **It does supersede it.**
...
- **GAP-19 (plugin install)** — item 9, line 822, `source:"path"` only, lands a directory in `WaitingApproval`. Still
  ...
  **not** in this plan. C7 replaces `"GAP-19"` with `"GAP-24"` in `apps/openalpaca-gui/src/lib/unavailable.ts`
  (union at `:37`, entry at `:233`); item 9 (line 822) and the summary row (line 870) are **already relabelled**
  in plan rev 3, so the 23-gap registry and the plan agree on landing once C7 lands. **Its specification lives in the plan
  ...
  in-process half.
- **GAP-20 part 2 (agent-template `enabled`)** — item 5, line 818 / summary row 868. Untouched. It is the ALLOW axis's own
...
document** — each names what changes if you say yes. Q1–Q4 carry a default that takes effect if you say
nothing. **Q5–Q14 carry a recommendation, not a default: silence enacts none of them and leaves the design as
written** — api-fix-plan §0 states the same rule for its pending table (*"the default is a recommendation, not
a decision"*, plan rev 3 line 58), and several recommendations below (Q7, Q9, Q10, Q11, Q13) are the *opposite*
of the design as written, which is exactly why they cannot be defaults. Nothing here is settled by S1–S4.
...
→ **Recommendation: no** (record the rejection in §11.2 as finding (4): a single-operator daemon has no policy author
...
→ **Recommendation: record the rule, build nothing yet** — measure from real bytes (lessons C-1), log once per lane
...
→ **Recommendation: yes**, in the moving-slot shape first — within S4's "and/or"; it reverses a §7.5 row, so the owner
...
→ **Recommendation: keep respawn** (a respawn is cheaper than a human retry on an unattended daemon). *If flipped:*
...
→ **Recommendation: yes** — a hand edit to a live server's `command`/`url` being silently deferred is the confusion
...
→ **Recommendation: yes** — one re-resolve, one retry, never a ladder, never a poller. *If no:* a rotated env value is
...
→ **Recommendation: yes** — a reason on an existing state, consent bit in the home store, never in the committed file.
...
→ **Recommendation: `secret_encrypted`** — D1-pure, and Claude Code itself falls back to an in-root secrets file when
...
→ **Recommendation: drop it if this design lands before Phase 8** — the `Extension*` family carries both fields from
...
→ **Recommendation: yes** — a fixed named set written into §6.1 and api-fix-plan A0, beaten by `denied_capabilities`,
...
- **rev 6** (2026-09-02) — reconciliation pass: residue round 4 (1 blocking, 29 non-blocking) + the Claude Code
  lessons (`tasks/research/claude-code-design-lessons.md` rev 3; rev 4 — a re-verification with no change to
  any design-targeted row — was on disk when the addendum below was written). **S4 — the blocking item:** the legacy
...
- **rev 6 — verification addendum** (2026-09-02, later the same day; revision number unchanged). The rev-6 text
  was re-read end to end against the round-4 residue, the lessons doc as it now stands (rev 4) and the tree.
  *Verified on disk, no change:* every residue item above resolves to text in the sections named; the
  anchors the residue flagged (`main.rs:337`/`:338-339`, `client.rs:54`, `manager.rs:152`/`:173-178`/`:193`/
  `:199`, `capacity.rs:145`, `events.rs:385`, `invoke_executor.rs:289`, `bootstrap/config.rs:63-66`,
  `PluginsSection.tsx:10-12`) resolve to the stated code; the §3.7 rmcp claims hold (`handler/client.rs:59`
  dispatches `ToolListChangedNotification` to `on_tool_list_changed`, whose default body is at `:222`;
  `impl ClientHandler for () {}` at `:257`; `().serve(..)` at `client.rs:436-437`; `RunningService<RoleClient,
  ()>` at `:59`); no owner-gated item (T1, T3, T4 incl. X-13/X-15/X-16/X-17/X-20/P-18, T6(a), T6(c) incl. X-11's
  auto-reload and X-28's watcher use, T7, T8, T9, T10–T15, P-2's cap change, G-3, X-33) is enacted anywhere —
  each is a §13 question or a pointer to the plan. *Three defects fixed:* (1) §3.7's channel ownership — the
  sender "lived in the handler", but `do_handshake` builds a new handler on every in-session `reconnect`
  (`client.rs:221`), so the first transparent respawn (§3.6 item 1) would have dropped the only sender and
  silently ended the supervisor's receiver while the server stayed `Enabled`; the sender now lives on
  `ClientInner` (`changes_tx`, created once in `connect`, cloned per handshake, taken by `disconnect`),
  `changes()` is a take-once accessor, and §3.3 E5 and C2 say so; (2) C2's seal-in-flight test asserted the
  sealed client returns `TransportClosed`, contradicting X-5 (a) three sentences later — it now asserts
  `McpError::Closed`; (3) C7 still cited `PluginEventSink` at `manager.rs:153` after §7.3 was corrected to
  `:152`. *Precision:* §3.7's "plugins have no change notification" now names `PluginProcess.notification_rx`
  (`process_pool.rs:19-24`, undrained) and states that the protocol defines no tools-changed method, so the
  claim cannot be read as "the channel carries no notifications". *Cross-file follow-ups unchanged and still
  owed elsewhere (not this file):* api-fix-plan §0 N5 line 46 → cite rev 6 (write-first bit, install-point
  seal, generation-carrying reaper, `reload`, §3.7); Phase 8 item 9 / summary row GAP-19 → GAP-24;
  `unavailable.ts:18-41`/`:37`/`:233`; ADR-030 citing rev 6 and recording the write-first and install-point
  decisions; `scripts/release/templates/config/mcp.toml` created in C2; the seal at both `client.rs:180` and
  `:137`; `PluginManager::with_event_bus` and `plugin_skill_frontmatter_never_carries_cron` in C3. *[rev 7:
  the plan §0 N5 citation, the Phase 8 item 9 / summary-row relabel and ADR-030 had already landed when this
  addendum was written — plan rev 3 lines 47, 822, 870 and `10-decision-log.md` ADR-030, commit `94a8f42`; only
  the GUI `GapId` union and the code-side obligations remained, as the rev-7 status line now says.]*
- **rev 7** (2026-09-02) — reconciliation pass: round-5 critique (1 blocking — plan-side, recorded here as owed;
  ~45 non-blocking) + the repaired lessons doc (rev 4), every design-targeted adopt/adapt row re-verified in
  place. **§3.7 (X-35) reworked:** the refresh **fetches outside the per-extension mutex** through a new
  one-shot `McpClient::list_tools_once()` that never enters `reconnect()` — `list_tools` (`client.rs:233-256`)
  loops through the reconnect ladder, so rev 6's fetch-under-mutex let a dying server hold a queued `disable`
  behind minutes of backoff with neither W nor T0 run; the mutex is now taken only for the in-memory apply
  (state + generation re-check first), a failed refresh consults `connection_state()`/`classify_call_failure`
  and `mark_failed`s so a dead client never sits under a live row, coalescing covers fetch and apply, the
  receiver's home between `connect` and E2 is `changes_rx` on `ClientInner`, and `disconnected_for_tests`
  (`client.rs:73-88`) is named as the second `ClientInner` literal that must initialise the new fields; the
  mutex holders and their bounds are listed once (§3.2 T4) and §8's `disable` bound cites them. **E5 vs
  step 7:** `record_tools` no longer clears `server_withdrawn` — `restore(ext)` does — so the union write at
  step 7 keeps the flags it relies on; the receiver is taken and the reader spawned at E2, first acted on
  after E5 (the mutex). **Case 13:** a live incumbent is `Enabled` **and** not server-withdrawn; a dead one is
  displaced and its flag cleared (§3.7 step 6 agrees). **Legacy `tools.allow` reaches availability:** the
  `CapabilityOracle` (§6.2 #12), the cron skip (#13) and the T1-step-3 dependent scan (§7.3, event field
  `tools`) run the `owner_of` reading over a skill's allow list when `requires_capabilities` is empty —
  `resolve_capabilities` cannot (plugin tool names are not capabilities, `manager.rs:839`); C5 gains the router/
  cron tests. **`reload` wording:** `begin(ext, Disabling, cause)` stores `pending_cause`; `describe` renders
  *"is being reloaded right now"* for that window (§7.1 row, §3.4.1). **Cron-skip dedup:** `ScheduledSkip` is
  exempt; scope key = skill id (§6.2 #13, §7.4). **Watcher on an unparseable `mcp.toml`:** pseudo-record +
  skip the diff, last-good desired set kept, dropped on the next good parse; fail-closed is a boot rule (§5.1,
  cases 9 and 15; C2 test). **`Allowlist` home:** A0, else C5 — never C1, because `Only(..)` at the seven policy
  sites flips the plugin-skill empty allowlist to deny-all (§6.1 prose + table, C1, C5). **X-3 scan order
  pinned** (name → consent → bit) and the scan-time `Failed{ConfigInvalid}` record carries the bit as read —
  the one exception to `Failed ⇒ true` (§2.2, §4, §9.2 row, C3 test). **Fingerprint preimage defined:** the
  parsed, value-masked `McpServerConfig` (deriving `Serialize`) → `toml::Value` → keys sorted recursively →
  `toml::to_string` → `blake3`; comment/key-order edits are inert (§3.3 E2, §2.1, C2). **§7.2:** a capability
  tombstoned under a still-`Enabled` owner is attributed "still enabled" and rendered with the server-withdrawn
  row. **§4.1:** the `Enabled` + declaration-gone-plugin cell has no trigger before GAP-24 (no watcher on the
  plugins root). **§4.2:** S3's *Crashed* ↔ `Failed{Crashed}` bridge. **§12 sequencing:** the list-change
  dependent-scan event assertion moves from C2 to C4; C4 declares the `ScheduledSkip` arm and C5 publishes it;
  C4's stale "keyed on state" clause deleted; C3 surfaces the `StoreUnreadable` refusal that C6 maps to 409;
  `check` is 4-arg everywhere; `blake3` needs nothing (`Cargo.toml:142`, core `:37`). **§13:** the preamble
  says silence enacts none of Q5–Q14 and every Q5–Q14 arrow reads *Recommendation*. **Plan/KB anchors
  re-verified against api-fix-plan rev 3 and ADR-030:** N5 is line 47; Phase 8 heading 810, item 2 815, item 5
  818, item 9 822; summary rows 865/868/870; §7 list-shape rule 838; the rev-2 sentences §12.1 quoted are gone
  and are no longer quoted; the plan-side N5/GAP-24/ADR-030 items are marked landed in the addendum, C8 and
  §9.1. **Anchors corrected:** `transport/mod.rs:39-40` (not `stdio.rs:120`), `handler/client.rs:222`/`:226`,
  `manager.rs:560-562`, `:547`/`:555`, `:263` (block `:261-280`), `client.rs:47-51`, `permission_gate.rs:133`,
  `events.ts:62-67`, `invoke_executor.rs:372-374` binds `sandbox_policy`. **Cross-file items this file cannot
  fix, recorded once so they are not lost:** (1) api-fix-plan Phase 0 needs **A5 — main-loop cost lockout,
  option 1 (C-3/P-3)**: baseline `LoopState.last_cost` (`runner/agentic_loop/mod.rs:68`) from `agent_cost()`
  (`backend.rs:167-189`) before round 0, delta at `:313-317`, no daily budget; a *Verify* clause (after >$1
  cumulative `orchestrator` spend a fresh turn does not exit `CostExceeded` before its first LLM call; the
  per-turn $1 cap still trips within a turn); the roadmap's Known-broken row → "Confirmed by ADR-028; not yet
  reproduced live. Scheduled: Phase 0 A5" and its Phase 0 cell; (2) plan Phase 0 **A1** should write the denial
  *first* to match ADR-030's write-first W-deny (§3.2) — a crash mid-teardown must not lose a consent decision;
  (3) plan N5's S3 sentence gains "— realised as the design's `Failed{reason}`; `Crashed` is one reason";
  (4) `10-decision-log.md:167` "rev 2" → rev 3; (5) plan Phase 8 item 1 / ADR-027: `apps/openalpaca/src/manager.rs:39`
  is `File::create` (fresh per start), not append — rotation still warranted; (6) plan §1.4 needs
  `install_id`/`project_id` readers beside `layout_version`; (7) plan §1.2 rule (2) should be plugin-shaped
  with T8 as the rule, not a footnote; (8) plan P-17's `kind: "builtin"` is unreachable under its own
  condition; (9) plan §0 pending table lacks a T8 row; (10) `push_steering` is `runner/steering.rs:60-79`;
  (11) GUI `unavailable.ts:37`/`:233` `GAP-19` → `GAP-24` (C7). **Owner-gated items remain unapplied
  throughout:** T1 (Q5), T3 (Q6), T4 incl. X-13/X-15/X-16/X-17/X-20/P-18 (Q7), T6(a) (Q8), T6(c) incl. X-11's
  auto-reload and X-28's watcher use (Q9), T7 (Q10), T8 (Q11), T9 (Q12), T14 (Q13), T15 (Q14); T10–T13
  (incl. P-2's cap change) are plan-shaped and only pointed at; G-3 and X-33 are not built.
- **rev 8** (2026-09-02) — round-6 critique: three blocking findings on rev 7 (all internal consistency; the
  three directive lenses were READY on rev 6 and the fidelity lens found the same three), each applied exactly
  as suggested and then checked against every other statement of the same rule. **(a) §6.2 #8 deny ordering →
  write-first.** The ordered-enforcement row said "teardown **before** writing the denial" while §3.2 W-deny,
  the §4.1 `Enabled`+`deny` cell, C3's "step W write-first for `approve`/`deny`" and ADR-030 consequence (a)
  all say the denial is written at W and T0–T4 follow; the row now reads the W-deny shape (write
  `approved = Some(false)`, `approved_at`, `capabilities = []`, `enabled` untouched, `500` + no CAS on write
  failure; then T0–T4; `Unapproved{Denied}` at T5; consent word `denied`) with the crash-between-T4-and-write
  rationale. One row; nothing else in the mechanism moves. **(b) §3.7 step 4 diff base → the live subset**
  (`retained \ server_withdrawn`). Diffing against the whole retained set put a server-re-added name into
  `kept` (still in `tool_names`, flagged), so it was never re-registered and step 6's "a re-added name is
  removed from `server_withdrawn`" was unreachable; the name stayed refused on both arms until a `reload`.
  Step 4 now names the base and why; step 6 states that a re-added name is not a collision (`owner_of`
  returns this extension) and goes through replace + `restore_caps` + unflag; step 7's union carries it once;
  case 18 names the base; a **seventh C2 test** (remove X → refused with the server-withdrawn row → re-add X →
  registered under the same generation, tombstone restored, `withdrawn_by_server` empty, a call succeeds) is
  listed in §3.7 and in the C2 verification cell ("the seven §3.7 tests"). **(c) One skill-refusal
  predicate.** §6.2 #10 ("non-empty list resolving to **zero** defs → refuse; partial → run") and #11
  ("resolves to nothing") diverged from §10 case 3, §7.2, #12 and §7.5 ("unsatisfiable iff at least one
  required capability is wholly withheld") on any multi-capability skill — `[A, B]` with A wholly withheld and
  B live ran with a prefix at invocation but was hidden by the router and the catalog. All sites now state the
  case-3 rule as `CapabilityOracle::is_satisfiable`: refuse when **any** required capability classifies
  `withheld`; "partial" is `partially_withheld` only (a surviving provider of the same capability, so every
  required capability still resolves) and runs with the prefix; "resolves to nothing" is kept as the special
  case #11's escalation closure names, not as the rule; #12's opening reads "any one of their
  `requires_capabilities` wholly withheld"; the legacy `tools.allow` name-level reading (#10, #12) is named as
  the other arm of the same oracle. C5 gains the multi-capability test (refused at top level and nested,
  absent from router candidacy and `<available_skills>`, cron skipped; the partially-withheld variant runs and
  stays listed). **Not applied, by instruction:** nothing else; no owner-gated item (§13 Q5–Q14); no
  shortening. **Cross-file items still owed (unchanged from rev 7's list):** (2) plan Phase 0 **A1** still
  says teardown-then-write and should flip to write-first to match this file's #8 / §3.2 / ADR-030 (a); the
  roadmap's Known-broken row for the cost lockout; the GUI `GapId` union; items (3)–(11). Item (1) — plan
  Phase 0 **A5** with its *Verify* clause and §8 row — landed in the plan alongside this revision.
- **rev 9** (2026-09-02) — round-7 critique (1 blocking, plan/KB lens 1 blocking, 37 non-blocking). **C1 builds alone:**
  the `event_bridge.rs:30` arm for `SystemEvent::ExtensionStateChanged` moves into C1 as a `debug!` no-op (the
  daemon's `SystemEvent` match is exhaustive by design — verified: zero catch-all arms); §3.2 T5, the C1 cell and a
  new §12 rule ("a commit that adds a `SystemEvent` variant adds its bridge arm in the same commit") say so; the
  `ServerEvent` peer, forwarding and persistence stay in C2. **Anchors:** §3.2 T2 steps 2/3 re-read against
  `manager.rs` — `catalog.remove` is `:546` (`for` `:545`), `remove_template` `:554` (`for` `:553`). **Cross-file:**
  plan Phase 0 A5 (cost lockout, option 1) and A1 (deny write-first) landed; roadmap Known-broken row updated.
  Edited by hand against the critics' exact text; re-verified by the consistency and plan/KB lenses.
- **rev 10** (2026-09-02) — round-8 critique (consistency lens: 1 blocking, 8 non-blocking; plan/KB lens: ready).
  **S2 hole closed (E-FAIL, §3.3):** any exit from E2–E4 after `connect`/`spawn` returned runs T4 + T4b on the
  just-built handle before `Failed{..}` commits — previously a `list_tools` failure after a good handshake left a
  live, unsealed client and child under a `failed` row that a later `disable` could not reach. The supervisor's
  handle map now holds entries only for `Enabling`/`Enabled` [superseded in rev 11: also a pre-reaper `Failed{Crashed}`, until the reaper's T4 or E-PRE]; E4b's opening widened; §3.7's "sealed at T4 (or never
  built)" corrected; a C2 test named. **Cleanups:** C2 cell no longer claims the `SystemEvent` variant/bridge arm
  (C1's, per rev 9) — it upgrades the `debug!` arm; §12 rule extended to `ServerEvent` + `events/persistence.rs:12`
  (also exhaustive); `ledger.begin` given one signature (`cause: Option<WithdrawalCause>`, `None` for `Enabling`);
  the two `is_satisfiable` forms named as the capability arm and the frontmatter dispatcher; `state/backups/` rooted
  at `paths::app_dir()` pre-Phase-1; `affected_skills` comment restated per §7.2; anchors `:546`/`:554` (§6.2 #14,
  §10 case 5) and `:559-561` (T2 step 4). Hand-edited against the critic's exact text.
- **rev 11** (2026-09-02) — round-9 critique (consistency lens: 1 blocking, 10 non-blocking). **S2 hole closed
  (E-PRE, §3.3):** every handle-building verb (`enable`/`reload` from `Failed{*}`, the config-route enable) runs
  T1–T4 on any handle the map still holds before E0. Previously a Retry that won the mutex before the reaper built
  load N+1 over load N's live residue (stale-generation tools still advertised, a leaked reader task, a duplicate
  capability provider) and the reaper's "superseded" branch then did nothing. Invariant restated: at most one handle
  per extension; `Failed{Crashed}` owns its handle until the reaper's T4 or the next verb's E-PRE. §3.4.1, §4.1
  `Failed{*}` row, §8 Retry, the §3.6 trace and both *Reaper superseded* tests updated. **Cleanups:** E3 uses
  `list_tools_once()` (bring-up no longer loops through `reconnect()` under the mutex); T1 step 1 says an absent
  registry entry contributes nothing (the idempotency argument); E5 "applied" + overlap note; T0 names its five
  non-route entrants; §3.6 item 3 states the plugin held-process window; #12 stray backticks; §3.7 anchor
  `client.rs:232-237`. Hand-edited against the critic's exact text.
- **rev 12** (2026-09-02) — round-10 critique (consistency lens: 2 blocking, 13 non-blocking). **Handle ownership
  is now a table (§3.3.1)**, every cell derived from a named rule, and the two remaining contradictions with it are
  fixed: `deny` from a pre-reaper `Failed{Crashed}` ran W + commit only (residue carried into `Unapproved{Denied}`,
  reaper superseded forever) — now W-deny → T1 → T2 → T4 → T5-deny; declaration-gone from `Failed{*}` dropped the
  record without teardown (dead gen-N tools then advertised through §6.2a's fail-open) — now T1 → T2 → T4 then drop.
  **E-PRE is the first step of the shared load path** for every E0 entrant (boot, watcher trigger 2, `enable`,
  `reload`, `approve`-with-bit, config route), reaper-shaped (T1 → T2 → T4, cause `Crash`), and runs before any
  write to the supervisor's map; **§2.2's insert guard keys on a held handle** (`process`/`capability_provider_handle`
  is `Some`), not on state, so step 1 can no longer overwrite a live residue before E-PRE reads it. T4 states that
  it asks the map and removes the handle from it. Tests: crash → deny → approve (C3), crash → declaration-gone (C2).
  Cleanups: §3.7 step 3 names the held-handle window; §10 case 6 qualified; T0 has four non-route entrants (the
  reaper enters at T1); rev-10 log annotated. Hand-edited against the critic's exact text; the matrix was drafted
  first and disagreed with the text on exactly the cells the critic found.
- **rev 13** (2026-09-02) — round-11 critique (consistency lens: 3 blocking, 12 non-blocking; the four races
  (i)–(iv) confirmed correct under the prose). **The blockers were in rev 12's own additions:** (1) E-PRE was
  written "before E0", so a redundant `enable` on `Enabled` (or one queued behind a mutex hold) would have torn down
  the live handle and then CAS-failed — E-PRE now runs **after E0's CAS and before E1**, and `reload` from `Enabled`
  is stated as T0(Reload)–T4 then the load path whose E-PRE finds nothing; (2) four §3.3.1 cells had been copied
  from the pre-rev-12 draft and disagreed with the prose on T0/T3/cause; (3) several cells stated end states §4.1
  forbids (`Unapproved` × `disable`, `reload` from non-loaded states, the declaration-gone column without the
  mcp/plugin split, `Orphaned` re-declared). **§3.3.1 is regenerated row by row from §4.1 and the prose**, with
  `enable`/`reload`/`approve` as separate columns, declaration-gone split by kind, a re-declared column, and "holds"
  defined per kind (a plugin's `PluginState` entry persists; "holds" means a live `process`/provider handle). T5-deny
  and T5-gone from a pre-reaper `Failed{Crashed}` are plain stores, not CASes from `Disabling`. Cleanups: W-deny
  heading; §3.7 step-3 exit list; T0 entrants reworded (deny is a route); §2.2 guard outcome named; reaper treats an
  absent record as superseded; C3's crash-then-deny test asserts through the ledger (the event is C4's). Hand-edited.
- **rev 14** (2026-09-02) — round-12 critique (consistency lens: 1 blocking, the rest CONFIRMED — §3.3.1 derivation
  cell by cell against §4.1 and each cited rule; races (i)–(iv) under the table; E-FAIL; C1 builds alone; §3.7;
  one `is_satisfiable`; §12; §13; ~60 code anchors). **Residual sites** that still read the pre-rev-13 order
  "E-PRE, then E0" — §4.1 `Failed{*}` × `enable` and × `reload`, §3.4.1's `reload` definition, §9.2's Retry
  contract, the §3.3.1 `reload` cell, the §3.6 trace — now all read **E0 CAS → `Enabling` (generation bumps, one
  `begin` call) → E-PRE (T1→T2→T4, cause `Crash`) on any held handle → E1–E5**. §4.1 `Enabling` row: verbs queue
  behind the mutex and then take the outcome row (a queued `enable` landing on `Failed{*}` retries). Hand-edited
  against the critic's exact text and grep-verified — but the E-PRE definition's own closing sentence still read "then E0–E5"; rev 15 corrects it to "then E1–E5" and re-verifies: the only remaining "then E0" strings are this status line and the revision log.
- **rev 15** (2026-09-02) — round-13 targeted critique (1 blocking, 7 non-blocking; every other order site confirmed).
  **The blocker was one word:** E-PRE's own closing sentence read "Only then E0–E5 as written" — now "E1–E5" (E0's
  CAS has already run). Re-verified by grep: no live sentence pairs E-PRE with "then E0"; the remaining hits are the
  status line and this log. Wording: §2.2 says E-PRE is the first statement *after E0's CAS* on the load path; the
  §3.3 CAS-failure parenthetical is narrowed to `enable` (a queued `reload` runs T0–T4 then a load path whose E-PRE
  finds nothing; a queued `disable`/`deny` tears down); `approve` dropped from the two residue-exit lists (it never
  loads from `Failed`); §4.1 `Enabling` row queues `approve`/`deny` too; §8 `reload` names its `Failed{*}` shape and
  that a crash-residue E-PRE announces the crash regardless of outcome. Remaining non-blocking notes from rounds 12
  and 13 are preserved below for the implementer. **This is the last prose round: residual risk is now better
  burned down by C2/C3's integration tests than by further review.**
- **rev 16** (2026-09-05) — **implemented: C1–C8 landed** (`e92d76e..` on `feat/ui-rework`). C8 purged
  `execution.skill_defaults.global_tool_deny` exactly as §11.1 specifies — field, parameter (`extension_tool_defs`
  is now `(&self)`), all three surface sites, the nested-skill retain and both `denied_capabilities` folds, the
  tests, and both `tool_selection` comment blocks — leaving only the stale-key `WARN` probe in
  `load_daemon_config` (`daemon_config/mod.rs`), which parses to `toml::Value`, looks for the key and then
  `try_into::<DaemonConfig>()`. §13 Q5 (T1) stays **pending and unapplied**: this is a purge, not a migrate. The
  documentation sweep landed with it (CLAUDE.md, `docs/agent-loop.md`, `docs/Skill_Template_Reference.md` — the
  config-key row deleted, `docs/Daemon_Manual.md` — new Extensions section, `docs/GUI_Manual.md` — Extensions and
  Tools sections replacing Plugins/Skills, `docs/CLI_Manual.md` — `openalpaca ext …`, `config/mcp.toml`,
  `tasks/gui-api-requirements.md` reconciled with the 19-entry registry). **Corrections recorded from C1–C7, each
  at its section:** §6.2 #11 — `!is_satisfiable(&fm)` does not cover an `unknown` capability, so A0's
  `resolved.is_empty()` refusal is kept beside it on the plugin-skill arm (C5); §7.3's `state` comment now lists
  `enabling` and the residue paths that never enter `Disabling`; §3.4.1/§7.3 — the reload suppression has a
  mechanism, option (a): defer the publish to E5/E-FAIL, emit the outcome state and empty `affected_cron_skills`
  when it ended `Enabled` (C4); §7.3 — the cron notice's `<detail>` is wrapped as untrusted, widening §7.1's rule
  to this chat row; §4.2 — plugin `NeedsAuthorization` is reachable only via `PluginError::RpcError { data }`
  (R17) and MCP `Unauthorized` only for a 401/403 carrying `www-authenticate` (R14); §3.3 E2 — `enabled` is not
  in the fingerprint preimage (R15) and reload's `Reload` cause survives the `Disabling → Enabling` CAS (R16);
  §8 — `invocations_today` is one grouped query over the covering index, not a per-name seek.
- **Implementer residue observed while building C1–C8** (recorded in rev 16; none is a mechanism change):
  - `Extensions` (`AppState.extensions`, `managers/extensions.rs`) has **no `ledger` field** — it holds the two
    supervisors and the config handle, and the ledger is reached through `tool_registry.extensions()` (one
    ledger, owned by the registry). §6.2 #15's literal shape differs from what was built; the ownership claim it
    makes is unchanged.
  - The config route **pair**: an unknown `{kind}` is `404` (family consistency with the verb routes), `kind=mcp`
    is `409 unsupported_for_kind`, and `409 store_unreadable` is **unreachable there** — the config write path
    never reads `.permissions.toml`.
  - The **shutdown sweep budget is shared across both supervisors** and bounded by the daemon's force-exit
    window — `min(drain_timeout_secs + T4, window − reserve)` (C6) — not by the per-server drain, which remains
    the per-extension bound inside a supervisor.
  - The **Authorize affordance is a link, not a flow** (§9.2, C7): a `needs_authorization` row renders
    *"Authorize at &lt;url&gt;"* only when the row's `hint` carries one — for MCP that means the plugin or server
    supplied it, since the classifier itself recovers no URL — and the daemon never opens a browser or holds a
    callback. **Open-config** is likewise a form over the existing `POST …/config` route, one key per call, with
    no new route; setting the last missing key starts the plugin without a second call.
  - **`not_orphaned` needs more than its word** (C6, from the C3 review): the flat `{"error":"<word>"}` envelope
    is kept, and that one refusal adds a `message` field saying a plugin whose directory is present but no
    longer declares a `plugin.toml` becomes `Orphaned` only at the next daemon start. It is the single exception
    to "the word is the whole body", and it exists because the caller cannot otherwise know why a removable-looking
    row refuses.
- **Known non-blocking residue for the implementer** (round-12 and round-13 consistency critiques, preserved verbatim so it is not
  rediscovered; none changes the mechanism, and none is a decision an engineer should make alone — ask if unsure):
  - Reload's "cron notice is suppressed when the reload ends `Enabled` and fires normally if it ends `Failed{*}`" (§3.4.1 line 1028, §7.3 line 1959) has no mechanism: T1 step 3 runs — and emits `ExtensionCapabilityWithdrawn { cause: Reload, affected_cron_skills }` — before the outcome is known, and §7.3 step 2 has `NotificationDispatcher` post whenever `affected_cron_skills` is non-empty. State one of: (a) for cause `Reload` the supervisor defers the publish to E5/E-FAIL and empties `affected_cron_skills` when it ended `Enabled`; or (b) the dispatcher ignores cause `Reload` and the supervisor publishes a second event (cause `Reload`, state `Failed`) on a failed outcome. Either is a few lines; the doc should pick.
  - E-FAIL paragraph (line 899-900) and §3.3.1 invariant 2 (line 944) list "`approve`-with-bit via E-PRE" among the exits from a pre-reaper `Failed{Crashed}`, but §4.1 `Failed{*}`×`approve` and the §3.3.1 cell say approve from `Failed` re-records consent and never loads. Approve reaches E-PRE only from `Unapproved` (holds none). Drop it from both lists; the residue stays owned until the reaper, which is correct.
  - The `WithdrawalCause` for T1 step 3 on the two declaration-gone exits from a pre-reaper `Failed{Crashed}` (line 937, last two cells; line 900) is unstated — `deny` and E-PRE on the same residue say `Crash`; T5-gone from `Enabled` implies `DeclarationGone` ("disabled" wording). Pick one (Crash, for parity with the other residue exits) and say so; also extend §7.3's `state` field comment (line 1943) to cover `Failed{Crashed}` on the deny/declaration-gone residue paths, which never enter `Disabling`.
  - §3.3.1 `Enabling` sub-rows (lines 933-935) place the end state "→ `Failed{..}`" under the "declaration gone — mcp" column and "idem" under "declaration gone — plugin", which reads as a (sub-row × declaration-gone) cell. Add an explicit end-state note or an "outcome" column; add the missing E1-consent-fails sub-row (holds none — never built; → `Unapproved{..}`, bit stays true).
  - §3.3.1 `Orphaned` row (line 940) reads "—" for `enable`/`disable`/`approve`/`deny` where §4.1 says 409; write "409 (no handle)" so the two tables agree literally. Likewise the `Failed{*}` rows should carry a footnote for the two store-unreadable `Failed{ConfigInvalid}` rows (§4 line 1396-1402): every verb is `409 store_unreadable` with no transition, which the table does not carve out.
  - §8 `disable` (line 2164-2181) does not state that W is skipped when the bit is already `false`, while `enable` says "W is skipped — the bit is already `true`"; without the symmetric rule a redundant disable against a read-only file would `500` on a no-op, contradicting §3.3.1 `Disabled`×`disable` "CAS fail → 200 current". One clause.
  - The legal source states of the two CASes are never enumerated: `begin(ext, Enabling, None)` must accept {absent (boot), `Disabled`, `Failed{*}`, `Unapproved{*}` (approve path only), `Disabling` (reload path only, under the same mutex hold)}; `begin(ext, Disabling, cause)` accepts {`Enabled`, `Failed{*}`}. Reload's `Disabling → Enabling` CAS also implies T5's dedup-clear/emit do not run on that path. State the sets once beside the signature at line 902 so an implementer does not derive them from six §4.1 cells.
  - C3 verification cell (line 2614): "the T1-step-3 scan recorded with cause `Crash` (asserted through the ledger's withdrawal record in C3)" — the ledger has no structure that records a cause (`pending_cause` is written only by `begin(Disabling, cause)`, and the residue exits have no T0), and the scan itself lands in C4. In C3 assert the tombstones under the plugin's id plus no gen-N residue; assert the cause in C4 with the event.
  - §4.1 `Enabled`×declaration-gone-plugin (line 1432, note at 1445-1449) says its trigger is GAP-24's uninstall run in-process, but §12.1's uninstall is "T0–T5 if loaded → remove the permissions entry → trash the directory", which never produces `Orphaned`. Either mark the cell unreachable until a plugins-root watcher exists or align §12.1's uninstall sequence with the cell.
  - §3.6 item 3 (line 1123): "`PluginState.process` stays `Some` after `mark_failed` until the reaper's T4 or the next verb's E-PRE" is an incomplete exit list (disable's T4, deny's and declaration-gone's T1→T2→T4); §3.7 step 3 (line 1259) has the full list — cite it.
  - E-FAIL wording (line 892-893): "disconnect (...) then the closed seal" — per T4b the seal is stored inside `disconnect` before the lock is taken; reword to "`disconnect` (which sets the seal first, then `take()`s `changes_tx` ...)" so nobody implements a second seal store after disconnect.
  - T1 step 3 (line 477-481): "`Disabling` is entered by four different verbs and reads differently for each" then lists the reaper and §3.7's list change, which never enter `Disabling`; reword as "T1 runs under seven causes". Cosmetic, but it is the sentence an implementer keys the wording table on.
  - §8 `POST …/reload`: the one-line summary "T0–T4 then E0–E5 … From `Enabled` and `Failed{*}` only" describes the Enabled shape; from `Failed{*}` there is no T0 and no T3 drain (E0 CAS → E-PRE → E1–E5 per §3.4.1, which the sentence cites). It does not state the old order, but a parenthetical "(from `Failed{*}`: E0 CAS → E-PRE on any held handle → E1–E5, §3.4.1)" would close the last summary that names only T0–T4/E0–E5. Related: §8 reload's "It never fires the cron notice unless it ends `Failed`" and §3.4.1's suppression clause describe the Reload-cause scan; a reload (or enable) from a pre-reaper `Failed{Crashed}` runs E-PRE's T1 step 3 with cause `Crash`, whose crash notice fires on total loss regardless of how the reload ends — arguably correct (it is the crash's one announcement), but §8 should say so.
  - §3.6 trace (line 1157, a rev-14 edit): "(one `begin` call, §4.1)" — the one-begin-call / generation-bump rule is stated in §3.0 rule 1 and §3.3 E0; §4.1's Failed{*} cell only says "generation bumps". Citation nit, no contradiction.
  - §3.6 (line 1176): "Retry is `POST …/enable`, which re-runs W, E0–E5 from `Failed`" vs §8 config route "invokes the `enable` verb (W is skipped — the bit is already `true`)": whether W runs as an idempotent rewrite or is skipped when the bit already matches is worded two ways. Not order-related, pre-rev-14.
