...
**Status:** design of record, **rev 7** (reconciliation pass: round-5 critique + the repaired Claude Code lessons, rev 4), accepted mechanism. **Residue for the next pass:** none blocking in this file; the owner-gated items are enumerated in §13 (Q5–Q14) and are **not applied anywhere** in this document — §13's arrows are recommendations, and silence enacts none of them. **Cross-file follow-ups still owed (not this file):** api-fix-plan Phase 0 has no slot for the adopted C-3/P-3 main-loop cost-lockout fix (an **A5** item, its *Verify* clause and the roadmap's "Scheduled:" pointer are owed there — the plan's §0 N4 row says "adopted", the roadmap's Known-broken row still says "suspected", ADR-028 says "confirmed"); the GUI `GapId` union (`unavailable.ts:37`/`:233` still read `GAP-19`, C7 work); and the small plan/KB wording items listed in the rev-7 log entry. The plan-side N5/Phase 8/ADR-030 items rev 6 listed as owed **landed** with plan rev 3 and ADR-030 (`94a8f42`). Supersedes ADR-029 (flat per-tool disable list) entirely.
...
2. `skill_catalog.remove(skill_id)` per contributed skill (`manager.rs:547`; the `for` is `:546`);
3. `agent_registry.remove_template(agent_id)` per contributed template (`manager.rs:555`; the `for` is `:554`);
...
T0 without waiting for the fetch (the mutex is free), then T1–T5 normally.
