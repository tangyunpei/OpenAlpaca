...
4. **connector + provider.** `unload_plugin` carries a NOTE (`manager.rs:560-562`) that *"connector
...
3. **Mutex + re-check, then apply.** Take the per-extension mutex; proceed only if the record reads `Enabled`
   **and** its generation equals the message's `g`. Anything else — `Disabling`, `Disabled`, `Failed{*}`, or a
   ...
   server's client was sealed at T4 (or never built), its receiver is gone with it, and a straggler queued
...
**Rule for every commit here:** a commit that adds a `SystemEvent` variant adds its `event_bridge.rs:30` arm in the *same* commit (a `debug!` no-op is enough) — that match is exhaustive by design, so a deferred arm breaks `cargo build --workspace` for the intermediate commit. **Sequencing:** C1 alone first (additive, zero behaviour change, and its three gate tests are the property the
