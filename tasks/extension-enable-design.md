...
- **Identity: the directory name is the extension id (rev 6 decision — X-3).** Today `.permissions.toml`
  ...
  **scan order is pinned**: name check → consent (`is_approved`) → bit, so the name check runs before the
  ...
  whose state is not `Disabled`/`Failed`/`Unapproved`
...
The five non-route entrants of this sequence — watcher, declaration-gone, reaper, deny (which writes consent at W-deny, never the bit) and reload (no W) — never write the bit here: on the **watcher** path the write *is* the trigger
...
`Failed` records that got there through E-FAIL or the reaper, own none. Ledger signature, stated
...
**E-PRE — TEAR DOWN BEFORE YOU BUILD.** Every verb that builds a new handle — `enable` and `reload` from
`Failed{*}` (§3.4, §3.4.1, §4.1) and the config-route enable (§8) — runs **T1–T4 on any handle the supervisor's
map still holds for the extension, under the same mutex hold, before E0**: for MCP `disconnect` + seal (which
...
3. **Mutex + re-check, then apply.** Take the per-extension mutex; proceed only if the record reads `Enabled`
   **and** its generation equals the message's `g`. Anything else — `Disabling`, `Disabled`, `Failed{*}`, or a
   ...
   server's client was sealed at T4, at its failed bring-up (E-FAIL), or never built, its receiver is gone with it, and a straggler queued
   before T4 fails the state check under the mutex; a `mark_failed` that landed between step 2 and this step
...
| `Failed{*}` (bit=true) | → `Enabling` (retry: E-PRE T1–T4 on any held handle, then E0–E5) | → `Disabling` | → `Enabling` (as `enable`: E-PRE, then E0–E5) | stays `Failed`; consent re-recorded, **never a load** — Retry is `enable` | → `Unapproved{Denied}` | — | — | — | — | → `Orphaned` | record dropped |
...
- **rev 10** (2026-09-02) — round-8 critique (consistency lens: 1 blocking, 8 non-blocking; plan/KB lens: ready).
  **S2 hole closed (E-FAIL, §3.3):** any exit from E2–E4 after `connect`/`spawn` returned runs T4 + T4b on the
  ...
  handle map now holds entries only for `Enabling`/`Enabled`; E4b's opening widened; §3.7's "sealed at T4 (or never
