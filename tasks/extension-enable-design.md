...
The four non-route entrants of this sequence — watcher, declaration-gone, deny (which writes consent at W-deny, never the bit) and reload (no W) — never write the bit here; the reaper never takes T0 at all (`mark_failed` already set the state) and enters at T1: on the **watcher** path the write *is* the trigger
...
T5-deny** (§3.3.1), so the crash's T1-step-3 scan fires and nothing is carried into `Unapproved{Denied}`.
...
**E-PRE — TEAR DOWN BEFORE YOU BUILD.** E-PRE is not a property of named verbs but **the first step of the shared
...
holds for the extension, under the same mutex hold, before E0 and before any write to the supervisor's map** (plugins:
before the step-1 insert at `manager.rs:263`, §2.2; MCP: it takes and tears down the existing `Arc<McpClient>` before
`connect` builds the new one): for MCP `disconnect` + seal (which
...
### 3.3.1 Handle ownership matrix (derived from T0–T5, E0–E5, E-FAIL, E-PRE, §3.6, §4.1)

One handle per extension at most: the MCP `Arc<McpClient>` (with its §3.7 reader task) or the plugin `PluginProcess` + `capability_provider_handle`. "Holds" means the supervisor's map has an entry for this extension. Every cell is derived from a named rule; if a future edit contradicts a cell, the edit is wrong or the table is — never silently both.

| State (bit) | Holds? | `enable` / `reload` | `disable` | `deny` (plugin) | crash (`mark_failed`) | reaper (gen matches) | watcher / declaration-gone |
|---|---|---|---|---|---|---|---|
| `Disabled` (false) | **none** | E-PRE finds none → E0–E5 → `Enabled` (holds one) | CAS fail → `200` with the current record (§4.1; symmetric with enable-on-Enabled, §3.3) | W-deny write → T0–T4 find none → `Unapproved{Denied}` | n/a (nothing running) | n/a | bit flip → as `enable`; gone → record dropped |
| `Unapproved{*}` | **none** (never loaded) | `enable` sets the bit, **never loads** (§4.1); `approve` → `Enabling` | → `Disabled` (bit false) | re-recorded, no load | n/a | n/a | gone → dropped |
| `Enabling` (transient, mutex held) | **one from E2 onward** | n/a (mutex) | n/a (queued behind the mutex; sees the outcome state) | n/a | n/a | n/a | n/a |
| ↳ E2 `connect`/`spawn` fails | none (never built) | — | — | — | — | — | → `Failed{..}` (E2 clause: nothing to unwind) |
| ↳ E3/E4 fail after connect | one → **E-FAIL tears it down** → none | — | — | — | — | — | → `Failed{..}` |
| ↳ E4b partial registration | one → unwind published + **E-FAIL** → none | — | — | — | — | — | → `Failed{..}` |
| `Enabled` (true) | **one** | `reload`: T1–T4 on it (E-PRE) → E0–E5 (new one). `enable`: CAS fails → 200, no reload | T0–T4 tear it down → T5 `Disabled` → none | W-deny → T0–T4 → `Unapproved{Denied}` → none | bare CAS → `Failed{Crashed}` **still holds one** (residue) | n/a (state ≠ Failed) | fingerprint change: Q9 (owner-gated; as written → no auto-reload); gone → T0–T4, dropped |
| `Failed{Crashed}` **pre-reaper** (true) | **one (residue)** | **E-PRE T1–T4 on the residue** → E0–E5 → `Enabled` (one). Reaper then finds gen ≠ / state ≠ → superseded no-op | T0–T4 (T4: "tear down the handle if the map holds one") → `Disabled` → none | W-deny → T0–T4 → `Unapproved{Denied}` → none | n/a (already Failed) | T1→T2→T4 → **none**, stays `Failed{Crashed}` (never T5) | gone → T0–T4, dropped |
| `Failed{*}` **post-reaper / via E-FAIL / via E2-fail** (true) | **none** | E-PRE finds none → E0–E5 | T0–T4 find none → `Disabled` (bit written) | W-deny → `Unapproved{Denied}` | n/a | n/a (nothing queued, or superseded) | gone → dropped |
| `Disabling` (transient) | one until T4, then none | n/a | n/a | n/a | n/a | n/a | n/a |
| `Orphaned` (plugin, declaration gone) | **none** | n/a | n/a | n/a | n/a | n/a | re-declared → `Unapproved{NeverSeen}` |

**Invariants the table proves, and the rule each rests on:**
- *At most one handle per extension* — every "builds" cell is preceded by an E-PRE or E-FAIL teardown of anything held (E-PRE; E-FAIL).
- *A `Failed{Crashed}` record owns its handle until the reaper's T4 or the next verb's E-PRE, whichever takes the mutex first* — the only state with a residue; both exits tear it down under the mutex (§3.6; E-PRE).
- *`disable` is total* — T4 tears down "if the map holds one", so it is correct from `Enabled`, from pre-reaper `Failed{Crashed}`, and a no-op from every "none" state (§3.2 T4).
- *The reaper never writes state and never tears down a newer load* — it acts only on `Failed{Crashed}` at its own generation; after an E-PRE it finds nothing (§3.6).
- *No path leaves a live child under a "none" cell* — E-FAIL covers bring-up failures after connect; the reaper or E-PRE covers crashes; T4 covers every disable/deny/gone (S2).
