...
- **Identity: the directory name is the extension id (rev 6 decision — X-3).** Today `.permissions.toml`
  ...
  **scan order is pinned**: name check → consent (`is_approved`) → bit, so the name check runs before the
  ...
  and E-PRE (§3.3) runs **before** this insert, as the first statement of the load path for an id that already has an
...
`Failed{Crashed}` other than the reaper's own pass tears the residue down first** — `enable`/`reload`/`approve`-with-bit
via E-PRE, `disable` via T4 (which asks the map, not the state), `deny` via W-deny → T1 → T2 → T4 → T5-deny, declaration-gone
...
holds for the extension, under the same mutex hold, **after E0's CAS and before E1** — a CAS failure (`enable` on `Enabled`, or a verb queued behind an `Enabling`/`Disabling` hold that lands on `Enabled`) returns `200`/`409` with nothing touched and never reaches E-PRE — and before any write to the supervisor's map** (plugins:
...
`shutdown`/`kill` + the 2 s `wait`. Only then E0–E5 as written. Without it, a Retry that wins the mutex before the
...
- *A pre-reaper `Failed{Crashed}` owns its handle until whichever takes the mutex first: the reaper's T4, E-PRE (via `enable`/`reload`/`approve`-with-bit), `disable`'s T4, `deny`'s T1→T2→T4, or declaration-gone's T1→T2→T4* — the only state with a residue; every exit tears it down under the mutex (§3.3 rule beside the invariant).
...
| `Enabling` (bit=true) | queued after commit; then the outcome row (a queued `enable` that lands on `Failed{*}` retries — a second, harmless bring-up) | queued after commit | queued after commit | — | — | → `Enabled` | → `Unapproved{NeverSeen\|Denied\|CapabilitiesGrew}` (bit stays true) | → `Failed{reason}` | `mark_failed` is a no-op here; E2/E3 error → `Failed{reason}` | — | — |
...
- **rev 14** (2026-09-02) — round-12 critique (consistency lens: 1 blocking, the rest CONFIRMED — §3.3.1 derivation
  ...
  against the critic's exact text and grep-verified: no site pairs "E-PRE" with "then E0" any more.
