...
nothing while the child outlived the switch: an S2 hole. **The supervisor's client/process map holds an entry only
while the record is `Enabling` or `Enabled`**; `Failed` and `Disabled` records own no handle. Ledger signature, stated
...
the per-server reader task is spawned there too; its first message is **acted on** only after E5, because
...
of `list_tools` (`client.rs:239-244`: lock the service, `list_all_tools`) under `request_timeout`, returning the
