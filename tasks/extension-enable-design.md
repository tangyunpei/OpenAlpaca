...
| `Failed{Crashed}` **pre-reaper** (true) | **one (residue)** | E0 CAS → `Enabling` → **E-PRE: T1→T2→T4 (cause `Crash`) on the residue** → E1–E5 → `Enabled` (one); reaper then finds gen/state changed → superseded no-op | same as `enable` (§3.4.1: from `Failed{*}`, E-PRE then E0–E5) | T0–T4 — **T4 asks the map**, tears the residue down → `Disabled` → none | consent re-recorded; no load | W-deny → **T1→T2→T4 (cause `Crash`)** → T5-deny `Unapproved{Denied}` (plain store, §3.2) → none | n/a (already Failed) | **T1→T2→T4** → none; stays `Failed{Crashed}` (never T5) | **T1→T2→T4 (no write)** → record dropped (plain store; reaper then finds no record → superseded) | **T1→T2→T4** → `Orphaned`, entry kept | — |
...
from `Enabled` (the ordinary case) and from `Failed{*}` (where, like `enable`, it first runs T1–T4 on any held handle — E-PRE — then E0–E5; one verb for the
...
clicks Retry; `enable` takes B's mutex first (`Failed{*}` + `enable` → `Enabling`, §4.1), **runs T1–T4 on load N's
still-held handle (E-PRE)**, bumps the generation, publishes load N+1 and commits `Enabled`. If the reaper then dequeued B and ran T1→T2→T4
...
| `Failed{*}` (bit=true) | → `Enabling` (retry: E-PRE T1→T2→T4 on any held handle, then E0–E5) | → `Disabling` (T4 tears down a held handle, else no-op) | → `Enabling` (as `enable`: E-PRE, then E0–E5) | stays `Failed`; consent re-recorded, **never a load** — Retry is `enable` | → T1→T2→T4 if a handle is held (pre-reaper `Failed{Crashed}`), then `Unapproved{Denied}` | — | — | — | — | → T1→T2→T4 if held, then `Orphaned` | → T1→T2→T4 (no write) if held, then record dropped |
...
| `failed` / `unreachable` \| `crashed` | Toggle **ON**, live | `crashed` | **`warn`** | Retry (= `reload`, §3.4.1 — as `enable` from `Failed`: E-PRE T1–T4 on any held handle, then E0–E5) | the transport error (rendered from `connection_state()`, X-5) + `since`; `detail` shown as a quoted diagnostic, never interpreted |
