# Daemon Fix Plan — rev 3.9: single root, artifact store, sessions, the 23 GUI gaps

**Status:** **implemented and merged — rev 3.9 is the record of what shipped.** Phases 0–8 all landed on `feat/ui-rework` (`2656a55..ab31299`; the per-phase ranges and their deviations are on the `### Phase N` headings in §6), and the extension design's C1–C8 landed with them between Phases 1 and 2 (`5c105b8..97740a5`; `tasks/extension-enable-design.md` rev 16). D1–D5 and N1–N5 stand as resolved and N5 was built from the design, never from the N5 table. Migration head is **042** (`042_unattended_declaration.sql`, round 3 of the local-model readiness wave — landed outside this plan, whose own last migration is §11's **041**, ruling R80). Exactly one planned item was deliberately never built: Phase 0 item 6 (GAP-22), left alone under owner decision T14 and made moot by C7, which deleted the six `plugin_*` event variants it would have amended. The **owner decisions are still pending — T1–T22, T24–T27 and T31–T33 in §0 below (twins at design §13 Q5–Q14) are recommendations, decided nowhere and applied nowhere**; T23, T28, T29 and T30 were taken by the owner on 2026-09-22 and are recorded, with the commit that applied each, under §0's *Owner decisions taken*; they and §12's recorded residue are what remains. `feat/ui-rework` itself is **merged**: PR #31 landed on `main` as `adf492cb` on 2026-09-20 and the branch is deleted, so every range in this document lies inside that commit's history. · **Date:** 2026-09-20 · **Branch:** merged to `main` (written on `feat/ui-rework`)
**Rev 3.9 changelog (one line), 2026-09-22:** the simplification wave on `chore/deps-upgrade-and-simplify` (the legacy mover retired, `config reset --factory` made reachable, the daemon's stop and restart fixed, the app's Stop/Start daemon control — owner decisions D-A…D-E) landed outside this plan and adds nothing to its phases; §0 gains owner decisions **T23 and T25–T33** — T25–T27 and T31–T33 as pending rows beside T24, and T23, T28, T29, T30 in a new *Owner decisions taken* subsection, because the owner adopted T23 and T28 and the commits that built T29 and T30 applied them. §2's mover and §9's live-DB root-move risk row describe code that no longer exists (`store/migrate.rs` was deleted at `b315a38a`); they are left as the record of what Phase 1 shipped, not rewritten.
**Rev 3.8 changelog (one line), 2026-09-22:** the GUI first-run wave (owner decisions D-F, D-G — first-run card, `Add key` form, key rows; GUI-only, on `chore/deps-upgrade-and-simplify`) landed outside this plan and adds nothing to its phases — the only plan-side change is §0, which gains owner decision **T24** (offer credentials discovered from Claude Code / Codex on the GUI's first-run surface), unadopted. It is numbered T24 because T23 is reserved for the daemon-lifecycle track's `busy` block on `GET /v1/status`, which records it in its own commit. No item text, gap row, phase or argument was rewritten.
**Rev 3.7 changelog (one line), 2026-09-20:** status only — `feat/ui-rework` merged to `main` as `adf492cb` (PR #31) and was deleted, so "merging this branch" leaves the list of what remains and the status line says where the ranges now live; rounds 5–15 of the local-model wave and the PR #31 review follow-ups landed outside this plan and add no plan-side row (`CLAUDE.md`'s Current Work section is their record). No item text, gap row, phase, decision or argument was rewritten.
**Rev 3.6 changelog (one line), 2026-09-18:** round 4 of the local-model readiness wave (rulings V1–V12, from a live acceptance run on a local Ollama) adds one plan-side row — §0 gains owner decision **T22** (`GET /v1/chat/confirmations` is an unscoped read that carries full `tool_arguments`, a category of data no unscoped surface carried before), unadopted like the rest — and corrects the status line's migration head from 041 to **042**, which round 3 landed outside this plan (§11's 041 row says so too). No item text, gap row, phase or argument was rewritten.
**Rev 3.5 changelog (one line), 2026-09-18:** the local-model readiness wave (rulings L1–L14, `65314af..f17655f`; the spec lived in a session scratchpad and is not tracked — the commits and `CLAUDE.md`'s Current Work section are the record) landed outside this plan and adds nothing to its phases — the only plan-side change is §0, which gains owner decisions **T20** (auto-enable a reachable Ollama on first boot when no cloud key exists) and **T21** (the 24 h discovery-token expiry the CLI enforces client-side and the daemon does not), both unadopted; the manuals and `CLAUDE.md` carry the shipped behaviour. T17's `docs/CLI_Manual.md` citation is re-pointed to `:322` — the `llm` and `chat` notes added to that manual in the same wave moved the line. No item text, gap row, phase or argument was rewritten.
**Rev 3.4 changelog (one line), 2026-09-12:** the whole-branch review's fix wave is folded in — §11 gains the **041** row (`041_execution_log_timestamp_indexes.sql`, ruling R80, with the `INDEXED BY` note and why `subagent_span` is left alone) and the head line moves there from 040; §0 gains owner decisions **T18** (a CLI turn from another project switches the shared `{owner}:gui` lane's session under R48) and **T19** (`[web_search] api_key` in the clear in `llm.toml` and its backups), both unadopted; new **§12 records the residue** the review refuted — at-most-once steering as §5.6(b)/(c) specify it, the `file_write` final-component symlink, `ArtifactStore::verify` having no boot caller by design, and `set_plugin_secret` waiting on T9 (ruling R86); T17's citation is corrected to `install.rs:686` and the evidence appendices are named as **tracked**, which they are. No item text, gap row or argument was rewritten.
...
**Evidence appendices (tracked under `tasks/research/`, whitelisted out of `.gitignore`'s `*.md` blanket ignore by its `# UI rework reference docs` block):**
...
Raised by `tasks/research/claude-code-design-lessons.md` §6 against rules that are settled. Each carries the lessons' recommended default; **the default is a recommendation, not a decision, and this plan is written as if the answer were "no change" until the owner says otherwise.** Design-side twins (T1, T3, T4, T6(a), T6(c), T7, T8, T9, T14, T15) are also listed as `tasks/extension-enable-design.md` §13 Q5–Q14 and are unapplied there too. **T16–T22 were raised during execution**, not by the lessons — T16 and T17 by ruling R40 and the uninstall verb of Phase 8 item 9, T18 and T19 by the whole-branch review (ruling R86: a finding refuted as owner-decision territory is recorded, never adopted), T20 and T21 by the local-model readiness wave (rulings L1–L14), whose spec recorded both as decisions rather than shipping them, and T22 by that wave's round 4 (ruling V10) against the route round 3 added. **T24** was raised by the GUI first-run track (D-F/D-G), which wired the key routes but deliberately not the credential-discovery pair; it is T24 rather than T23 because T23 belongs to the daemon-lifecycle track. **T25–T27 and T31–T33** were raised by the simplification wave's integrated plan (the factory-reset, legacy-mover and daemon-lifecycle tracks) and shipped their no-change answer; that wave's T23, T28, T29 and T30 are not in this table because they are decided — see *Owner decisions taken* below it. Their recommendation column carries the recommendation recorded with the ruling, and like every other row it is **not adopted** — the shipped code is the "no change" answer.
...
| **T17** (Phase 8 item 9, raised in execution) | A retention policy for `plugins/.trash/<name>-<ts>/`, where uninstall moves a plugin directory (`crates/openalpaca_plugins/src/install.rs:686`, `fn trash`)? Nothing purges it today. | **Not adopted.** Recommendation: no auto-purge by default; an age sweep or a `store purge`-style verb is the owner's call | Either the boot retention sweep (§5.4) gains a `.trash/` age/size pass or `openalpaca store purge` (Phase 8 item 15) grows a trash target; uninstall keeps moving rather than deleting either way (§1.3 rule 3). If **no**: `.trash/` grows until the owner empties it by hand — `docs/CLI_Manual.md:322` is where that is stated for the uninstall verb. |
| **T18** (whole-branch review, raised in execution) | The CLI and the GUI share one lane (`{owner}:gui`), and every CLI turn carries its own working directory as the turn's project (`apps/openalpaca/src/chat_stream/mod.rs:73`, `ChatTarget`'s doc). Under R48 — "changing project = new session" — a one-shot `openalpaca chat` from another checkout therefore archives the conversation the GUI window is sitting in and opens a new one. R48 weighed one client; two share the lane. | **Not adopted.** Recommendation: keep the behaviour and document it — one lane holds one live conversation, and the project is the turn's, whichever client sent it. The alternative, if the archiving proves disruptive in daily use, is to pin the GUI's session on its own turns (the window already names `session_id` on an explicit resume, R49) rather than to split the lane | A `{owner}:cli` lane of its own, or an explicit `--project` on the CLI that defaults to "the lane's current session's project" instead of the cwd, plus the manual sentence either way. If **no**: R48 stands as landed; `docs/CLI_Manual.md`'s `chat` notes already state that a resumed conversation's own project governs the turn, which is the half that keeps `--resume` and `--session` safe. |
| **T19** (whole-branch review, raised in execution; beside T9) | `[web_search] api_key` is written into `config/llm.toml` as a plaintext `String` (`PUT /v1/daemon/config/providers/web-search`) and rotates from there into the five copies under `state/backups/`. The field and the plain write predate this branch; what the branch added is the rotating writer that copies it. R65/R65a's "the daemon never writes a secret into config" rule is scoped to `POST /v1/extensions/mcp` and `mcp.toml` and does not reach it. | **Not adopted.** Recommendation: bring it under the same secret resolution order the LLM providers use — `api_key_env` / `secret_ref` / `secret_encrypted`, in that order — so the daemon stores a reference rather than the value, exactly as `env_from` does for MCP | The route takes `api_key_env` (and refuses a literal `api_key`, `secret_literal_refused`, the R65 text), `LlmConfig`'s field becomes the reference, and one migration pass moves an existing literal into the chosen store. If **no**: the value stays in the clear inside the 0700 `state/` beside `.master_key`, nothing exports it, and rotating a leaked key means editing `llm.toml` and its backups. Decide with T9, which picks the store. |
| **T20** (local-model readiness, ruling L4) | Should a first boot **enable** `[providers.ollama]` by itself when Ollama answers on `base_url` and no provider holds a key? The seeded template ships `enabled = false` (`scripts/release/templates/config/llm.toml`), so a machine with a running Ollama, no cloud key and no hand edit boots with nothing routable: the first turn fails with `NoRoutableModel` until the owner flips one bit in the GUI or the file. Everything downstream of that bit already needs nothing — no key, no `[models]` row, discovery does the rest (L1, L2). | **Not adopted.** Recommendation: keep `enabled = false`. Enabling a provider is consent, and a daemon that probes a local port on first boot and starts routing to whatever it finds there has decided for the owner which model answers their mail. The cost of the current answer is one documented action; the cost of the other is a surprise. | `seed_default_configs` (or a first-boot probe beside it) writes `enabled = true` when `GET {base_url}/api/tags` answers inside a short timeout and no provider holds a key, with a log line naming what it turned on, and a rule that it never re-enables after an owner disable (otherwise the toggle does not stick across boots). If **no**: the shipped `false` stands and the one action is written down — `docs/Installation_Manual.md` → Local Models (Ollama), `docs/QuickStart_Manual.md` §4, and the `NoRoutableModel` text itself. |
| **T21** (local-model readiness, raised in execution) | `discovery.json`'s `auth.expires_at` is `issued_at + 24 h` (`crates/openalpaca_storage/src/discovery/mod.rs:237`) and **only the clients enforce it**: the CLI refuses every call past it (`apps/openalpaca/src/client.rs:24`, `ensure_not_expired`), while the daemon's `auth_middleware` compares the token string and nothing else (`apps/openalpacad/src/middleware.rs:28`). A daemon up for more than a day is therefore reachable by the GUI — whose `ensure_daemon_running` deliberately prefers liveness over the file's deadline (`apps/openalpaca-gui/src-tauri/src/lib.rs:38`) — and unreachable by the CLI until `openalpaca daemon restart` re-issues the file. Should the expiry be the daemon's, refreshed while it runs, or dropped? | **Not adopted.** Recommendation: make the token's life the daemon's — rotate `discovery.json` (token and `expires_at`) on a schedule while the process is up, so client and server agree. A deadline only the client honours is a false refusal against a daemon that would still have answered, and no protection at all against anything that reads the file. | Either the daemon re-issues `discovery.json` periodically (and the clients keep their check, which then never fires against a live daemon), or `ensure_not_expired` moves into `auth_middleware` and the clients stop second-guessing the file. Both change what a long-running daemon hands out, so pick one. If **no**: the asymmetry stands and is documented — a CLI that says "Discovery token has expired" after a day means `openalpaca daemon restart`, which is what `docs/CLI_Manual.md`'s Troubleshooting already says. |
| **T22** (local-model readiness round 4, ruling V10) | `GET /v1/chat/confirmations` (S9, `apps/openalpacad/src/routes/chat.rs:784`) is an **unscoped** read — R40's shape, the same line `GET /v1/tasks` draws: seeing that something is waiting is not acting on it, and answering stays owner-scoped at `POST /v1/chat/confirmations/{request_id}`. What is new is *what* it carries: the row copies the broker's `tool_arguments` verbatim (`chat.rs:813`), and no unscoped surface carried a tool's arguments before — the persisted `tool_confirmation_requested` row deliberately holds `request_id`, `agent_id`, `tool_name` and `task_id` and nothing else (`apps/openalpacad/src/events/persistence.rs:289-303`). Downstream, `openalpaca tasks confirmations list --format json` hands the whole row on, arguments included (`apps/openalpaca/src/commands/confirmations.rs:91-103`; the table's four columns do not print them, `:112-136`), and the GUI's lane filter is client-side (`confirmationBelongsHere`). A prompt's arguments are the file being written, the command being run, the message being sent. Should the pending list be owner-scoped, or should it redact `tool_arguments` for rows the caller does not own? | **Not adopted.** Recommendation: scope it, and decide it **with T16** — same question, same reason, and answering them apart would leave the task routes unscoped while the route that reveals what those tasks are about is scoped, or the reverse. Until then the behaviour is documented rather than changed: a machine with one owner (every machine today) cannot tell the difference, and the live WS frame has always carried the arguments to every connected client anyway (`ServerEvent::ToolConfirmationRequested`, `/v1/events` is unfiltered), so scoping this route alone would close a window while the door stands open. | `list_confirmations` takes `state.local_user_id` and drops — or redacts `tool_arguments` on — every row whose `lane_key` the caller does not own; a workflow prompt carries `lane_key: null` (`SandboxPolicy` holds a lane only on the main-loop policy), so the scope has to resolve ownership through the row's `task_id` as well, or those rows become unanswerable from anywhere. Redaction is the lighter half and keeps the list complete. Either way the GUI's client-side filter stays (it is about *which window*, not which owner), and `/v1/events` needs the same treatment or the change buys nothing. If **no**: the read stays unscoped and says so in its own comment, `docs/CLI_Manual.md`'s `tasks confirmations` section, and `docs/Daemon_Manual.md`'s route list. |
| **T24** (GUI first-run track, D-F/D-G; renumbered from that spec's T23) | `GET /v1/settings/llm/credentials` and `POST /v1/settings/llm/credentials/rescan` exist, `lib/api/settings.ts` wraps both and `useDiscoveredCredentials` / `useRescanCredentials` exist — and nothing calls them. What they find (`crates/openalpaca_llm/src/keys/credential_discovery/mod.rs`) is an OAuth token in `~/.claude/.credentials.json` (provider `anthropic`, source `claude_code`), on macOS the Keychain item `Claude Code-credentials` when that file is missing, and `~/.codex/auth.json` (`openai`, `codex`); the route returns metadata only (`source`, `provider`, `status`, `expires_at`, `auto_refresh`), the token stays in the daemon's `TokenManager`, which can refresh it in the background. It reads **no** environment variables. Should the GUI's first-run surface (the chat card, or the `Add key` form) offer those credentials? | **Not adopted.** Recommendation: **not offered.** Surfacing it would have the GUI state on screen that this machine is signed in to another product, with that account's token expiry, inferred from files the user never pointed OpenAlpaca at; adopting one would use a credential minted for a different product, under its terms, with background refresh, against a quota the user is not watching here. Its own design with its own consent step, not a first-run path whose job is one answering model. | Offered **opt-in per source**: a separate section (not the first-run card) lists what `GET …/credentials` found, each with an explicit adopt action and a row that says where the credential came from and what happens when its origin product rotates it; `…/rescan` becomes a button. An environment-variable offer (`ANTHROPIC_API_KEY`) would be **new daemon surface** — a new scan and a decision about reading the launcher's environment — not wiring. If **no**: the two routes stay uncalled, and `API_MAP.md` §2.4 says so. |
| **T25** (daemon-lifecycle track, plan row 19) | `Stop daemon`'s confirm button: a new filled `dangerSm` variant built from §1.2's `#8C2F1E` / `#F7E7DE` / `#E9D3C6`, or the existing `dangerGhost`? | **Not adopted.** Recommendation: reuse `dangerGhost` — it exists in `components/ui/Button.tsx` and in DESIGN_SPEC §3.35 ("Danger ghost"); a filled danger treatment the design never drew is a design change, and the dialog's gate is already the busy line plus a `Cancel` that holds the default focus. Shipped that way (`1d08365d`); §3.35's row names `Stop daemon` as its second user. | A `dangerSm` variant in `Button.tsx` and a §3.35 row — exactly one, shared with any later danger control. If **no**: as shipped. |
| **T26** (factory-reset track, D-B) | Should `openalpaca config reset --factory` ever take a `--yes` / `--force`? | **Not adopted.** Recommendation: never, for now. A verb whose only safety is a typed word (`factory-reset`, D-B) must not grow a way to skip it in the change that made it reachable. The consequence to accept: a GUI "repair" button or an installer step **cannot** script this verb — such a surface gets its own route, not a flag. Shipped with no flag (`73a0698d`). | A flag that skips the prompt on a non-terminal caller, plus a rule for what it prints instead. If **no**: as shipped; `docs/CLI_Manual.md` says the verb asks at a terminal. |
| **T27** (factory-reset track) | Should `openalpaca config list` degrade to a partial listing when the database is refused (the legacy-schema guard)? | **Not adopted.** Recommendation: keep the refusal, which now names the rescue verb. `list` is the inventory verb; silently omitting the database third of it would be a lie, and a partial listing is a wider job than making the rescue reachable. Shipped as the refusal (`73a0698d`). | `list` prints the two TOML halves and one line saying the database half is unavailable and why. If **no**: as shipped. |
| **T31** (legacy-mover track, D-D) | After the pre-D2 upload re-home was retired (`f307df7e`), nothing owns `state/assets/` on a machine that had pre-D2 uploads — no writer, no sweeper, no warning — and those rows address it by absolute path for ever. | **Not adopted.** Recommendation: leave it unswept and say so, the stance W6 took for `event_log`: whether a directory needs a retention pass is nobody's default to adopt. No sweeper is built. | A boot or `store purge` pass over `state/assets/` that deletes a blob no row references. If **no**: the directory stays until the owner removes it; the rows still resolve. |
| **T32** (daemon-lifecycle track) | Should the daemon post a farewell to connector lanes (Telegram / Discord / iMessage) before it shuts down? | **Not adopted.** Recommendation: silence, said honestly in the GUI's stop dialog ("…get no reply and no error until you start it again — there is no farewell message"). A farewell would spend the 10 s force-exit window that already bounds the MCP/plugin sweep, and it could only be sent on the graceful path — absent exactly when a user most needs it. Shipped as silence (`1d08365d`). | A best-effort broadcast to every live connector lane at the head of the shutdown tail, bounded inside the window. If **no**: as shipped. |
| **T33** (daemon-lifecycle track) | Should the GUI's `Stop daemon` require a typed word, for parity with D-B's `factory-reset`? | **Not adopted.** Recommendation: no. D-B's typed word is justified by irrecoverable data loss; a stop destroys a process and whatever turn was in flight, not data. Shipped with a confirmation dialog and no word (`1d08365d`). | A `TypeToConfirm` sub-component in the dialog with the word `stop`. If **no**: as shipped. |

### Owner decisions taken — `chore/deps-upgrade-and-simplify`, 2026-09-22

The owner settled the simplification wave's scope as **D-A…D-G** (D-A a factory reset deletes the database file and its `-wal`/`-shm`; D-B its confirmation names the store root and what survives, and takes a typed word; D-C it refuses while a daemon runs; D-D the legacy disk-layout movers are retired for a boot diagnostic; D-E the daemon's two lifecycle defects are fixed before the GUI control; D-F the GUI key form refuses a switched-off provider; D-G "setup completed" is derived from `llm.effective_default_model !== null`). Four questions that wave raised are decided too. Each row names the commit that applied it; unlike the table above, these **are** the shipped behaviour.

| # | Question | Decision | Applied by |
|---|---|---|---|
| **T23** | Should `GET /v1/status` carry a `busy` block so a client can warn honestly before stopping the daemon? | **Adopted (owner).** A counts-only block, `busy: { running_tasks, pending_confirmations, connected_clients }` — one `COUNT(*)` over the non-terminal task rows (`TaskRepository::count_non_terminal`) and two in-memory reads (the confirmation broker's pending count, `EventBroadcaster::client_count`); `null` on a daemon with no broker, never a guessed zero; never the prompts' contents. It widens what `/v1/status` is *for* (the installation → also its workload), which is why it was a decision, and it answers the stop dialog without leaning on the unscoped reads T16 and T22 are about. | `743817ad` (`apps/openalpacad/src/routes/status.rs`); read by the GUI's stop dialog, `1d08365d` |
| **T28** | With the mover gone, a database at **both** the legacy root and `~/.openalpaca` no longer has to be chosen between. Keep the downgrade from a refused boot to one `WARN`? | **Adopted (owner).** Keep the `WARN` (`LegacyRoot::Residue { holds_database: true }`). The old refusal existed because the mover had to pick which database to boot on; with no mover there is no choice and nothing merges them. The **stranded** case still refuses: critical data at the legacy root and no database here means starting would create an empty one beside it. One line in `classify_legacy_root` reverses it. | `a0383e3e` (`crates/openalpaca_storage/src/store/legacy_root.rs`) |
| **T29** | `sysinfo` enters `openalpaca_storage`, making the leaf storage crate process-aware (the natural home, `openalpaca_platform`, was an unused scaffold deleted at `1bad6352`). | **Adopted by the commit that needed it.** One liveness predicate in the tree — `daemon_lifecycle::pid_is_daemon` — shared by the CLI, the factory reset's daemon check and the GUI shell, rather than a fourth crate for three functions or a copy in each binary. `sysinfo` was already a workspace dependency; `Cargo.lock` gained one edge and no package. | `26085722` (`crates/openalpaca_storage/src/daemon_lifecycle.rs`) |
| **T30** | Once the GUI sidecar writes `state/logs/daemon.log`, `GET /v1/status.log_path` is non-null for a sidecar-launched daemon. Widen `managed_log` from "`openalpaca daemon start` marked this run" to "whichever launcher pointed stdio at the file and rotated it", or add a second flag? | **Adopted by the commit that needed it.** Widen the meaning — the flag was always about *ownership* of the file, not which launcher; the doc comments, the Connection panel's note and the manuals changed in the same commit. `log_path` is `null` only for a daemon started by hand. | `79e3cc98` (`apps/openalpacad/src/state.rs`, `store::MANAGED_LOG_ENV`) |
...
| **040** | `040_llm_call_log_timestamp_index.sql` | 8 | `llm_call_log(timestamp)` index (ruling R63: `GET /v1/usage/summary` full-scanned the append-only call log under the single connection mutex on every Settings poll — both existing indexes lead on `agent_id`/`task_id`) |
| **041** | `041_execution_log_timestamp_indexes.sql` | fix wave | `tool_execution_log(timestamp, tool_name)` and `skill_execution_log(timestamp, skill_id)` indexes (ruling R80: the same R63 shape one table over — `GET /v1/tools` and `GET /v1/skills` count "today" by full-scanning the append-only logs under the single connection mutex on every Settings poll). The index alone does not move the planner, which prefers 030's grouping-ordered covering index with no statistics, so both counts statements name the new index with `INDEXED BY` (`repository/skill_execution/mod.rs:198`, `:229`). `subagent_span` is deliberately **not** indexed: `run_counts_by_template` leads on `state != 'running'` over a table that is updated rather than appended, so it needs a different index if it needs one at all. **This plan's last migration**; head at the time it finished. Head today is **042** (`042_unattended_declaration.sql` — `lane_followups.unattended` and `task.unattended`, landed by round 3 of the local-model readiness wave, outside this plan); the next migration is 043 (`crates/openalpaca_storage/src/migrations/mod.rs`; asserted at `database/tests.rs:11`) |
...
## 12. Residue — recorded, adjudicated, not defects

The whole-branch review raised each of these against shipped code, and each was
refuted — as **this plan working exactly as it specifies**, or as disclosed
residue whose next move belongs to an owner decision (ruling **R86**: a finding
refuted as owner-decision territory is recorded, never adopted). None is a
to-do. They are listed so the next reader does not re-raise them, and so that
anyone who *does* want one changed knows which decision they are reopening.

- **Steering is at-most-once, by specification.** `steering_drained` is written
  before the round consumes the interjection, and recovery is
  membership-based — §5.6(b) verbatim; replay's message set is §5.6(c)
  verbatim, with `resume_enabled` default off. Every in-process exit re-queues
  drained-but-unsent messages (`return_pending_steering`, six sites), so the
  window is hard-crash-only, one message wide, and user-visible: the `steering`
  record stays in the log. Moving the record after the response would reopen
  the double-delivery class R56 closed. If at-most-once is ever the wrong
  trade, it is §5.6(b) that gets rewritten, not the code.
- **`file_write` follows a symlink at the final path component**
  (`crates/openalpaca_core/src/tools/builtins/helpers/mod.rs:99`) —
  pre-existing, byte-identical to `main`, and outside this plan's only symlink
  promise, which is `confine_to_root` for the store writers (§1.4, whose
  symlinked-parent test Phase 2 carries). The snapshot tier deliberately does
  not image such a target:
  imaging would copy out-of-workspace bytes *into*
  `sessions/<id>/snapshots/`. Eight of the nine templates that grant
  `file_write` also grant an unjailed `shell_execute`, so this is not the
  narrow edge of that surface. Disclosed at T56.
- **`ArtifactStore::verify` has no boot caller, by design.** §4.8 promises
  read-time marking (`resolve_content` stamps `missing_since`) and
  put-or-verify hand-edit recording (`put` rotates the hand edit first), and
  both ship. The plan never promised an artifact sweep — produced artifacts are
  *never* garbage-collected (§4.5) — so `verify` is the entry point a
  deliberate integrity check uses, not a pass something forgot to schedule.
  Disclosed at T54; same class as R24's deferred `get_by_sha256`.
- **`set_plugin_secret` has no production caller, and waits on T9.** Design §8
  X-29 promised the mechanism, the redacting `GET`, and "route unchanged in
  shape", and explicitly deferred *which store* a sensitive value goes to —
  design §13 Q12, which is owner decision **T9** above. No shipped manifest
  declares a sensitive key; a hand-written `{ secret_ref = … }` table plus
  `ext reload` clears `NeedsConfig` today. Wiring a writer before T9 is
  answered would be adopting the decision. Residue: `docs/CLI_Manual.md`'s
  plugin-config notes name no interim path for an owner who wants one now.
- **`[web_search] api_key` is stored in the clear** — recorded as owner
  decision **T19** above rather than here, because the answer changes a route.
