...
**Status:** design, rev 2 — **ready to implement, with one carve-out.** D1–D5 and N1–N4 are resolved and carry no alternatives. **N5 (the tool allow/enable model) is open and in design**; its only footprint here is GAP-18's read-only `/v1/tools`, which is unblocked and can ship as written. Nothing else waits on it. No production code written. · **Date:** 2026-09-01 · **Branch:** `feat/ui-rework`
...
### New calls raised by rev 2 — N1–N4 resolved 2026-09-01; **N5 is open (in design).** N1–N3 took their default; N4 is a product call answered by the owner; N5's *model* is fixed but its mechanism is still being designed and must not be implemented from this table.
...
| **N4** | **Cost-cap semantics.** `max_cost` is enforced per-workflow (`lead_agent_defaults` $5) and per-agent-turn (`agent_defaults` $1), but the Settings panel presents today's spend as if against a *daily* budget. Add a real daily cap, or relabel? | **Per-workflow — relabel; no daily cap is added.** `/v1/usage/summary.caps` carries `workflow_max_cost_usd` / `agent_max_cost_usd` and **no** `daily_*` key; today's spend is reported as an informational total with no denominator, so the design's progress bar stays omitted (it was never drawn). A daily budget would be a new enforcement point in the LLM router, not a label — out of scope. **Corollary:** the main-loop cost lockout (`tasks/bug-main-loop-cost-lockout.md`) is confirmed a bug, not a daily budget working as intended. |
| **N5** | **Tool allow / enable model.** Where does the `/v1/tools` toggle write, and what does it govern? | **Model fixed 2026-09-01; mechanism IN DESIGN — do not implement yet.** Two axes, not one: **allow** is per-agent (template `capabilities`, skills' `requires_capabilities` — exists today), **enable** is per-extension (**plugins, MCP servers, other installed extensions** get on/off toggles). **Builtins are not toggled**; agent config governs them. Open: granularity (whole server/plugin vs individual tool), what "disabled" means operationally (unloaded vs hidden), composition with the plugin **approval** gate and MCP **connection** state, and the fate of capabilities withdrawn from templates/skills. A first attempt (a flat per-tool `[execution.tools] disabled` list spanning builtins) collapsed both axes and was superseded the day it was written — kept at KB `ADR-029`. Note for the design: `global_tool_deny` is extension-only *by construction* (`registry/mod.rs:627` matches `ToolBackend::Mcp \| Plugin`), which under the two-axis model is arguably the right **reach** at the wrong **granularity**. |
...
2. **GAP-18 — `GET /v1/tools`, `/v1/skills`.** Clone the tool-registry `Arc` before its move into `Orchestrator::new` (`main.rs:373`); two `AppState` fields. `author` exists; `destructive_hint` → `requires_confirmation` (`sandbox/mod.rs:398-417`); `global_tool_deny` → `denied`; grouped `tool_execution_log` query → `invocations_today`. Sort by name (`DashMap` iteration jitters). Bare arrays (§7 rule). **Per-tool enable *writes* stay out of scope pending the N5 design** — the toggle ships **disabled rather than lying**. `/v1/tools` serves enable state read-only; `global_tool_deny` → `denied` stays the read, and nothing is deleted or restructured until the allow/enable design is accepted. When it is, it supersedes this bullet and likely reaches further than `/v1/tools` (plugin- and MCP-level routes, not just a per-tool switch).
...
- **Tool/extension enable writes** and **agent-template `enabled`** (GAP-20 part 2) — both need enforcement, without which the toggles are decorative. The former is blocked on the **N5** allow/enable design, not on cost; expect it to come back as its own scoped change once that design lands.
