...
### New calls raised by rev 2 — **N1–N5 all resolved.** N1–N3 took their default (2026-09-01); N4 is a product call answered by the owner (2026-09-01); N5's *model* was fixed 2026-09-01 and its *mechanism* accepted 2026-09-02 as `tasks/extension-enable-design.md` rev 6 — implement N5 from that document, never from this table.
...
Items 1–7 unchanged from rev 1; **A0–A4 added in rev 3** (lessons §4 and §8 Stream 1 — standalone, no design dependency, each its own commit, A0–A3 reviewed as security changes). Every item independently mergeable.
...
- **A1 — `deny_plugin` unloads (bug B).** `manager.rs:601-618` sets `status = Disabled`, emits `PluginDisabled{reason:"denied by user"}` and returns — the child keeps running, tools/skills/templates stay registered. Route `deny_plugin` through the teardown `disable_plugin` (`:645-690`) already contains (`unload_plugin` → kill → re-insert), **then** write the denial; report a consent word (`denied`), never `disabled`. Guard test: after deny, `list_plugins()` reports zero tools/skills/agents for the plugin and `process.try_wait()` reports exit. The status-word/bit semantics finish in the design's C3 (leave the `enabled` bit untouched once the tri-state entry exists).
