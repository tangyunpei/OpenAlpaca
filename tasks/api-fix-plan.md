# Daemon Fix Plan — rev 3.7: single root, artifact store, sessions, the 23 GUI gaps

**Status:** **implemented and merged — rev 3.8 is the record of what shipped.** Phases 0–8 all landed on `feat/ui-rework` (`2656a55..ab31299`; the per-phase ranges and their deviations are on the `### Phase N` headings in §6), and the extension design's C1–C8 landed with them between Phases 1 and 2 (`5c105b8..97740a5`; `tasks/extension-enable-design.md` rev 16). D1–D5 and N1–N5 stand as resolved and N5 was built from the design, never from the N5 table. Migration head is **042** (`042_unattended_declaration.sql`, round 3 of the local-model readiness wave — landed outside this plan, whose own last migration is §11's **041**, ruling R80). Exactly one planned item was deliberately never built: Phase 0 item 6 (GAP-22), left alone under owner decision T14 and made moot by C7, which deleted the six `plugin_*` event variants it would have amended. The **owner decisions are still pending — T1–T22 and T24 in §0 below (twins at design §13 Q5–Q14) are recommendations, decided nowhere and applied nowhere**; they and §12's recorded residue are what remains. `feat/ui-rework` itself is **merged**: PR #31 landed on `main` as `adf492cb` on 2026-09-20 and the branch is deleted, so every range in this document lies inside that commit's history. · **Date:** 2026-09-20 · **Branch:** merged to `main` (written on `feat/ui-rework`)
