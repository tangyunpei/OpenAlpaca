...
**Date:** 2026-09-01 · **Reconciled:** 2026-09-05 (extension design C1–C8, ADR-030) · **Branch:** feat/ui-rework · **Source of truth in code:** `apps/openalpaca-gui/src/lib/unavailable.ts` (**19**-entry gap registry; every unbacked surface in the UI renders its gap id + proposed endpoint) · **Full detail:** `apps/openalpaca-gui/API_MAP.md` §3.

**What the extension work closed** (C1–C8; the registry is the count that matters, and it now holds 19): **GAP-19 was renamed GAP-24** (extension install/uninstall — the toggle it originally asked for now exists, only installing does not); **GAP-22 is gone** — the six `plugin_*` events it wanted `ts`/`instance_id` on were deleted with the `/v1/plugins*` routes in C7, and the `extension_*` family carries both fields from birth; **GAP-18's `GET /v1/tools` half landed in C6** — the entry is now the skill-listing half alone (`GET /v1/skills`). Everything else below stands.
...
| ~~**GAP-22**~~ | ~~Six `ServerEvent::Plugin*` variants omit `ts`/`instance_id`~~ | **Closed by C7** — those variants no longer exist; `extension_state_changed` / `extension_capability_withheld` / `extension_capability_withdrawn` carry `ts` and `instance_id` | — |
...
| **GAP-18** | No skill listing (only `/v1/skills/health`, which has no names). `GET /v1/tools` **landed in C6** | `GET /v1/skills` |
...
**GAP-15** provider enable/disable · **GAP-17** connector call counts + "unwired" signal + add flow · **GAP-24** (was GAP-19) extension install/uninstall — for both kinds, now that enable/disable is served · **GAP-20** agent-template run counts and enabled state.

---

## Notes for whoever implements these

1. **Two error envelopes and two list shapes coexist** — chat/settings use `{error:{code,message}}`, tasks/agents use `{error:"string"}`; some routes return bare arrays, others envelopes. New routes should pick the envelope style and stay consistent (the client handles both today). The `/v1/extensions*` routes are the deliberate exception: they use the design §8 flat `{"error":"<word>"}` envelope, whose words the CLI and GUI match on; `GET /v1/tools` is a bare array.
