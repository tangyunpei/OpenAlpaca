...
- **GAP-09 — Subagent timeline. Blocks the Work view's "Parallel work" swimlanes.** `agent_task_history` has no `started_at` (only `runtime_seconds` + `completed_at`, so in-flight spans can't be derived). `DagNodeStatus` has the right shape but its producer was deleted in Routing V2 Phase 5 — do not build on it. Needs a `started_at` column written at spawn, `GET /v1/tasks/{id}/timeline`, and a `SubagentSpan` event.
