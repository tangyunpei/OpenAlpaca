...
- `stop` asks the daemon to stop and waits until its process has exited and its single-instance lock is free — at most 15 s — then looks for a GUI process to stop, by name: on macOS and Linux it sends SIGTERM to any process whose name contains `openalpaca-gui` or `OpenAlpaca`. The installed macOS app's process is `openalpaca_gui` (the Cargo binary name — `openalpaca-gui.app/Contents/MacOS/openalpaca_gui`) and matches neither, so an open app window stays open and shows `stopped elsewhere` (below), and `stop` prints `No GUI process found`. A daemon still there after 15 s is reported with its PID, the daemon log to read and the `kill -9` that finishes the job by hand, and `stop` exits with status 2.
...
- It **refuses while a daemon is running.** Stop it with `openalpaca daemon stop` first. Deleting a file the daemon has open would leave it writing into an unlinked inode while the next start created a second database beside it. The check is the daemon's own singleton lock, taken before the prompt and held until the files are gone, so a daemon that is still booting also refuses it, and a daemon started while the prompt is open — the app, or `daemon start` in another terminal — exits on the lock instead of opening the database.
