...
| **Subagents** | `resolve_agent_tools()` — the template's capabilities resolved to tools, plus the workspace tools | The template's capability **names** plus the workspace tools. The sandbox compares tool names, so this admits a tool only when its name is also a listed capability — see [TECHNICAL.md §11.2](./TECHNICAL.md#112-sandboxpolicy) |
